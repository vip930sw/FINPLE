...
- 베타 안내 문구
- Toss 결제 준비 / 승인 / Webhook / 구독 종료 예약 라우트
- 포트폴리오 AI 분석 live/mock API 연결 및 Personal 전용 접근 제어
...
프론트엔드에서 백엔드 API를 호출하려면 Vercel에 다음 값을 확인합니다.
...
- 이메일 회원가입 / 로그인 운영 DB 스키마 적용 여부
- Google 로그인 실제 연결
- Kakao 로그인 실제 연결
- Toss 심사 승인 후 live 결제 전환
- Toss Webhook 운영 URL 등록 및 실수신 검증
- Personal 구독 권한 동기화 검증
- Pro 플랜 고급 기능
- 모바일 시뮬레이터 전용 입력 UI
- 관리자용 문의 삭제 / 숨김 기능
- 사용량 기반 API 제한 고도화
