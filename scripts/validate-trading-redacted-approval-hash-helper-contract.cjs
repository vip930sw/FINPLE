...
function validateSampleOutputShape(sample, errors) {
  ...
  for (const label of REQUIRED_HASH_INPUT_LABELS) {
    if (!isLabelledPlaceholderHash(sample[label])) {
...
function validateRedactedApprovalHashHelperContract(contract) {
  const errors = [];
  ...
  if (!isPlainObject(boundary)) {
    ...
    if (boundary.futureApprovalPacketPath !== FUTURE_APPROVAL_PACKET_PATH) {
      errors.push(makeError("invalid_future_approval_packet_path", "futureLocalHashHelperBoundary.futureApprovalPacketPath", "future approval packet path must stay fixed"));
    }
    for (const flag of ["currentStepImplementsHelper", "currentStepCreatesHashes", "currentStepCreatesApprovalPacket"]) {
...
function main() {
  ...
  if (args.help) {
    console.log("Usage: node scripts/validate-trading-redacted-approval-hash-helper-contract.cjs --contract <path>");
    return;
  }
  if (args.unknown) {
  ...
  const result = validateRedactedApprovalHashHelperContract(contract);
