...
const SCRIPT_PATH = path.resolve("scripts", "check-trading-step125-manual-approval-clearance-review-result-recording-gate.cjs");
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-step125-check-"));
  [
...
test("passes with current Step 125 manual approval clearance review result recording gate", () => {
  const workspace = makeWorkspace();
  const result = runCheck(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /step125-manual-approval-clearance-review-result-recording-gate/);
});

test("blocks if clearance review UI appears in mypage source", () => {
  const workspace = makeWorkspace();
  const accountPagePath = path.join(workspace, "src", "components", "AccountPages.jsx");
  fs.appendFileSync(accountPagePath, "\n<TradingReadinessPanel manualApprovalClearanceReviewResultStatus />\n");
  const result = runCheck(workspace);
...
test("blocks if clearance review endpoint loses the admin guard", () => {
  const workspace = makeWorkspace();
