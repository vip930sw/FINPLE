...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-packet-validation-preflight-validator-fixtures.cjs",
);
const CONTRACT =
  "trading_lab_step116_manual_order_permission_packet_validation_preflight_validator_fixtures.json";
const PREFLIGHT = "trading_lab_step116_manual_order_permission_packet_validation_preflight.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-packet-validation-preflight-fixtures-"));
...
test("passes with current manual order permission packet validation preflight validator fixtures", () => {
...
test("records synthetic packet validation preflight validator fixtures while keeping packet reads and trading blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionPacketValidationPreflightValidatorRegression, true);
...
test("rejects stale fixtures if packet validation, provider, or order flags are manually enabled", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  report.currentState.ownerPacketReadAllowedNow = true;
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_manual_order_permission_packet_validation_preflight_validator_fixtures\.json is out of date/,
  );
});

test("blocks if upstream packet validation preflight opens too early", () => {
  const workspace = makeWorkspace();
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.checks.packetValidationPreflightReady, false);
