...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-shadow-history-review-result-contract.cjs",
);
const CONTRACT = "trading_lab_step116_manual_order_permission_shadow_history_review_result_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-shadow-history-review-result-"));
...
test("passes with current shadow-history review result contract", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /shadow_history_review_result_contract\.json/);
});

test("opens result contract without reviewing history, opening adapter review, or ordering", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionShadowHistoryReviewResultContract, true);
  assert.equal(report.readiness.readyForLiveGuardedAdapterReviewAfterShadowHistory, false);
  assert.equal(report.currentState.ownerShadowHistoryReviewResultRecordedNow, false);
  assert.equal(report.currentState.currentStepReviewsShadowHistory, false);
  assert.equal(report.currentState.currentStepRecordsShadowHistoryReviewResult, false);
  assert.equal(report.currentState.currentStepOpensLiveGuardedAdapterReview, false);
...
test("blocks if dry-run replay execution result boundary starts opening shadow history", () => {
  const workspace = makeWorkspace();
  const dryRun = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_dry_run_replay_execution_result_contract.json",
  );
  dryRun.readiness.readyForManualOrderPermissionDryRunReplayExecutionResultContract = false;
  dryRun.readiness.currentStepOpensShadowHistoryReview = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionShadowHistoryReviewResultContract, false);
  assert.match(
...
test("blocks if shadow-history review contract starts allowing runtime implementation", () => {
  const workspace = makeWorkspace();
...
test("blocks if live-guarded adapter review opens before the shadow-history result is separately recorded", () => {
  const workspace = makeWorkspace();
...
test("blocks if private packet, shadow review runtime, adapter, route, UI, DB, or scenario artifact appears", () => {
  const workspace = makeWorkspace();
  const files = [
    ...
    path.join(workspace, "server", "src", "services", "tradingShadowHistoryReviewService.js"),
    path.join(workspace, "server", "src", "services", "trading", "shadowHistoryReview.js"),
