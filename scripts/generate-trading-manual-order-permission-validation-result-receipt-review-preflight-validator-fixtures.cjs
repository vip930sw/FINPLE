...
} = require("./validate-trading-manual-order-permission-validation-result-receipt-review-preflight.cjs");
...
const CONTRACT_VERSION =
  "trading-lab-step116-manual-order-permission-validation-result-receipt-review-preflight-validator-fixtures-v0.1";
...
function invalidSyntheticFixtures(basePreflight) {
  ...
  missingReviewGate.futureValidationResultReceiptReviewBoundary.reviewGates =
    missingReviewGate.futureValidationResultReceiptReviewBoundary.reviewGates.filter(
      (gate) => gate !== "requires_separate_permission_import_review",
    );
  ...
  allowFlagEnabled.currentState.permissionPacketImportedNow = true;
...
function fixtureValidationEvidence(validPreflight, invalidFixtures) {
  const validResult = validateManualOrderPermissionValidationResultReceiptReviewPreflight(validPreflight);
  const invalidResults = invalidFixtures.map((fixture) => {
    const result = validateManualOrderPermissionValidationResultReceiptReviewPreflight(fixture.contract);
...
function buildContract() {
  ...
  const checks = {
    ...
    reviewPreflightReady:
      preflight.readiness?.readyForFutureManualOrderPermissionValidationResultReceiptReview === true &&
    ...
    validatorExportsLocalValidation:
      validatorSource.includes("validateManualOrderPermissionValidationResultReceiptReviewPreflight") &&
    ...
    architectureDocMentionsReviewPreflightValidatorFixtures:
      architectureDoc.includes("Trading Manual Order Permission Validation Result Receipt Review Preflight Validator Fixtures") &&
      architectureDoc.includes("manual_order_permission_validation_result_receipt_review_preflight_validator_fixtures"),
  ...
  const readyForManualOrderPermissionValidationResultReceiptReviewPreflightValidatorRegression =
  ...
  return stableJson({
    ...
    step: "Step 116-4Q",
    scope: "manual_order_permission_validation_result_receipt_review_preflight_validator_fixtures",
    sourceFiles: {
    ...
    validation: {
      redactionBoundary:
        "synthetic review-preflight validator fixtures only; no real validation receipt, private packet, raw account, operator, credential, provider payload, order payload, token, app key, app secret, execution, fill, or order confirmation content",
    ...
    readiness: {
      status: readyForManualOrderPermissionValidationResultReceiptReviewPreflightValidatorRegression
        ? "fixtures_ready_for_manual_order_permission_validation_result_receipt_review_preflight_validator_regression"
        : "blocked_before_manual_order_permission_validation_result_receipt_review_preflight_validator_fixture_regression",
      readyForManualOrderPermissionValidationResultReceiptReviewPreflightValidatorRegression,
      fixturesOnly: true,
      validationReceiptRecordedNow: false,
      validationReceiptReadAllowedNow: false,
      permissionPacketImportedNow: false,
      ...
      liveTradingAllowed: false,
      blockers: [
        ...(checks.reviewPreflightReady ? [] : ["manual_order_permission_validation_result_receipt_review_preflight_not_ready"]),
        ...(checks.validatorExportsLocalValidation ? [] : ["local_validator_export_not_ready"]),
        ...(checks.architectureDocMentionsReviewPreflightValidatorFixtures
          ? []
          : ["architecture_doc_missing_manual_order_permission_validation_result_receipt_review_preflight_validator_fixtures"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-manual-order-permission-validation-result-receipt-review-preflight-validator-fixtures.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-manual-order-permission-validation-result-receipt-review-preflight-validator-fixtures.cjs`,
      );
    }
    console.log("[generate-trading-manual-order-permission-validation-result-receipt-review-preflight-validator-fixtures] ok");
    console.log(
      `[generate-trading-manual-order-permission-validation-result-receipt-review-preflight-validator-fixtures] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-manual-order-permission-validation-result-receipt-review-preflight-validator-fixtures] wrote contract");
  console.log(
    `[generate-trading-manual-order-permission-validation-result-receipt-review-preflight-validator-fixtures] readyForManualOrderPermissionValidationResultReceiptReviewPreflightValidatorRegression=${parsed.readiness.readyForManualOrderPermissionValidationResultReceiptReviewPreflightValidatorRegression}`,
  );
}
