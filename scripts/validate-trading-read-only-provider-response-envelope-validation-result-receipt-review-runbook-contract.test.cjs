...
test("validates the current read-only provider response validation result receipt review runbook contract", () => {
...
test("rejects missing top-level fields and review assertions", () => {
  ...
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.requiredReviewAssertions =
    contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.requiredReviewAssertions.filter(
...
test("rejects validation, receipt read, receipt record, or provider call actions enabled now", () => {
  const contract = currentContract();
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.currentStepRunsValidator = true;
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.currentStepReadsReceipt = true;
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.currentStepRecordsReceipt = true;
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.currentStepCallsProvider = true;
...
test("rejects incomplete redacted output fields and forbidden output content lists", () => {
  const contract = currentContract();
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.redactedReviewOutputFields = [
    "responseValidationReceiptReviewId",
  ];
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.forbiddenReviewOutputContent = ["app_key"];
...
test("rejects changed future receipt path or validator commands", () => {
  const contract = currentContract();
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.futureValidationResultReceiptPath =
    "data/private/trading/live_order_receipt.json";
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.validatorCommandTemplate =
    "node scripts/validate-trading-read-only-provider-response-envelope-validation-result-receipt.cjs";
  contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.reviewPreflightValidatorCommandTemplate =
