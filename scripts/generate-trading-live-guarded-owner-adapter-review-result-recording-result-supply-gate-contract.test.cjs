...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-owner-adapter-review-result-recording-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-owner-adapter-recording-result-supply-"));
...
test("passes with current owner adapter review result recording result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /live_guarded_owner_adapter_review_result_recording_result_supply_gate_contract\.json/);
});

test("opens recording result supply gate without reading, recording, implementing, calls, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultSupplyGate, true);
...
test("blocks if recording preflight is no longer ready", () => {
  ...
  preflight.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingPreflight = false;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultSupplyGate, false);
  assert.match(
