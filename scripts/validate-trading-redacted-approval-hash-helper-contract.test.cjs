...
const SCRIPT_PATH = path.resolve("scripts", "validate-trading-redacted-approval-hash-helper-contract.cjs");
const CONTRACT_PATH = path.join("data", "processed", "trading_lab_step116_redacted_approval_hash_helper_contract.json");
...
function makeContractFile(contract) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "finple-redacted-hash-helper-contract-"));
  const filePath = path.join(dir, "redacted-approval-hash-helper-contract.json");
...
test("validates the current redacted approval hash helper contract", () => {
...
test("rejects missing top-level and hash input labels", () => {
  ...
  contract.futureLocalHashHelperBoundary.requiredHashInputLabels =
    contract.futureLocalHashHelperBoundary.requiredHashInputLabels.filter((label) => label !== "accountIdHash");
...
test("rejects helper implementation, hash creation, packet creation, or trading flags", () => {
  ...
  contract.futureLocalHashHelperBoundary.currentStepCreatesApprovalPacket = true;
...
test("rejects unsafe sample output values", () => {
  const contract = currentContract();
  contract.futureLocalHashHelperBoundary.sampleOutputShape.accountIdHash = "50195326";
...
test("rejects changed future helper or approval packet paths", () => {
  ...
  contract.futureLocalHashHelperBoundary.futureApprovalPacketPath = "data/private/trading/live_order_permission.json";
