...
const CONTRACT_VERSION =
  "trading-lab-step116-read-only-provider-response-envelope-validation-result-receipt-review-runbook-v0.1";
...
const REQUIRED_REVIEW_ASSERTIONS = [
  "review_uses_explicit_owner_supplied_response_validation_receipt_path_later",
  "review_runs_local_response_validation_receipt_validator_later",
  "review_does_not_record_receipt_path",
  "review_does_not_record_raw_response",
  "review_does_not_record_provider_payload",
...
function buildContract() {
  ...
  const checks = {
    ...
    reviewPreflightReady:
      reviewPreflight.readiness?.readyForFutureResponseEnvelopeValidationResultReceiptReview === true &&
      ...
      reviewPreflight.readiness?.responseEnvelopePathRecorded === false &&
      reviewPreflight.readiness?.rawResponseRecorded === false &&
      reviewPreflight.readiness?.providerPayloadRecorded === false &&
    ...
    reviewPreflightValidatorFixturesReady:
      reviewPreflightValidatorFixtures.readiness
        ?.readyForResponseEnvelopeValidationResultReceiptReviewPreflightValidatorRegression === true &&
      ...
      reviewPreflightValidatorFixtures.readiness?.rawResponseRecorded === false &&
      reviewPreflightValidatorFixtures.readiness?.providerPayloadRecorded === false &&
    ...
    reviewPreflightValidatorReady:
      reviewPreflightValidatorSource.includes(
        "validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewPreflight",
      ) &&
    ...
    architectureDocMentionsResponseValidationResultReceiptReviewRunbook:
      architectureDoc.includes("Trading Read-Only Provider Response Envelope Validation Result Receipt Review Runbook") &&
      architectureDoc.includes("read_only_provider_response_envelope_validation_result_receipt_review_runbook"),
  ...
  const readyForOwnerAssistedResponseValidationResultReceiptReviewRunbookReview =
    ...
    checks.architectureDocMentionsResponseValidationResultReceiptReviewRunbook &&
  ...
  return stableJson({
    ...
    step: "Step 116-3L-G",
    scope: "read_only_provider_response_envelope_validation_result_receipt_review_runbook",
    sourceFiles: {
    ...
    futureOwnerAssistedResponseValidationResultReceiptReviewRunbook: {
      ...
      validatorCommandTemplate:
        "node scripts/validate-trading-read-only-provider-response-envelope-validation-result-receipt.cjs --receipt <owner-supplied-redacted-response-validation-result-receipt-path>",
      reviewPreflightValidatorCommandTemplate:
        "node scripts/validate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight.cjs --contract data/processed/trading_lab_step116_read_only_provider_response_envelope_validation_result_receipt_review_preflight.json",
      currentStepRunsValidator: false,
      currentStepReadsReceipt: false,
      ...
      forbiddenReviewOutputContent,
      safetyNotes: [
        "the owner-supplied redacted response validation result receipt path is provided later and is not read by this contract",
        "review output may record hashes and redacted status later, but must not record receipt paths, raw response values, or provider payloads",
        "successful response receipt review is not provider call authorization clearance",
        "successful response receipt review is not runtime route, UI, DB, order, or live trading clearance",
      ],
    },
    checks,
    evidence: {
    ...
    readiness: {
      status: readyForOwnerAssistedResponseValidationResultReceiptReviewRunbookReview
        ? "runbook_ready_for_owner_assisted_response_validation_result_receipt_review"
        : "blocked_before_read_only_provider_response_validation_result_receipt_review_runbook",
      readyForOwnerAssistedResponseValidationResultReceiptReviewRunbookReview,
      runbookOnly: true,
      currentStepRunsValidator: false,
      currentStepReadsReceipt: false,
      validationReceiptRecordedNow: false,
      ...
      blockers: [
        ...
        ...(checks.reviewPreflightValidatorReady
          ? []
          : ["read_only_provider_response_envelope_validation_result_receipt_review_preflight_validator_not_ready"]),
        ...(checks.receiptValidatorReady
          ? []
          : ["read_only_provider_response_envelope_validation_result_receipt_validator_not_ready"]),
        ...
        ...(checks.architectureDocMentionsResponseValidationResultReceiptReviewRunbook
          ? []
          : ["architecture_doc_missing_read_only_provider_response_envelope_validation_result_receipt_review_runbook"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-contract.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-contract.cjs`,
      );
    }
    console.log("[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-contract] ok");
    console.log(
      `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-contract] wrote contract");
  console.log(
    `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-contract] readyForOwnerAssistedResponseValidationResultReceiptReviewRunbookReview=${parsed.readiness.readyForOwnerAssistedResponseValidationResultReceiptReviewRunbookReview}`,
  );
}
