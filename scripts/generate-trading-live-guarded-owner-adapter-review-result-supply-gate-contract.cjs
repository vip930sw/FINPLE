...
const ADAPTER_PREFLIGHT_PATH = path.join(
  "data",
  "processed",
...
const CONTRACT_VERSION = "trading-lab-step116-live-guarded-owner-adapter-review-result-supply-gate-v0.1";
const AUDITED_AT = "2026-07-02T00:00:00Z";
const REQUIRED_OWNER_ADAPTER_REVIEW_RESULT_SUPPLY_GATES = [
  "private_worker_implementation_preflight_contract_ready",
  "adapter_review_result_contract_ready",
  "adapter_implementation_preflight_still_closed",
...
const FORBIDDEN_OWNER_ADAPTER_REVIEW_RESULT_SUPPLY_CONTENT = [
...
function buildContract() {
  ...
  const adapterPreflight = readJson(ADAPTER_PREFLIGHT_PATH);
  const launchReadinessPlan = readJson(LAUNCH_READINESS_PLAN_PATH);
  const progressSummary = readJson(PROGRESS_SUMMARY_PATH);
  const architectureDoc = readText(ARCHITECTURE_DOC_PATH);
  const supplyGates = [...REQUIRED_OWNER_ADAPTER_REVIEW_RESULT_SUPPLY_GATES];
  const forbiddenSupplyContent = [...FORBIDDEN_OWNER_ADAPTER_REVIEW_RESULT_SUPPLY_CONTENT];
  const missingSupplyGates = missingValues(supplyGates, REQUIRED_OWNER_ADAPTER_REVIEW_RESULT_SUPPLY_GATES);
  const missingForbiddenSupplyContent = missingValues(
    forbiddenSupplyContent,
    FORBIDDEN_OWNER_ADAPTER_REVIEW_RESULT_SUPPLY_CONTENT,
  );
  const forbiddenArtifacts = forbiddenRuntimeArtifacts();

  const checks = {
    contractOnly: true,
    ownerAdapterReviewResultSupplyGateOnly: true,
    privateWorkerPreflightReady:
      ...
      privateWorkerPreflight.readiness?.currentStepCallsProvider === false &&
      privateWorkerPreflight.readiness?.currentStepSubmitsOrders === false,
    ...
    adapterPreflightStillClosed:
      ...
      adapterPreflight.readiness?.runtimeRouteAllowed === false &&
      adapterPreflight.readiness?.publicUiAllowed === false &&
      adapterPreflight.readiness?.dbMigrationAllowed === false,
    ...
    architectureDocMentionsOwnerAdapterReviewResultSupplyGate:
      architectureDoc.includes("Trading Live-Guarded Owner Adapter Review Result Supply Gate") &&
      architectureDoc.includes("live_guarded_owner_adapter_review_result_supply_gate"),
    noRuntimeArtifacts: forbiddenArtifacts.length === 0,
    kisPersonalPermissionExternalBlocker: false,
    ownerAdapterReviewResultSuppliedNow: false,
    currentStepReadsAdapterReviewResult: false,
  ...
  const readyForLiveGuardedOwnerAdapterReviewResultSupplyGate =
    checks.privateWorkerPreflightReady &&
    checks.adapterReviewResultBoundaryStillReady &&
    checks.adapterPreflightStillClosed &&
    ...
    checks.architectureDocMentionsOwnerAdapterReviewResultSupplyGate &&
  ...
  return stableJson({
    ...
    step: "Step 116-7D",
    scope: "live_guarded_owner_adapter_review_result_supply_gate",
    sourceFiles: {
      ...
      liveGuardedOrderAdapterImplementationPreflight: ADAPTER_PREFLIGHT_PATH,
    ...
    ownerAdapterReviewResultBoundary: {
      ...
      futureReviewResultRules: [
        "owner_supplied_redacted_result_only",
    ...
    evidence: {
      ...
      adapterPreflightStatus: statusOf(adapterPreflight),
      launchReadinessPlanStatus: statusOf(launchReadinessPlan),
      progressSummaryStatus: statusOf(progressSummary),
    },
    readiness: {
      status: readyForLiveGuardedOwnerAdapterReviewResultSupplyGate
        ? "live_guarded_owner_adapter_review_result_supply_gate_ready_pending_owner_result"
        : "blocked_before_live_guarded_owner_adapter_review_result_supply_gate",
      readyForLiveGuardedOwnerAdapterReviewResultSupplyGate,
      ...
      dbMigrationAllowed: false,
      liveTradingAllowed: false,
      pendingExternalInputs: ["owner_redacted_live_guarded_adapter_review_result"],
      blockers: [
        ...(checks.privateWorkerPreflightReady ? [] : ["private_worker_implementation_preflight_not_ready"]),
        ...(checks.adapterReviewResultBoundaryStillReady
          ? []
          : ["live_guarded_order_adapter_review_result_contract_not_ready"]),
        ...(checks.adapterPreflightStillClosed ? [] : ["live_guarded_order_adapter_preflight_not_closed"]),
        ...
        ...missingSupplyGates.map((gate) => `missing_owner_adapter_review_result_supply_gate_${gate}`),
        ...missingForbiddenSupplyContent.map(
          (content) => `missing_forbidden_owner_adapter_review_result_supply_content_${content}`,
        ),
        ...(checks.architectureDocMentionsOwnerAdapterReviewResultSupplyGate
          ? []
          : ["architecture_doc_missing_owner_adapter_review_result_supply_gate"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log("[generate-trading-live-guarded-owner-adapter-review-result-supply-gate-contract] ok");
    console.log(
      `[generate-trading-live-guarded-owner-adapter-review-result-supply-gate-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    `[generate-trading-live-guarded-owner-adapter-review-result-supply-gate-contract] wrote ${CONTRACT_PATH}`,
  );
}
