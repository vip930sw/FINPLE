...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-hash-helper-preflight-validator-fixtures.cjs",
);
const CONTRACT =
  "trading_lab_step116_manual_order_permission_hash_helper_preflight_validator_fixtures.json";
const HASH_HELPER_PREFLIGHT = "trading_lab_step116_manual_order_permission_hash_helper_preflight.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-hash-helper-preflight-fixtures-"));
...
test("passes with current manual order permission hash helper preflight validator fixtures", () => {
...
test("records synthetic hash helper preflight validator fixtures while keeping private hashes and trading blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionHashHelperPreflightValidatorRegression, true);
...
test("rejects stale fixtures if hash helper, provider, or order flags are manually enabled", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_manual_order_permission_hash_helper_preflight_validator_fixtures\.json is out of date/,
  );
});

test("blocks if upstream hash helper preflight opens too early", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(workspace, HASH_HELPER_PREFLIGHT);
  preflight.readiness.hashHelperImplementationAllowed = true;
...
test("blocks if hash helper, private packet, adapter, route, UI, DB, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
