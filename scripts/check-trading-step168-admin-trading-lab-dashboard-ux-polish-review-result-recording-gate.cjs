...
const REQUIRED_SNIPPETS = [
  ["server/src/services/tradingAdminLabDashboardShell.js", "STEP168_ADMIN_TRADING_LAB_DASHBOARD_UX_POLISH_REVIEW_RESULT_FLAGS"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_DASHBOARD_UX_POLISH_REVIEW_RESULT_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_DASHBOARD_UX_POLISH_REVIEW_RECEIPT_SCHEMA"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_DASHBOARD_UX_POLISH_REVIEW_DECISION_SUMMARY_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "validateTradingLabDashboardUxPolishReviewResult"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabDashboardUxPolishReviewResult"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabDashboardUxPolishReviewDecisionSummary"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabDashboardUxPolishReviewReceipt"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabDashboardUxPolishReviewResultRecordingGate"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildAdminTradingLabDashboardUxPolishReviewResultStatus"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "admin_only_trading_lab_dashboard_ux_polish_review_result_fail_closed"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "mock_dashboard_ux_polish_review_recorded"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "mock_dashboard_ux_polish_core"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "dashboardUxPolishExecuted: false"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "dashboardSectionDeleted: false"],
  ...
  ["server/src/routes/adminTradingReadinessRoutes.js", "buildAdminTradingLabDashboardUxPolishReviewResultStatus"],
  ["server/src/routes/adminTradingReadinessRoutes.js", 'router.get("/trading-lab-dashboard-ux-polish-review-result"'],
  ["server/src/routes/adminTradingReadinessRoutes.js", "requireAdminAccess"],
  ["server/src/routes/adminTradingReadinessRoutes.test.js", "trading-lab-dashboard-ux-polish-review-result"],
  ["src/components/portfolio/services/serverPortfolioService.js", "fetchAdminTradingLabDashboardUxPolishReviewResultStatus"],
  ["src/components/portfolio/services/serverPortfolioService.js", "\"/admin/trading-readiness/trading-lab-dashboard-ux-polish-review-result\""],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabDashboardUxPolishReviewResultStatus"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabDashboardUxPolishReviewCards"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabDashboardUxPolishReviewList"],
  ["src/components/TradingReadinessPanel.jsx", "data-admin-panel-key=\"trading-lab-dashboard-ux-polish-review-result\""],
  ["src/App.css", ".tradingLabDashboardUxPolishReviewCards"],
  ["src/App.css", ".tradingLabDashboardUxPolishReviewList"],
  ["package.json", "check:trading-step168-admin-trading-lab-dashboard-ux-polish-review-result-recording-gate"],
];
...
function main() {
  ...
  const step168CssLines = appCss
    .split(/\r?\n/)
    .filter((line) => line.includes("tradingLabDashboardUxPolishReview"));
  if (step168CssLines.length === 0) fail("Step 168 must include scoped dashboard UX polish review CSS");
  const broadStep168Selector = step168CssLines.find((line) => /\.(card|panel|table|dashboard|section)\b/.test(line));
  if (broadStep168Selector) {
    fail(`Step 168 must not add broad global card/panel/table/dashboard/section selectors: ${broadStep168Selector.trim()}`);
  }
  ...
  for (const filePath of MY_PAGE_FILES) {
    ...
    const text = readText(filePath);
    if (
      text.includes("trading-lab-dashboard-ux-polish-review-result")
      || text.includes("tradingLabDashboardUxPolishReview")
    ) {
      fail(`dashboard UX polish review UI must not touch mypage/account/subscription/billing surface: ${filePath}`);
    }
  }
  ...
  for (const filePath of scenarioRuntimeFiles) {
    if (fs.existsSync(filePath) && readText(filePath).includes("dashboard_ux_polish_review")) {
      fail(`scenario runtime files must remain untouched by Step 168: ${filePath}`);
    }
  }

  console.log("[check-trading-step168-admin-trading-lab-dashboard-ux-polish-review-result-recording-gate] ok");
}
