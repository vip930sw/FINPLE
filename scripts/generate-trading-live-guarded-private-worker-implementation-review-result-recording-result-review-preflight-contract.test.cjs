...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-review-preflight-contract.cjs",
);
const CONTRACT =
...
test("passes with current private worker recording result review preflight", () => {
  ...
  assert.match(
    result.stdout,
    /private_worker_implementation_review_result_recording_result_review_preflight_contract\.json/,
  );
});
...
test("opens review preflight without reading, recording, implementing, calls, or orders", () => {
  ...
  assert.equal(
    report.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultReviewPreflight,
    true,
  );
  ...
  assert.equal(report.currentState.ownerPrivateWorkerImplementationReviewResultRecordingResultReviewAcceptedNow, false);
...
test("blocks if recording result contract no longer passes", () => {
  ...
  contract.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultContract = false;
