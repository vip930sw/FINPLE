...
const REQUIRED_FILES = [
  ...
  "scripts/check-trading-step148-admin-trading-lab-mock-fill-simulation-core.test.cjs",
  "scripts/check-trading-step147-admin-trading-lab-mock-fill-simulation-core-review-result-recording-gate.cjs",
];

const REQUIRED_SNIPPETS = [
  ["server/src/services/tradingAdminLabDashboardShell.js", "STEP148_ADMIN_TRADING_LAB_MOCK_FILL_SIMULATION_CORE_FLAGS"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_FILL_RESULT_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_FILL_CALCULATION_INPUT_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_FILL_RESULT_SCHEMA"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabMockFillCalculationInputs"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "calculateTradingLabMockFillResult"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "validateTradingLabMockFillSimulationCore"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabMockFillSimulationCore"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildAdminTradingLabMockFillSimulationCoreStatus"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "admin_only_trading_lab_mock_fill_simulation_core_fail_closed"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "mock_portfolio_ledger_update_preflight"],
  ...
  ["server/src/routes/adminTradingReadinessRoutes.js", "buildAdminTradingLabMockFillSimulationCoreStatus"],
  ["server/src/routes/adminTradingReadinessRoutes.js", 'router.get("/trading-lab-mock-fill-simulation-core"'],
  ["server/src/routes/adminTradingReadinessRoutes.js", "requireAdminAccess"],
  ["server/src/routes/adminTradingReadinessRoutes.test.js", "trading-lab-mock-fill-simulation-core"],
  ["src/components/portfolio/services/serverPortfolioService.js", "fetchAdminTradingLabMockFillSimulationCoreStatus"],
  ["src/components/portfolio/services/serverPortfolioService.js", "\"/admin/trading-readiness/trading-lab-mock-fill-simulation-core\""],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabMockFillSimulationCoreStatus"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabMockFillCoreResultCards"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabMockFillCoreResultList"],
  ["src/components/TradingReadinessPanel.jsx", "Mock fill simulation core"],
  ["src/App.css", ".tradingLabMockFillCoreResultCards"],
  ["src/App.css", ".tradingLabMockFillCoreResultList"],
  ["package.json", "check:trading-step148-admin-trading-lab-mock-fill-simulation-core"],
];
...
function main() {
  ...
  if (!/router\.get\("\/trading-lab-mock-fill-simulation-core"[\s\S]*requireAdminAccess[\s\S]*buildAdminTradingLabMockFillSimulationCoreStatus/.test(routeText)) {
    fail("mock fill simulation core endpoint must stay admin-only and read-only");
  }
  ...
  if (accountExposure.length > 0) fail(`mock fill simulation core must not be exposed on /mypage: ${accountExposure.join(", ")}`);
  const appExposure = publicExposureTerms.filter((term) => appText.includes(term));
  if (appExposure.length > 0) fail(`mock fill simulation core must not be exposed on homepage or public router: ${appExposure.join(", ")}`);
  ...
  const touchedScenarioFiles = scenarioFiles.filter((filePath) => fs.existsSync(filePath) && readText(filePath).includes("Step 148"));
  if (touchedScenarioFiles.length > 0) fail(`scenario runtime files must remain untouched: ${touchedScenarioFiles.join(", ")}`);

  console.log("[check-trading-step148-admin-trading-lab-mock-fill-simulation-core] ok");
}
