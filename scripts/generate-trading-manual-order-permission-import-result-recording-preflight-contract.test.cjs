...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-import-result-recording-preflight-contract.cjs",
);
const CONTRACT = "trading_lab_step116_manual_order_permission_import_result_recording_preflight_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-import-result-recording-"));
...
test("passes with current manual order permission import result recording preflight", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /manual_order_permission_import_result_recording_preflight_contract\.json/);
});

test("opens import result recording preflight without reading, recording, importing, or implementing", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionImportResultRecordingPreflight, true);
...
test("blocks if result supply gate is no longer ready", () => {
  ...
  supplyGate.readiness.readyForManualOrderPermissionImportImplementationReviewResultSupply = false;
  writeJson(
    workspace,
    "trading_lab_step116_manual_order_permission_import_implementation_review_result_supply_gate_contract.json",
    supplyGate,
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionImportResultRecordingPreflight, false);
  assert.match(
...
test("blocks if earlier result recording preflight starts reading packets or importing too early", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_import_implementation_review_result_recording_preflight_contract.json",
  );
  preflight.readiness.currentStepReadsPrivatePacket = true;
