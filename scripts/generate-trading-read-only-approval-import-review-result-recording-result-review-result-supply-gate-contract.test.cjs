...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-recording-result-review-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-import-recording-result-review-result-supply-"));
...
test("passes with current read-only approval import recording-result review-result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /read_only_approval_import_review_result_recording_result_review_result_supply_gate/);
});

test("opens only a future review-result supply gate while import, provider, route, UI, DB, and order work stay blocked", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultSupply, true);
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalImportImplementationReview, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultSuppliedNow, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultReadNow, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordedNow, false);
...
test("blocks if review preflight starts reading or recording too early", () => {
  const workspace = makeWorkspace();
  ...
  preflight.readiness.ownerRedactedApprovalImportReviewResultRecordingResultReviewReadNow = true;
  preflight.readiness.ownerRedactedApprovalImportReviewResultRecordingResultReviewRecordedNow = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultSupply, false);
  assert.match(report.readiness.blockers.join("|"), /read_only_approval_import_review_result_recording_result_review_preflight_not_ready/);
});
...
test("blocks if private review result, importer, provider implementation, route, UI, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
