...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-risk-gate-clearance-review-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-risk-gate-review-result-supply-"));
...
test("passes with current risk-gate clearance review result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /risk_gate_clearance_review_result_supply_gate_contract\.json/);
});

test("opens result supply gate without accepting, recording, dry-run replay, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionRiskGateClearanceReviewResultSupply, true);
  assert.equal(report.readiness.readyForRiskGateClearanceReviewResultRecording, false);
  assert.equal(report.readiness.readyForDryRunReplayAfterRiskGate, false);
  assert.equal(report.currentState.ownerRiskGateClearanceReviewResultSuppliedNow, false);
  assert.equal(report.currentState.currentStepAcceptsRiskGateResult, false);
...
test("blocks if risk-gate review preflight is no longer ready", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_risk_gate_clearance_review_preflight_contract.json",
  );
  preflight.readiness.readyForManualOrderPermissionRiskGateClearanceReviewPreflight = false;
  preflight.readiness.currentStepRecordsRiskGateClearanceResult = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionRiskGateClearanceReviewResultSupply, false);
  assert.match(report.readiness.blockers.join("|"), /manual_order_permission_risk_gate_clearance_review_preflight_not_ready/);
});

test("blocks if kill-switch receipt boundary starts opening risk gate or orders", () => {
  const workspace = makeWorkspace();
