...
const IMPORT_RESULT_RECORDING_PREFLIGHT_PATH = path.join(
  "data",
  "processed",
  "trading_lab_step116_manual_order_permission_import_result_recording_preflight_contract.json",
);
const IMPORT_IMPLEMENTATION_REVIEW_RESULT_SUPPLY_GATE_PATH = path.join(
  "data",
  "processed",
...
const CONTRACT_VERSION = "trading-lab-step116-manual-order-permission-import-result-supply-gate-v0.1";
const AUDITED_AT = "2026-07-01T00:00:00Z";
const REQUIRED_IMPORT_RESULT_SUPPLY_GATES = [
  "import_result_recording_preflight_ready",
...
const FORBIDDEN_IMPORT_RESULT_SUPPLY_CONTENT = [
  ...
  "permission_import_result_hash_inputs",
  "order_confirmation",
  "execution_id",
  "fill_payload",
  "live_order_endpoint",
...
function buildContract() {
  const importResultRecordingPreflight = readJson(IMPORT_RESULT_RECORDING_PREFLIGHT_PATH);
  const importImplementationReviewResultSupplyGate = readJson(IMPORT_IMPLEMENTATION_REVIEW_RESULT_SUPPLY_GATE_PATH);
  ...
  const importResultSupplyGates = [...REQUIRED_IMPORT_RESULT_SUPPLY_GATES];
  const forbiddenImportResultSupplyContent = [...FORBIDDEN_IMPORT_RESULT_SUPPLY_CONTENT];
  const missingImportResultSupplyGates = missingValues(importResultSupplyGates, REQUIRED_IMPORT_RESULT_SUPPLY_GATES);
  const missingForbiddenImportResultSupplyContent = missingValues(
    forbiddenImportResultSupplyContent,
    FORBIDDEN_IMPORT_RESULT_SUPPLY_CONTENT,
  );
  const forbiddenArtifacts = forbiddenRuntimeArtifacts();

  const checks = {
    contractOnly: true,
    importResultSupplyGateOnly: true,
    importResultRecordingPreflightReady:
      importResultRecordingPreflight.readiness?.readyForManualOrderPermissionImportResultRecordingPreflight === true &&
      importResultRecordingPreflight.readiness?.readyForPermissionImportAfterReviewResult === false &&
      importResultRecordingPreflight.readiness?.currentStepReadsReviewResult === false &&
      importResultRecordingPreflight.readiness?.currentStepReadsPrivatePacket === false &&
      importResultRecordingPreflight.readiness?.currentStepRecordsImportResult === false &&
      importResultRecordingPreflight.readiness?.permissionPacketImportedNow === false &&
      importResultRecordingPreflight.readiness?.orderSubmissionAllowed === false,
    importImplementationReviewResultSupplyGateStillReady:
      importImplementationReviewResultSupplyGate.readiness
        ?.readyForManualOrderPermissionImportImplementationReviewResultSupply === true &&
      importImplementationReviewResultSupplyGate.readiness?.readyForPermissionImportAfterReviewResult === false &&
      importImplementationReviewResultSupplyGate.readiness?.ownerReviewResultSuppliedNow === false &&
      importImplementationReviewResultSupplyGate.readiness?.currentStepReadsPrivatePacket === false &&
      importImplementationReviewResultSupplyGate.readiness?.permissionPacketImportedNow === false &&
      importImplementationReviewResultSupplyGate.readiness?.orderSubmissionAllowed === false,
    ...
    importResultSupplyGatesReady: missingImportResultSupplyGates.length === 0,
    forbiddenImportResultSupplyContentReady: missingForbiddenImportResultSupplyContent.length === 0,
    architectureDocMentionsImportResultSupplyGate:
      architectureDoc.includes("Trading Manual Order Permission Import Result Supply Gate") &&
      architectureDoc.includes("manual_order_permission_import_result_supply_gate"),
  ...
  const readyForManualOrderPermissionImportResultSupply =
    checks.importResultRecordingPreflightReady &&
    checks.importImplementationReviewResultSupplyGateStillReady &&
    ...
    checks.importResultSupplyGatesReady &&
    checks.forbiddenImportResultSupplyContentReady &&
    checks.architectureDocMentionsImportResultSupplyGate &&
  ...
  return stableJson({
    ...
    step: "Step 116-6Q",
    scope: "manual_order_permission_import_result_supply_gate",
    sourceFiles: {
      manualOrderPermissionImportResultRecordingPreflight: IMPORT_RESULT_RECORDING_PREFLIGHT_PATH,
      manualOrderPermissionImportImplementationReviewResultSupplyGate:
        IMPORT_IMPLEMENTATION_REVIEW_RESULT_SUPPLY_GATE_PATH,
    ...
    manualOrderPermissionImportResultSupplyGate: {
      ...
      ownerImportImplementationReviewResultRequiredLater: true,
      ...
      nextAllowedAction:
        "after the owner supplies a redacted permission import result outside repo commits, record a separate kill-switch clearance review preflight without reading private packet paths or opening order submission",
      importResultSupplyGates,
      forbiddenImportResultSupplyContent,
    },
    checks,
    evidence: {
      missingImportResultSupplyGates,
      missingForbiddenImportResultSupplyContent,
      forbiddenRuntimeArtifacts: forbiddenArtifacts,
      importResultRecordingPreflightStatus: statusOf(importResultRecordingPreflight),
      importImplementationReviewResultSupplyGateStatus: statusOf(importImplementationReviewResultSupplyGate),
    ...
    readiness: {
      status: readyForManualOrderPermissionImportResultSupply
        ? "manual_order_permission_import_result_supply_gate_ready_pending_owner_import_result"
        : "blocked_before_manual_order_permission_import_result_supply_gate",
      readyForManualOrderPermissionImportResultSupply,
      ...
      blockers: [
        ...(checks.importResultRecordingPreflightReady
          ? []
          : ["manual_order_permission_import_result_recording_preflight_not_ready"]),
        ...(checks.importImplementationReviewResultSupplyGateStillReady
          ? []
        ...
        ...missingImportResultSupplyGates.map((gate) => `missing_import_result_supply_gate_${gate}`),
        ...missingForbiddenImportResultSupplyContent.map(
          (content) => `missing_forbidden_import_result_supply_content_${content}`,
        ),
        ...(checks.architectureDocMentionsImportResultSupplyGate
          ? []
          : ["architecture_doc_missing_import_result_supply_gate"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log("[generate-trading-manual-order-permission-import-result-supply-gate-contract] ok");
    console.log(`[generate-trading-manual-order-permission-import-result-supply-gate-contract] contract=${CONTRACT_PATH}`);
    return;
  }
  ...
  console.log("[generate-trading-manual-order-permission-import-result-supply-gate-contract] wrote contract");
  console.log(
    `[generate-trading-manual-order-permission-import-result-supply-gate-contract] readyForManualOrderPermissionImportResultSupply=${parsed.readiness.readyForManualOrderPermissionImportResultSupply}`,
  );
}
