...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-validation-result-receipt-review-preflight-validator-fixtures.cjs",
);
const CONTRACT =
  "trading_lab_step116_manual_order_permission_validation_result_receipt_review_preflight_validator_fixtures.json";
const PREFLIGHT = "trading_lab_step116_manual_order_permission_validation_result_receipt_review_preflight.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-validation-result-review-preflight-fixtures-"));
...
test("passes with current validation result receipt review preflight validator fixtures", () => {
...
test("records synthetic review preflight validator fixtures while keeping receipt reads, imports, provider calls, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(
    report.readiness.readyForManualOrderPermissionValidationResultReceiptReviewPreflightValidatorRegression,
    true,
  );
...
test("rejects stale fixtures if receipt, provider, or order flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_manual_order_permission_validation_result_receipt_review_preflight_validator_fixtures\.json is out of date/,
  );
});
...
test("blocks if upstream review preflight opens too early", () => {
  ...
  assert.match(report.readiness.blockers.join("|"), /manual_order_permission_validation_result_receipt_review_preflight_not_ready/);
});
