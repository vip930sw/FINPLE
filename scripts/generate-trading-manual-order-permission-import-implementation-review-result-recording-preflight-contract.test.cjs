...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-import-implementation-review-result-recording-preflight-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-import-review-result-recording-"));
...
test("passes with current import implementation review result recording preflight", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /manual_order_permission_import_implementation_review_result_recording_preflight_contract\.json/);
});

test("opens result recording preflight without reading review result, packet, or importing permission", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionImportImplementationReviewResultRecordingPreflight, true);
...
test("blocks if import implementation review contract is no longer ready", () => {
  const workspace = makeWorkspace();
  const review = readJson(workspace, "trading_lab_step116_manual_order_permission_import_implementation_review_contract.json");
  review.readiness.readyForManualOrderPermissionImportImplementationReviewContract = false;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionImportImplementationReviewResultRecordingPreflight, false);
