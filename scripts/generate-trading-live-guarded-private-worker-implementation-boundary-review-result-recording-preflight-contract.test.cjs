...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-preflight-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-boundary-review-result-recording-"));
...
test("passes with current private worker implementation boundary review result recording preflight", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /private_worker_implementation_boundary_review_result_recording_preflight_contract\.json/);
});

test("opens recording preflight without accepting, reading, recording, implementing, calls, runtime, routes, UI, DB, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingPreflight, true);
  ...
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryReviewResultAcceptedNow, false);
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryReviewResultReadNow, false);
...
test("blocks if boundary review result supply gate starts accepting or recording result", () => {
  const workspace = makeWorkspace();
  const supply = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_boundary_review_result_supply_gate_contract.json",
  );
  supply.readiness.readyForPrivateWorkerImplementationAfterBoundaryReviewResult = true;
  ...
  supply.readiness.currentStepStartsWorkerRuntime = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingPreflight, false);
  assert.match(
...
test("blocks if boundary review starts opening worker implementation", () => {
  const workspace = makeWorkspace();
  const review = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_boundary_review_contract.json",
  );
  review.readiness.readyForPrivateWorkerImplementationAfterBoundaryReview = true;
  ...
  review.readiness.currentStepStartsWorkerRuntime = true;
  writeJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_boundary_review_contract.json",
    review,
...
test("blocks if private result, worker, adapter, route, UI, DB, or scenario artifact appears", () => {
  const workspace = makeWorkspace();
  const files = [
    path.join(workspace, "data", "private", "trading", "live_guarded_private_worker_boundary_review_result.redacted.json"),
