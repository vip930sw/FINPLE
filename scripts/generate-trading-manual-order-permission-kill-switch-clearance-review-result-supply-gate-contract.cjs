...
const CONTRACT_VERSION =
  "trading-lab-step116-manual-order-permission-kill-switch-clearance-review-result-supply-gate-v0.1";
const AUDITED_AT = "2026-07-01T00:00:00Z";
const REQUIRED_KILL_SWITCH_REVIEW_RESULT_SUPPLY_GATES = [
...
const FORBIDDEN_KILL_SWITCH_REVIEW_RESULT_SUPPLY_CONTENT = [
...
function buildContract() {
  ...
  const killSwitchReviewResultSupplyGates = [...REQUIRED_KILL_SWITCH_REVIEW_RESULT_SUPPLY_GATES];
  const requiredKillSwitchReviewResultFields = [...REQUIRED_KILL_SWITCH_REVIEW_RESULT_FIELDS];
  const forbiddenKillSwitchReviewResultSupplyContent = [...FORBIDDEN_KILL_SWITCH_REVIEW_RESULT_SUPPLY_CONTENT];
  const missingKillSwitchReviewResultSupplyGates = missingValues(
    killSwitchReviewResultSupplyGates,
    REQUIRED_KILL_SWITCH_REVIEW_RESULT_SUPPLY_GATES,
  );
  ...
  const missingForbiddenKillSwitchReviewResultSupplyContent = missingValues(
    forbiddenKillSwitchReviewResultSupplyContent,
    FORBIDDEN_KILL_SWITCH_REVIEW_RESULT_SUPPLY_CONTENT,
  );
  const forbiddenArtifacts = forbiddenRuntimeArtifacts();

  const checks = {
    contractOnly: true,
    killSwitchReviewResultSupplyGateOnly: true,
    killSwitchReviewPreflightReady:
      ...
      killSwitchReviewPreflight.readiness?.readyForKillSwitchClearanceReviewResultSupply === false &&
      killSwitchReviewPreflight.readiness?.ownerKillSwitchClearanceReviewResultSuppliedNow === false &&
    ...
    killSwitchReviewResultSupplyGatesReady: missingKillSwitchReviewResultSupplyGates.length === 0,
    killSwitchReviewResultFieldsReady: missingKillSwitchReviewResultFields.length === 0,
    forbiddenKillSwitchReviewResultSupplyContentReady:
      missingForbiddenKillSwitchReviewResultSupplyContent.length === 0,
    architectureDocMentionsKillSwitchReviewResultSupplyGate:
      architectureDoc.includes("Trading Manual Order Permission Kill Switch Clearance Review Result Supply Gate") &&
      architectureDoc.includes("manual_order_permission_kill_switch_clearance_review_result_supply_gate"),
  ...
  const readyForManualOrderPermissionKillSwitchClearanceReviewResultSupply =
    ...
    checks.killSwitchReviewResultSupplyGatesReady &&
    checks.killSwitchReviewResultFieldsReady &&
    checks.forbiddenKillSwitchReviewResultSupplyContentReady &&
    checks.architectureDocMentionsKillSwitchReviewResultSupplyGate &&
  ...
  return stableJson({
    ...
    step: "Step 116-6S",
    scope: "manual_order_permission_kill_switch_clearance_review_result_supply_gate",
    sourceFiles: {
    ...
    manualOrderPermissionKillSwitchClearanceReviewResultSupplyGate: {
      ...
      nextAllowedAction:
        "after the owner supplies a redacted kill-switch clearance review result outside repo commits, record a separate kill-switch clearance review result recording preflight without clearing the kill switch, opening risk-gate review, or opening order submission",
      killSwitchReviewResultSupplyGates,
      requiredKillSwitchReviewResultFields,
      forbiddenKillSwitchReviewResultSupplyContent,
    },
    checks,
    evidence: {
      missingKillSwitchReviewResultSupplyGates,
      missingKillSwitchReviewResultFields,
      missingForbiddenKillSwitchReviewResultSupplyContent,
    ...
    readiness: {
      status: readyForManualOrderPermissionKillSwitchClearanceReviewResultSupply
        ? "manual_order_permission_kill_switch_clearance_review_result_supply_gate_ready_pending_owner_result"
        : "blocked_before_manual_order_permission_kill_switch_clearance_review_result_supply_gate",
      readyForManualOrderPermissionKillSwitchClearanceReviewResultSupply,
      readyForKillSwitchClearanceReviewResultRecording: false,
      ...
      blockers: [
        ...
        ...missingKillSwitchReviewResultSupplyGates.map(
          (gate) => `missing_kill_switch_review_result_supply_gate_${gate}`,
        ),
        ...missingKillSwitchReviewResultFields.map((field) => `missing_kill_switch_review_result_field_${field}`),
        ...missingForbiddenKillSwitchReviewResultSupplyContent.map(
          (content) => `missing_forbidden_kill_switch_review_result_supply_content_${content}`,
        ),
        ...(checks.architectureDocMentionsKillSwitchReviewResultSupplyGate
          ? []
          : ["architecture_doc_missing_kill_switch_clearance_review_result_supply_gate"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log("[generate-trading-manual-order-permission-kill-switch-clearance-review-result-supply-gate-contract] ok");
    console.log(
      `[generate-trading-manual-order-permission-kill-switch-clearance-review-result-supply-gate-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-manual-order-permission-kill-switch-clearance-review-result-supply-gate-contract] wrote contract");
  console.log(
    `[generate-trading-manual-order-permission-kill-switch-clearance-review-result-supply-gate-contract] readyForManualOrderPermissionKillSwitchClearanceReviewResultSupply=${parsed.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewResultSupply}`,
  );
}
