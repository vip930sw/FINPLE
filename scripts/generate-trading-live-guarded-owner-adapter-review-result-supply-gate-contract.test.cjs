...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-owner-adapter-review-result-supply-gate-contract.cjs",
);
const CONTRACT = "trading_lab_step116_live_guarded_owner_adapter_review_result_supply_gate_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-owner-adapter-review-result-supply-"));
...
test("passes with current owner adapter review result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /live_guarded_owner_adapter_review_result_supply_gate_contract\.json/);
});

test("opens supply gate without reading, recording, implementing, provider calls, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultSupplyGate, true);
  ...
  assert.equal(report.currentState.ownerAdapterReviewResultSuppliedNow, false);
  assert.equal(report.currentState.currentStepReadsAdapterReviewResult, false);
...
test("blocks if private worker preflight starts opening implementation", () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultSupplyGate, false);
  assert.match(report.readiness.blockers.join("|"), /private_worker_implementation_preflight_not_ready/);
});
