...
function makeReceiptFile(receipt) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-permission-validation-result-receipt-"));
...
test("validates a redacted manual order permission validation result receipt", () => {
...
test("rejects enabled permission, provider, order, runtime, and UI flags", () => {
  const result = runValidator([
    "--receipt",
    makeReceiptFile(
      validReceipt({
        ...
        permissionPacketImportedNow: true,
