...
function makeContractFile(contract) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "finple-provider-response-receipt-review-result-validator-"));
  const filePath = path.join(dir, "provider-response-receipt-review-result-contract.json");
...
test("validates the current read-only provider response validation result receipt review result contract", () => {
...
test("rejects missing top-level fields and review result assertions", () => {
  ...
  contract.futureResponseValidationResultReceiptReviewResultBoundary.requiredReviewResultAssertions =
    contract.futureResponseValidationResultReceiptReviewResultBoundary.requiredReviewResultAssertions.filter(
...
test("rejects receipt read, review result record, or provider-call actions enabled now", () => {
  const contract = currentContract();
  contract.futureResponseValidationResultReceiptReviewResultBoundary.currentStepReadsReceipt = true;
  contract.futureResponseValidationResultReceiptReviewResultBoundary.currentStepRecordsReviewResult = true;
  contract.futureResponseValidationResultReceiptReviewResultBoundary.currentStepCallsProvider = true;
...
test("rejects incomplete review result fields and forbidden output content lists", () => {
  const contract = currentContract();
  contract.futureResponseValidationResultReceiptReviewResultBoundary.requiredReviewResultFields = [
    "responseValidationReceiptReviewId",
  ];
  contract.futureResponseValidationResultReceiptReviewResultBoundary.forbiddenReviewResultContent = ["app_key"];
...
test("rejects changed future receipt path", () => {
  const contract = currentContract();
  contract.futureResponseValidationResultReceiptReviewResultBoundary.futureValidationResultReceiptPath =
