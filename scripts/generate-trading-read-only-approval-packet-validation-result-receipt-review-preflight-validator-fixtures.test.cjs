...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-packet-validation-result-receipt-review-preflight-validator-fixtures.cjs",
);
const CONTRACT =
  "trading_lab_step116_read_only_approval_packet_validation_result_receipt_review_preflight_validator_fixtures.json";
const PREFLIGHT = "trading_lab_step116_read_only_approval_packet_validation_result_receipt_review_preflight.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-read-only-approval-receipt-review-fixtures-"));
  const processedDir = path.join(workspace, "data", "processed");
  fs.mkdirSync(processedDir, { recursive: true });
  for (const fileName of [CONTRACT, PREFLIGHT]) {
...
test("passes with current read-only approval receipt review preflight validator fixtures", () => {
...
test("records synthetic review preflight validator fixtures while keeping private reads, provider calls, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForReadOnlyApprovalValidationResultReceiptReviewPreflightValidatorRegression, true);
...
test("keeps fixture catalog redacted and validates expected local failures", () => {
  ...
  assert.match(invalidIds.join("|"), /missing_review_gate/);
  assert.match(invalidIds.join("|"), /boundary_action_enabled/);
  assert.match(invalidIds.join("|"), /missing_forbidden_review_content/);
  assert.match(invalidIds.join("|"), /invalid_future_validation_result_receipt_path/);
  assert.match(invalidIds.join("|"), /invalid_future_approval_packet_path/);
  assert.match(invalidIds.join("|"), /allow_flag_enabled/);
  assert.match(invalidIds.join("|"), /forbidden_raw_value/);
  assert.match(invalidIds.join("|"), /field_must_be_array/);
...
test("rejects stale fixtures if receipt, approval import, provider, or order flags are manually enabled", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_approval_packet_validation_result_receipt_review_preflight_validator_fixtures\.json is out of date/,
  );
});

test("blocks if upstream review preflight opens too early", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(workspace, PREFLIGHT);
  ...
  writeJson(workspace, PREFLIGHT, preflight);
  ...
  assert.match(report.readiness.blockers.join("|"), /read_only_approval_validation_result_receipt_review_preflight_not_ready/);
});

test("blocks if private receipt, approval packet, provider implementation, route, UI, migration, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
