...
const CONTRACT_VERSION =
  "trading-lab-step116-read-only-approval-import-review-result-recording-result-supply-gate-v0.1";
...
const REQUIRED_RECORDING_RESULT_SUPPLY_GATES = [
  ...
  "hash_only_recording_result_required_later",
];
...
function buildContract() {
  ...
  const recordingResultSupplyGates = [...REQUIRED_RECORDING_RESULT_SUPPLY_GATES];
  const forbiddenRecordingResultSupplyContent = [...FORBIDDEN_RECORDING_RESULT_SUPPLY_CONTENT];
  const missingRecordingResultSupplyGates = missingValues(
    recordingResultSupplyGates,
    REQUIRED_RECORDING_RESULT_SUPPLY_GATES,
  );
  const missingForbiddenRecordingResultSupplyContent = missingValues(
    forbiddenRecordingResultSupplyContent,
  ...
  const checks = {
    ...
    recordingResultSupplyGatesReady: missingRecordingResultSupplyGates.length === 0,
    forbiddenRecordingResultSupplyContentReady: missingForbiddenRecordingResultSupplyContent.length === 0,
    architectureDocMentionsRecordingResultSupplyGate:
      architectureDoc.includes("Trading Read-Only Approval Import Review Result Recording Result Supply Gate") &&
      architectureDoc.includes("read_only_approval_import_review_result_recording_result_supply_gate"),
    noRuntimeArtifacts: forbiddenArtifacts.length === 0,
    ownerRedactedApprovalImportReviewResultRecordingResultSuppliedNow: false,
  ...
  const readyForReadOnlyApprovalImportReviewResultRecordingResultSupply =
    ...
    checks.recordingResultSupplyGatesReady &&
    checks.forbiddenRecordingResultSupplyContentReady &&
  ...
  return stableJson({
    ...
    step: "Step 116-5Q-N",
    scope: "read_only_approval_import_review_result_recording_result_supply_gate",
    sourceFiles: {
    ...
    importReviewResultRecordingResultSupplyGate: {
      ...
      nextAllowedAction:
        "after the owner supplies a redacted approval import review recording result outside repo commits, record a separate hash-only recording result contract without opening approval import or provider calls",
      recordingResultSupplyGates,
      forbiddenRecordingResultSupplyContent,
    },
    checks,
    evidence: {
      missingRecordingResultSupplyGates,
      missingForbiddenRecordingResultSupplyContent,
    ...
    readiness: {
      status: readyForReadOnlyApprovalImportReviewResultRecordingResultSupply
        ? "read_only_approval_import_review_result_recording_result_supply_gate_ready_import_still_blocked_pending_owner_recording_result"
        : "blocked_before_read_only_approval_import_review_result_recording_result_supply_gate",
      readyForReadOnlyApprovalImportReviewResultRecordingResultSupply,
      ...
      blockers: [
        ...
        ...missingRecordingResultSupplyGates.map((gate) => `missing_recording_result_supply_gate_${gate}`),
        ...missingForbiddenRecordingResultSupplyContent.map(
          (content) => `missing_forbidden_recording_result_supply_content_${content}`,
        ),
        ...(checks.architectureDocMentionsRecordingResultSupplyGate
          ? []
          : ["architecture_doc_missing_read_only_approval_import_review_result_recording_result_supply_gate"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log("[generate-trading-read-only-approval-import-review-result-recording-result-supply-gate-contract] ok");
    console.log(
      `[generate-trading-read-only-approval-import-review-result-recording-result-supply-gate-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-read-only-approval-import-review-result-recording-result-supply-gate-contract] wrote contract");
  console.log(
    `[generate-trading-read-only-approval-import-review-result-recording-result-supply-gate-contract] readyForReadOnlyApprovalImportReviewResultRecordingResultSupply=${parsed.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultSupply}`,
  );
}
