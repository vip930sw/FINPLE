...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-validation-result-receipt-review-preflight.cjs",
);
const CONTRACT = "trading_lab_step116_manual_order_permission_validation_result_receipt_review_preflight.json";
const INPUTS = [
  CONTRACT,
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-validation-result-receipt-review-preflight-"));
...
test("passes with current validation result receipt review preflight", () => {
  const workspace = makeWorkspace();
  const result = runPreflight(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /trading_lab_step116_manual_order_permission_validation_result_receipt_review_preflight\.json/);
});

test("records future receipt review preflight while keeping receipt reads, imports, provider calls, routes, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runPreflight(workspace, []);
...
test("keeps review boundary redacted and requires separate import review", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  ...
  assert.match(reviewGates, /no_permission_import/);
  assert.match(reviewGates, /requires_separate_permission_import_review/);
...
test("rejects stale preflight if receipt, import, provider, or order flags are manually enabled", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  report.currentState.validationReceiptReadAllowedNow = true;
  report.currentState.permissionPacketImportedNow = true;
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_manual_order_permission_validation_result_receipt_review_preflight\.json is out of date/,
  );
});

test("blocks if upstream receipt contract or receipt validator fixtures open too early", () => {
  const workspace = makeWorkspace();
  const receiptContract = readJson(workspace, "trading_lab_step116_manual_order_permission_validation_result_receipt.json");
  receiptContract.readiness.packetPathRecorded = true;
  writeJson(workspace, "trading_lab_step116_manual_order_permission_validation_result_receipt.json", receiptContract);
  const fixtures = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_validation_result_receipt_validator_fixtures.json",
  );
  fixtures.readiness.providerCallsAllowed = true;
  writeJson(workspace, "trading_lab_step116_manual_order_permission_validation_result_receipt_validator_fixtures.json", fixtures);
  ...
  assert.match(report.readiness.blockers.join("|"), /manual_order_permission_validation_result_receipt_validator_fixtures_not_ready/);
});
