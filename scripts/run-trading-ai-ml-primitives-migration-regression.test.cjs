...
test("Step215 runner builds a complete explicit regression plan", () => {
  ...
  assert.equal(plan.uniqueCheckerTestCount, 23);
...
test("Scenario F: successful result contract", () => {
  ...
  try {
    ...
    assert.equal(result.sourceCheckerCount, 12);
    assert.equal(result.uniqueServiceTestCount, 9);
    assert.equal(result.uniqueMigrationCheckerTestCount, 13);
    assert.equal(result.uniqueSupportingTestCount, 10);
    assert.equal(result.uniqueCheckerTestCount, 23);
    assert.equal(result.uniqueTestFileCount, 32);
...
test("Scenario H: checker test count", () => {
  ...
  assert.equal(plan.uniqueMigrationCheckerTestCount, 13);
...
test("Scenario I: child failure result", () => {
  ...
  try {
    assert.throws(() => {
      ...
      assert.equal(error.result.uniqueCheckerTestCount, 23);
...
test("Scenario J: CLI public summary", () => {
  ...
  assert.equal(summary.uniqueCheckerTestCount, 23);
