...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-boundary-review-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-boundary-review-"));
...
test("passes with current private worker implementation boundary review gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /private_worker_implementation_boundary_review/);
});

test("opens boundary review without implementing, calls, runtime, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReview, true);
  assert.equal(report.readiness.readyForPrivateWorkerImplementationAfterBoundaryReview, false);
  assert.equal(report.currentState.kisPersonalPermissionExternalBlocker, false);
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryReviewSuppliedNow, false);
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryReviewReadNow, false);
...
test("blocks if boundary preflight no longer passes", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_boundary_preflight_contract.json",
  );
  preflight.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryPreflight = false;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReview, false);
  assert.match(
...
test("blocks if private worker implementation review opens implementation", () => {
  const workspace = makeWorkspace();
...
test("blocks if order adapter review opens provider calls or orders", () => {
  const workspace = makeWorkspace();
  ...
  review.readiness.providerCallsAllowed = true;
