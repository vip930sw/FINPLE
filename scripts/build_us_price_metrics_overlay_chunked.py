...
def main() -> None:
    ...
    source_tag = f"yfinance_close_price_{as_of_included.strftime('%Y%m%d')}"
    ...
    print(f"Date range: {start_date} to {as_of_included.isoformat()} inclusive; provider end={provider_end_exclusive}, benchmark={BENCHMARK_TICKER}", flush=True)

    benchmark_close = download_close(BENCHMARK_TICKER, start=start_date, end=provider_end_exclusive)
