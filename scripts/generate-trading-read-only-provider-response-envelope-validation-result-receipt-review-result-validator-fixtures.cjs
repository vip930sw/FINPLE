...
} = require("./validate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-contract.cjs");
...
const VALIDATOR_PATH = path.join(
  "scripts",
  "validate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-contract.cjs",
);
...
const CONTRACT_VERSION =
  "trading-lab-step116-read-only-provider-response-envelope-validation-result-receipt-review-result-validator-fixtures-v0.1";
...
function invalidSyntheticFixtures(baseContract) {
  return [
    ...
    },
    {
      ...
      contract: {
        ...
        futureResponseValidationResultReceiptReviewResultBoundary: {
          ...baseContract.futureResponseValidationResultReceiptReviewResultBoundary,
      ...
      contract: {
        ...
        futureResponseValidationResultReceiptReviewResultBoundary: {
          ...
          requiredReviewResultFields:
            baseContract.futureResponseValidationResultReceiptReviewResultBoundary.requiredReviewResultFields.filter(
      ...
      contract: {
        ...
        futureResponseValidationResultReceiptReviewResultBoundary: {
          ...
          requiredReviewResultAssertions:
            baseContract.futureResponseValidationResultReceiptReviewResultBoundary.requiredReviewResultAssertions.filter(
      ...
      contract: {
        ...
        futureResponseValidationResultReceiptReviewResultBoundary: {
          ...
          forbiddenReviewResultContent:
            baseContract.futureResponseValidationResultReceiptReviewResultBoundary.forbiddenReviewResultContent.filter(
    ...
    {
      id: "provider_payload_flag_enabled",
      expectedErrorCodes: ["allow_flag_enabled"],
      contract: {
        ...baseContract,
        currentState: { ...baseContract.currentState, providerPayloadRecorded: true },
      },
    },
    {
...
function fixtureValidationEvidence(validContract, invalidFixtures) {
  const validResult = validateReadOnlyProviderResponseValidationResultReceiptReviewResultContract(validContract);
  const invalidResults = invalidFixtures.map((fixture) => {
    const result = validateReadOnlyProviderResponseValidationResultReceiptReviewResultContract(fixture.contract);
...
function buildContract() {
  ...
  const checks = {
    ...
    reviewResultContractReady:
      reviewResultContract.readiness?.readyForFutureResponseValidationResultReceiptReviewResult === true &&
      ...
      reviewResultContract.readiness?.providerPayloadRecorded === false &&
    ...
    validatorExportsLocalValidation:
      validatorSource.includes("validateReadOnlyProviderResponseValidationResultReceiptReviewResultContract") &&
      validatorSource.includes("contract_path_required") &&
      validatorSource.includes("--contract"),
    architectureDocMentionsValidatorFixtures:
      architectureDoc.includes("Trading Read-Only Provider Response Envelope Validation Result Receipt Review Result Validator Fixtures") &&
      architectureDoc.includes("read_only_provider_response_envelope_validation_result_receipt_review_result_validator_fixtures"),
  ...
  const readyForResponseReviewResultValidatorFixtureRegression =
  ...
  return stableJson({
    ...
    step: "Step 116-3L-L",
    scope: "read_only_provider_response_envelope_validation_result_receipt_review_result_validator_fixtures",
    sourceFiles: {
    ...
    validation: {
      redactionBoundary:
        "synthetic review-result validator fixtures only; no real validation receipt, response envelope path, account, credential, provider payload, response payload, order payload, token, app key, app secret, execution, fill, or order confirmation content",
    ...
    readiness: {
      status: readyForResponseReviewResultValidatorFixtureRegression
        ? "fixtures_ready_for_read_only_provider_response_validation_result_receipt_review_result_validator_regression"
        : "blocked_before_read_only_provider_response_validation_result_receipt_review_result_validator_fixture_regression",
      readyForResponseReviewResultValidatorFixtureRegression,
      fixturesOnly: true,
      currentStepReadsReceipt: false,
      currentStepRecordsReviewResult: false,
      currentStepCallsProvider: false,
      ...
      blockers: [
        ...(checks.reviewResultContractReady
          ? []
          : ["read_only_provider_response_validation_result_receipt_review_result_contract_not_ready"]),
        ...(checks.validatorExportsLocalValidation
          ? []
          : ["read_only_provider_response_validation_result_receipt_review_result_validator_not_ready"]),
        ...(checks.architectureDocMentionsValidatorFixtures
          ? []
          : ["architecture_doc_missing_read_only_provider_response_validation_result_receipt_review_result_validator_fixtures"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-validator-fixtures.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-validator-fixtures.cjs`,
      );
    }
    console.log(
      "[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-validator-fixtures] ok",
    );
    console.log(
      `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-validator-fixtures] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    "[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-validator-fixtures] wrote contract",
  );
  console.log(
    `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-validator-fixtures] readyForResponseReviewResultValidatorFixtureRegression=${parsed.readiness.readyForResponseReviewResultValidatorFixtureRegression}`,
  );
}
