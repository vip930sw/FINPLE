...
const CONTRACT_VERSION =
  "trading-lab-step116-read-only-provider-response-envelope-validation-result-receipt-review-result-v0.1";
...
function buildContract() {
  ...
  const checks = {
    ...
    reviewRunbookReady:
      reviewRunbook.readiness?.readyForOwnerAssistedResponseValidationResultReceiptReviewRunbookReview === true &&
    ...
    reviewRunbookValidatorFixturesReady:
      reviewRunbookValidatorFixtures.readiness?.readyForResponseReviewRunbookValidatorFixtureRegression === true &&
    ...
    validationResultReceiptContractReady:
      validationResultReceipt.readiness?.readyForFutureResponseEnvelopeValidationResultReceiptReview === true &&
      validationResultReceipt.readiness?.validationReceiptRecordedNow === false &&
      validationResultReceipt.readiness?.responseEnvelopePathRecorded === false &&
      validationResultReceipt.readiness?.rawResponseRecorded === false &&
      validationResultReceipt.readiness?.providerPayloadRecorded === false &&
    ...
    architectureDocMentionsResponseValidationResultReceiptReviewResult:
      architectureDoc.includes("Trading Read-Only Provider Response Envelope Validation Result Receipt Review Result") &&
      architectureDoc.includes("read_only_provider_response_envelope_validation_result_receipt_review_result"),
  ...
  const readyForFutureResponseValidationResultReceiptReviewResult =
    ...
    checks.architectureDocMentionsResponseValidationResultReceiptReviewResult &&
  ...
  return stableJson({
    ...
    step: "Step 116-3L-J",
    scope: "read_only_provider_response_envelope_validation_result_receipt_review_result",
    sourceFiles: {
    ...
    futureResponseValidationResultReceiptReviewResultBoundary: {
      ...
      forbiddenReviewResultContent,
      sampleRedactedShape: {
        responseValidationReceiptReviewId: "response_validation_receipt_review_<opaque_id>",
      ...
      promotionRules: [
        "future review result can be recorded only after owner-assisted response receipt review",
        "review result cannot record private receipt paths, raw responses, or provider payloads",
    ...
    readiness: {
      status: readyForFutureResponseValidationResultReceiptReviewResult
        ? "review_result_contract_ready_pending_owner_response_validation_result_receipt_review"
        : "blocked_before_read_only_provider_response_validation_result_receipt_review_result",
      readyForFutureResponseValidationResultReceiptReviewResult,
      validationReceiptReviewRecordedNow: false,
      validationReceiptReadAllowedNow: false,
      receiptPathRecorded: false,
      rawResponseRecorded: false,
      ...
      blockers: [
        ...
        ...(checks.architectureDocMentionsResponseValidationResultReceiptReviewResult
          ? []
          : ["architecture_doc_missing_read_only_provider_response_validation_result_receipt_review_result"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-contract.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-contract.cjs`,
      );
    }
    console.log("[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-contract] ok");
    console.log(
      `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    "[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-contract] wrote contract",
  );
  console.log(
    `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-contract] readyForFutureResponseValidationResultReceiptReviewResult=${parsed.readiness.readyForFutureResponseValidationResultReceiptReviewResult}`,
  );
}
