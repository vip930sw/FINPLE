...
const SCRIPT_PATH = path.resolve("scripts", "generate-trading-manual-order-permission-hash-helper-preflight.cjs");
const CONTRACT = "trading_lab_step116_manual_order_permission_hash_helper_preflight.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-hash-helper-preflight-"));
...
test("passes with current manual order permission hash helper preflight", () => {
  const workspace = makeWorkspace();
  const result = runPreflight(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /trading_lab_step116_manual_order_permission_hash_helper_preflight\.json/);
});

test("keeps hash preparation deferred and runtime paths blocked", () => {
  const workspace = makeWorkspace();
  const result = runPreflight(workspace, []);
  ...
  assert.equal(report.readiness.readyForOwnerAssistedManualOrderHashPreparationLater, true);
...
test("records owner-assisted future review inputs without raw values", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  const boundary = report.futureOwnerAssistedManualOrderHashPreparationBoundary;
  const serialized = JSON.stringify(report);

  assert.match(boundary.futureReviewInputs.join("|"), /explicit_owner_request_to_prepare_manual_order_hashes/);
...
test("rejects stale preflight if hash generation is manually flipped on", () => {
  ...
  assert.match(result.stderr, /trading_lab_step116_manual_order_permission_hash_helper_preflight\.json is out of date/);
});

test("blocks if prior hash helper contract is not ready", () => {
  const workspace = makeWorkspace();
  const prior = readJson(workspace, "trading_lab_step116_manual_order_permission_hash_helper_contract.json");
  prior.readiness.readyForFutureManualOrderPermissionHashHelperImplementationReview = false;
  prior.readiness.orderSubmissionAllowed = true;
  writeJson(workspace, "trading_lab_step116_manual_order_permission_hash_helper_contract.json", prior);
...
test("blocks if actual helper, private packet, adapter, route, UI, or scenario artifact appears", () => {
  const workspace = makeWorkspace();
  const files = [
