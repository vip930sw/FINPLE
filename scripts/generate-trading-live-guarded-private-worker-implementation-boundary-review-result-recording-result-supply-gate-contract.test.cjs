...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-boundary-recording-result-supply-"));
...
test("passes with current private worker implementation boundary review result recording result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /private_worker_implementation_boundary_review_result_recording_result_supply_gate_contract\.json/);
});

test("opens recording result supply gate without accepting, reading, recording, implementing, calls, runtime, routes, UI, DB, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultSupplyGate, true);
  ...
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultSuppliedNow, false);
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultReadNow, false);
  assert.equal(report.currentState.privateWorkerImplementationBoundaryReviewResultRecordedNow, false);
...
test("blocks if boundary review result recording preflight starts recording result", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_boundary_review_result_recording_preflight_contract.json",
  );
  preflight.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingPreflight = false;
  preflight.readiness.ownerPrivateWorkerImplementationBoundaryReviewResultReadNow = true;
  preflight.readiness.privateWorkerImplementationBoundaryReviewResultRecordedNow = true;
  preflight.readiness.currentStepStartsWorkerRuntime = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultSupplyGate, false);
  assert.match(
...
test("blocks if boundary review result supply gate starts accepting result", () => {
  const workspace = makeWorkspace();
  const supply = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_boundary_review_result_supply_gate_contract.json",
  );
  supply.readiness.ownerPrivateWorkerImplementationBoundaryReviewResultSuppliedNow = true;
  supply.readiness.ownerPrivateWorkerImplementationBoundaryReviewResultReadNow = true;
  supply.readiness.privateWorkerImplementationBoundaryReviewResultRecordedNow = true;
...
test("blocks if private recording result, worker, adapter, route, UI, DB, or scenario artifact appears", () => {
  const workspace = makeWorkspace();
  const files = [
    path.join(workspace, "data", "private", "trading", "live_guarded_private_worker_boundary_review_result_recording_result.redacted.json"),
