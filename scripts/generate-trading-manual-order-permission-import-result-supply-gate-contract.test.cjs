...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-import-result-supply-gate-contract.cjs",
);
const CONTRACT = "trading_lab_step116_manual_order_permission_import_result_supply_gate_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-import-result-supply-"));
...
test("passes with current manual order permission import result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /manual_order_permission_import_result_supply_gate_contract\.json/);
});

test("opens import result supply gate without reading, recording, importing, or implementing", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionImportResultSupply, true);
  ...
  assert.equal(report.currentState.currentStepRecordsImportResult, false);
...
test("blocks if import result recording preflight is no longer ready", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(workspace, "trading_lab_step116_manual_order_permission_import_result_recording_preflight_contract.json");
  preflight.readiness.readyForManualOrderPermissionImportResultRecordingPreflight = false;
  writeJson(workspace, "trading_lab_step116_manual_order_permission_import_result_recording_preflight_contract.json", preflight);

  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionImportResultSupply, false);
  assert.match(report.readiness.blockers.join("|"), /manual_order_permission_import_result_recording_preflight_not_ready/);
});

test("blocks if implementation review result supply gate starts importing too early", () => {
  const workspace = makeWorkspace();
  const supplyGate = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_import_implementation_review_result_supply_gate_contract.json",
  );
  supplyGate.readiness.permissionPacketImportedNow = true;
