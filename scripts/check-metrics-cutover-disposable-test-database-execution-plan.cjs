...
const {
  ...
  safeResult,
} = require("./lib/metrics-cutover-disposable-test-database-execution-plan.cjs");

function runCheck() {
  return evaluateDisposableTestDatabaseExecutionPlan(
    buildValidPreparationPacket(),
  );
}
...
function evaluateCliRequest(argv = process.argv.slice(2), options = {}) {
  try {
    ...
    return safeResult("blocked", {}, ["execution_plan_check_failed"]);
  }
}
