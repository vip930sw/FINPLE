  ...
  if (!isPlainObject(boundary)) {
    errors.push(
      makeError(
        ...
        "futureValidationResultReceiptReviewBoundary",
        "future receipt review boundary must be an object",
      ),
    );
    return;
  }
...
function validateManualOrderPermissionValidationResultReceiptReviewPreflight(contract) {
  const errors = [];
...
function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/validate-trading-manual-order-permission-validation-result-receipt-review-preflight.cjs --contract <path>",
    );
    return;
  }
  if (args.unknown) {
  ...
  const result = validateManualOrderPermissionValidationResultReceiptReviewPreflight(contract);
