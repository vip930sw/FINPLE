...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-packet-validation-result-receipt-review-runbook-contract.cjs",
);
const CONTRACT =
...
test("passes with current read-only approval validation result receipt review runbook contract", () => {
...
test("records owner-assisted approval receipt review runbook while keeping validation execution, receipt reads, provider calls, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runRunbook(workspace, []);
  ...
  assert.equal(report.readiness.readyForOwnerAssistedReadOnlyApprovalValidationResultReceiptReviewRunbookReview, true);
...
test("keeps runbook command explicit and redacted", () => {
  ...
  assert.match(
    report.futureOwnerAssistedReadOnlyApprovalValidationResultReceiptReviewRunbook.validatorCommandTemplate,
    /--receipt <owner-supplied-redacted-approval-validation-result-receipt-path>/,
  );
  assert.match(
    report.futureOwnerAssistedReadOnlyApprovalValidationResultReceiptReviewRunbook.reviewPreflightValidatorCommandTemplate,
    /--contract data\/processed\/trading_lab_step116_read_only_approval_packet_validation_result_receipt_review_preflight\.json/,
  );
  assert.match(
    report.futureOwnerAssistedReadOnlyApprovalValidationResultReceiptReviewRunbook.requiredReviewAssertions.join("|"),
...
test("rejects stale runbook if validation, receipt read, provider, or order flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_approval_packet_validation_result_receipt_review_runbook_contract\.json is out of date/,
  );
});
