...
const {
  validateRedactedApprovalHashHelperContract,
} = require("./validate-trading-redacted-approval-hash-helper-contract.cjs");
...
const CONTRACT_VERSION = "trading-lab-step116-redacted-approval-hash-helper-validator-fixtures-v0.1";
...
const REQUIRED_INVALID_FIXTURE_IDS = [
  ...
  "changed_future_helper_path",
  "changed_future_approval_packet_path",
  "hash_helper_implementation_flag_enabled",
  "provider_call_flag_enabled",
  "order_submission_flag_enabled",
  "unsafe_sample_output_value",
];
...
function withSecretBoundary(baseContract, patch) {
  return withBoundary(baseContract, {
...
function withSampleOutput(baseContract, patch) {
  return withBoundary(baseContract, {
    sampleOutputShape: {
...
function invalidSyntheticFixtures(baseContract) {
  return [
    ...
    },
    {
      ...
      expectedErrorCodes: ["boundary_action_enabled"],
      contract: withBoundary(baseContract, { currentStepCreatesApprovalPacket: true }),
    },
    {
    ...
    {
      id: "invalid_hash_algorithm",
      expectedErrorCodes: ["invalid_hash_algorithm"],
      contract: withBoundary(baseContract, { acceptedHashAlgorithm: "SHA-256" }),
    },
    {
      ...
      contract: withSecretBoundary(baseContract, {
    ...
    {
      id: "changed_future_helper_path",
      expectedErrorCodes: ["invalid_future_hash_helper_path"],
      contract: withBoundary(baseContract, {
    ...
    {
      id: "hash_helper_implementation_flag_enabled",
      expectedErrorCodes: ["allow_flag_enabled"],
      contract: {
        ...baseContract,
        currentState: { ...baseContract.currentState, hashHelperImplementationAllowed: true },
      },
    },
    {
    ...
    {
      id: "unsafe_sample_output_value",
      expectedErrorCodes: ["invalid_sample_hash_placeholder", "sample_forbidden_value"],
      contract: withSampleOutput(baseContract, { accountIdHash: ["raw", "account"].join("_") }),
    },
  ];
}

function fixtureValidationEvidence(validContract, invalidFixtures) {
  const validResult = validateRedactedApprovalHashHelperContract(validContract);
  const invalidResults = invalidFixtures.map((fixture) => {
    const result = validateRedactedApprovalHashHelperContract(fixture.contract);
...
function buildContract() {
  ...
  const syntheticValidContract = clone(hashHelperContract);
  const syntheticInvalidFixtures = invalidSyntheticFixtures(syntheticValidContract);
  const validationEvidence = fixtureValidationEvidence(syntheticValidContract, syntheticInvalidFixtures);
  ...
  const checks = {
    ...
    redactedApprovalHashHelperReady:
      ...
      hashHelperContract.readiness?.approvalPacketImportedNow === false &&
    ...
    validatorExportsLocalValidation:
      validatorSource.includes("validateRedactedApprovalHashHelperContract") &&
      validatorSource.includes("contract_path_required") &&
      validatorSource.includes("--contract"),
    architectureDocMentionsValidatorFixtures:
      architectureDoc.includes("Trading Redacted Approval Hash Helper Validator Fixtures") &&
      architectureDoc.includes("redacted_approval_hash_helper_validator_fixtures"),
  ...
  const readyForRedactedApprovalHashHelperValidatorRegression =
    checks.redactedApprovalHashHelperReady &&
  ...
  return stableJson({
    ...
    step: "Step 116-2X-A",
    scope: "redacted_approval_hash_helper_validator_fixtures",
    sourceFiles: {
      redactedApprovalHashHelper: HASH_HELPER_CONTRACT_PATH,
    ...
    validation: {
      redactionBoundary:
        "synthetic redacted approval hash helper validator fixtures only; no raw operator, account, evidence, revocation, pepper, credential, provider payload, order payload, private packet, execution, fill, or order confirmation content",
      validatorPath: VALIDATOR_PATH,
      validSyntheticRedactedApprovalHashHelperContract: syntheticValidContract,
      syntheticInvalidFixtures,
      evidence: validationEvidence,
    },
    checks,
    evidence: {
      redactedApprovalHashHelperStatus: hashHelperContract.readiness?.status,
      invalidFixtureIds,
    ...
    readiness: {
      status: readyForRedactedApprovalHashHelperValidatorRegression
        ? "fixtures_ready_for_redacted_approval_hash_helper_validator_regression"
        : "blocked_before_redacted_approval_hash_helper_validator_fixture_regression",
      readyForRedactedApprovalHashHelperValidatorRegression,
      fixturesOnly: true,
      hashHelperImplementationAllowed: false,
      hashCreationAllowed: false,
      approvalPacketCreatedNow: false,
      ...
      liveTradingAllowed: false,
      blockers: [
        ...(checks.redactedApprovalHashHelperReady ? [] : ["redacted_approval_hash_helper_not_ready"]),
        ...(checks.validatorExportsLocalValidation ? [] : ["local_validator_export_not_ready"]),
        ...(checks.architectureDocMentionsValidatorFixtures
          ? []
          : ["architecture_doc_missing_redacted_approval_hash_helper_validator_fixtures"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-redacted-approval-hash-helper-validator-fixtures.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-redacted-approval-hash-helper-validator-fixtures.cjs`,
      );
    }
    console.log("[generate-trading-redacted-approval-hash-helper-validator-fixtures] ok");
    console.log(`[generate-trading-redacted-approval-hash-helper-validator-fixtures] contract=${CONTRACT_PATH}`);
    return;
  }
  ...
  console.log("[generate-trading-redacted-approval-hash-helper-validator-fixtures] wrote contract");
  console.log(
    `[generate-trading-redacted-approval-hash-helper-validator-fixtures] readyForRedactedApprovalHashHelperValidatorRegression=${parsed.readiness.readyForRedactedApprovalHashHelperValidatorRegression}`,
  );
}
