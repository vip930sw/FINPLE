...
function makeContractFile(contract) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "finple-approval-receipt-review-result-validator-"));
  const filePath = path.join(dir, "approval-receipt-review-result-contract.json");
...
test("validates the current read-only approval validation result receipt review result contract", () => {
...
test("rejects missing top-level fields and review result assertions", () => {
  ...
  contract.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.requiredReviewResultAssertions =
    contract.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.requiredReviewResultAssertions.filter(
...
test("rejects receipt read, review result record, or provider-call actions enabled now", () => {
  const contract = currentContract();
  contract.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.currentStepReadsReceipt = true;
  contract.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.currentStepRecordsReviewResult = true;
  contract.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.currentStepCallsProvider = true;
...
test("rejects incomplete review result fields and forbidden output content lists", () => {
  const contract = currentContract();
  contract.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.requiredReviewResultFields = [
    "approvalValidationReceiptReviewId",
  ];
  contract.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.forbiddenReviewResultContent = ["app_key"];
...
test("rejects changed future receipt path", () => {
  const contract = currentContract();
  contract.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.futureValidationResultReceiptPath =
