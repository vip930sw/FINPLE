...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-validation-result-receipt-review-runbook-validator-fixtures.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-receipt-review-runbook-fixtures-"));
...
test("passes with current manual order permission validation result receipt review runbook validator fixtures", () => {
...
test("records synthetic validator fixtures while keeping receipt reads, imports, routes, UI, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForReviewRunbookValidatorFixtureRegression, true);
...
test("keeps fixture catalog redacted and validates expected local failures", () => {
  ...
  assert.match(invalidIds.join("|"), /permission_import_enabled/);
...
test("rejects stale fixtures if receipt read or trading flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_manual_order_permission_validation_result_receipt_review_runbook_validator_fixtures\.json is out of date/,
  );
});

test("blocks if upstream review runbook opens too early", () => {
  const workspace = makeWorkspace();
...
test("blocks if private receipt, private packet, adapter, route, UI, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
