...
function validateTradingReadOnlyApprovalPacketValidationResultReceipt(receipt) {
  const errors = [];
  ...
  for (const field of ["validatorVersionHash", "approvalPacketShapeHash"]) {
    ...
    if (!Array.isArray(receipt.errorCodeHashes)) {
      ...
      receipt.errorCodeHashes.forEach((value, index) => {
        if (!isLabelledHash(value)) {
          errors.push(
            makeError("malformed_error_code_hash", `errorCodeHashes[${index}]`, "error code entries must be labelled hashes"),
          );
        }
      });
    }
...
function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/validate-trading-read-only-approval-packet-validation-result-receipt.cjs --receipt <path>",
    );
    return;
  }
  if (args.unknown) {
  ...
  const result = validateTradingReadOnlyApprovalPacketValidationResultReceipt(receipt);
