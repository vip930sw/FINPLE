...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-review-result-contract.cjs",
);
const CONTRACT =
...
test("passes with current private worker recording-result review result gate", () => {
  ...
  assert.match(result.stdout, /private_worker_implementation_boundary_review_result_recording_result_review_result/);
});
...
test("opens review result contract without reading, recording, implementing, calls, or orders", () => {
  ...
  assert.equal(
    report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResult,
    true,
  );
  ...
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultSuppliedNow, false);
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultReadNow, false);
  assert.equal(report.currentState.privateWorkerImplementationBoundaryReviewResultReviewRecordedNow, false);
...
test("blocks if review result supply gate no longer passes", () => {
  ...
  supplyGate.readiness.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultReadNow = true;
  writeJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_boundary_review_result_recording_result_review_result_supply_gate_contract.json",
    supplyGate,
...
test("blocks if review preflight no longer passes", () => {
  ...
  preflight.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewPreflight = false;
  preflight.readiness.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewAcceptedNow = true;
  preflight.readiness.privateWorkerImplementationBoundaryReviewResultReviewRecordedNow = true;
  writeJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_boundary_review_result_recording_result_review_preflight_contract.json",
    preflight,
...
test("blocks if recording result contract starts reading or recording result", () => {
  ...
  contract.readiness.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultReadNow = true;
  contract.readiness.privateWorkerImplementationBoundaryReviewResultRecordedNow = true;
...
test("blocks if recording result supply gate starts supplying or recording result", () => {
  ...
  supplyGate.readiness.privateWorkerImplementationBoundaryReviewResultRecordedNow = true;
  writeJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_boundary_review_result_recording_result_supply_gate_contract.json",
    supplyGate,
