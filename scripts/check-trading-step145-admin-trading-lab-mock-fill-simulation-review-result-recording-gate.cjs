...
const REQUIRED_FILES = [
  ...
  "scripts/check-trading-step145-admin-trading-lab-mock-fill-simulation-review-result-recording-gate.test.cjs",
];

const REQUIRED_SNIPPETS = [
  ["server/src/services/tradingAdminLabDashboardShell.js", "STEP145_ADMIN_TRADING_LAB_MOCK_FILL_SIMULATION_REVIEW_RESULT_FLAGS"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_FILL_SIMULATION_REVIEW_RESULT_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_FILL_SIMULATION_REVIEW_RECEIPT_SCHEMA"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_FILL_SIMULATION_REVIEW_DECISION_SUMMARY_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_FILL_SIMULATION_REVIEW_IMPACT_SUMMARY_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabMockFillSimulationReviewImpactSummary"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "validateTradingLabMockFillSimulationReviewResult"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabMockFillSimulationReviewResult"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabMockFillSimulationReviewReceipt"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabMockFillSimulationReviewDecisionSummary"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabMockFillSimulationReviewResultRecordingGate"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildAdminTradingLabMockFillSimulationReviewResultStatus"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "admin_only_trading_lab_mock_fill_simulation_review_result_recording_gate_fail_closed"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "mock_fill_simulation_core_preflight"],
  ...
  ["server/src/routes/adminTradingReadinessRoutes.js", "buildAdminTradingLabMockFillSimulationReviewResultStatus"],
  ["server/src/routes/adminTradingReadinessRoutes.js", 'router.get("/trading-lab-mock-fill-simulation-review-result"'],
  ["server/src/routes/adminTradingReadinessRoutes.js", "requireAdminAccess"],
  ["server/src/routes/adminTradingReadinessRoutes.test.js", "trading-lab-mock-fill-simulation-review-result"],
  ["src/components/portfolio/services/serverPortfolioService.js", "fetchAdminTradingLabMockFillSimulationReviewResultStatus"],
  ["src/components/portfolio/services/serverPortfolioService.js", "\"/admin/trading-readiness/trading-lab-mock-fill-simulation-review-result\""],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabMockFillSimulationReview"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabMockFillSimulationReviewCards"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabMockFillSimulationReviewList"],
  ["src/components/TradingReadinessPanel.jsx", "Mock fill simulation review result"],
  ["src/App.css", ".tradingLabMockFillSimulationReviewCards"],
  ["src/App.css", ".tradingLabMockFillSimulationReviewList"],
  ["package.json", "check:trading-step145-admin-trading-lab-mock-fill-simulation-review-result-recording-gate"],
];
...
function main() {
  ...
  if (!/router\.get\("\/trading-lab-mock-fill-simulation-review-result"[\s\S]*requireAdminAccess[\s\S]*buildAdminTradingLabMockFillSimulationReviewResultStatus/.test(routeText)) {
    fail("mock fill simulation review result endpoint must stay admin-only and read-only");
  }
  ...
  if (accountExposure.length > 0) fail(`mock fill simulation review must not be exposed on /mypage: ${accountExposure.join(", ")}`);
  const appExposure = publicExposureTerms.filter((term) => appText.includes(term));
  if (appExposure.length > 0) fail(`mock fill simulation review must not be exposed on homepage or public router: ${appExposure.join(", ")}`);
  ...
  const touchedScenarioFiles = scenarioFiles.filter((filePath) => fs.existsSync(filePath) && readText(filePath).includes("Step 145"));
  if (touchedScenarioFiles.length > 0) fail(`scenario runtime files must remain untouched: ${touchedScenarioFiles.join(", ")}`);

  console.log("[check-trading-step145-admin-trading-lab-mock-fill-simulation-review-result-recording-gate] ok");
}
