...
const CONTRACT_VERSION =
  "trading-lab-step116-live-guarded-private-worker-implementation-review-result-recording-result-supply-gate-v0.1";
const AUDITED_AT = "2026-07-02T00:00:00Z";
const REQUIRED_WORKER_REVIEW_RESULT_RECORDING_RESULT_SUPPLY_GATES = [
...
const FORBIDDEN_WORKER_REVIEW_RESULT_RECORDING_RESULT_SUPPLY_CONTENT = [
...
function buildContract() {
  ...
  const workerReviewResultRecordingResultSupplyGates = [
    ...REQUIRED_WORKER_REVIEW_RESULT_RECORDING_RESULT_SUPPLY_GATES,
  ];
  const forbiddenWorkerReviewResultRecordingResultSupplyContent = [
    ...FORBIDDEN_WORKER_REVIEW_RESULT_RECORDING_RESULT_SUPPLY_CONTENT,
  ];
  const missingWorkerReviewResultRecordingResultSupplyGates = missingValues(
    workerReviewResultRecordingResultSupplyGates,
    REQUIRED_WORKER_REVIEW_RESULT_RECORDING_RESULT_SUPPLY_GATES,
  );
  const missingForbiddenWorkerReviewResultRecordingResultSupplyContent = missingValues(
    forbiddenWorkerReviewResultRecordingResultSupplyContent,
    FORBIDDEN_WORKER_REVIEW_RESULT_RECORDING_RESULT_SUPPLY_CONTENT,
  );
  ...
  const checks = {
    contractOnly: true,
    privateWorkerImplementationReviewResultRecordingResultSupplyGateOnly: true,
    ...
    workerReviewResultRecordingResultSupplyGatesReady:
      missingWorkerReviewResultRecordingResultSupplyGates.length === 0,
    forbiddenWorkerReviewResultRecordingResultSupplyContentReady:
      missingForbiddenWorkerReviewResultRecordingResultSupplyContent.length === 0,
    architectureDocMentionsPrivateWorkerImplementationReviewResultRecordingResultSupplyGate:
      architectureDoc.includes(
        "Trading Live-Guarded Private Worker Implementation Review Result Recording Result Supply Gate",
      ) &&
      architectureDoc.includes("live_guarded_private_worker_implementation_review_result_recording_result_supply_gate"),
  ...
  const readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultSupplyGate =
    ...
    checks.workerReviewResultRecordingResultSupplyGatesReady &&
    checks.forbiddenWorkerReviewResultRecordingResultSupplyContentReady &&
    checks.architectureDocMentionsPrivateWorkerImplementationReviewResultRecordingResultSupplyGate &&
  ...
  return stableJson({
    ...
    step: "Step 116-7N",
    scope: "live_guarded_private_worker_implementation_review_result_recording_result_supply_gate",
    sourceFiles: {
    ...
    privateWorkerImplementationReviewResultRecordingResultSupplyGateBoundary: {
      ...
      nextAllowedAction:
        "after the owner supplies a redacted private-worker implementation review recording result outside repo commits, add a separate hash-only recording result contract boundary",
      workerReviewResultRecordingResultSupplyGates,
      forbiddenWorkerReviewResultRecordingResultSupplyContent,
    ...
    checks,
    evidence: {
      missingWorkerReviewResultRecordingResultSupplyGates,
      missingForbiddenWorkerReviewResultRecordingResultSupplyContent,
    ...
    readiness: {
      status: readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultSupplyGate
        ? "live_guarded_private_worker_implementation_review_result_recording_result_supply_gate_ready_pending_owner_recording_result"
        : "blocked_before_live_guarded_private_worker_implementation_review_result_recording_result_supply_gate",
      readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultSupplyGate,
      ...
      blockers: [
        ...
        ...missingWorkerReviewResultRecordingResultSupplyGates.map(
          (gate) => `missing_private_worker_implementation_review_result_recording_result_supply_gate_${gate}`,
        ),
        ...missingForbiddenWorkerReviewResultRecordingResultSupplyContent.map(
          (content) =>
            `missing_forbidden_private_worker_implementation_review_result_recording_result_supply_content_${content}`,
        ),
        ...(checks.architectureDocMentionsPrivateWorkerImplementationReviewResultRecordingResultSupplyGate
          ? []
          : ["architecture_doc_missing_private_worker_implementation_review_result_recording_result_supply_gate"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log(
      "[generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-supply-gate-contract] ok",
    );
    console.log(
      `[generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-supply-gate-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    `[generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-supply-gate-contract] wrote ${CONTRACT_PATH}`,
  );
}
