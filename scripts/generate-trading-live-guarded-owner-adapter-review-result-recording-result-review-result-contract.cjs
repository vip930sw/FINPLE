...
const CONTRACT_VERSION =
  "trading-lab-step116-live-guarded-owner-adapter-review-result-recording-result-review-result-v0.1";
const AUDITED_AT = "2026-07-02T00:00:00Z";
const REQUIRED_REVIEW_RESULT_GATES = [
  "owner_adapter_review_result_recording_result_review_result_supply_gate_ready",
...
function buildContract() {
  ...
  const reviewResultGates = [...REQUIRED_REVIEW_RESULT_GATES];
  const forbiddenReviewResultContent = [...FORBIDDEN_REVIEW_RESULT_CONTENT];
  const missingReviewResultGates = missingValues(reviewResultGates, REQUIRED_REVIEW_RESULT_GATES);
  ...
  const checks = {
    contractOnly: true,
    recordingResultReviewResultContractOnly: true,
    reviewResultSupplyGateReady:
      reviewResultSupplyGate.readiness?.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewResultSupplyGate ===
        true &&
      reviewResultSupplyGate.readiness?.readyForPrivateWorkerImplementationAfterOwnerAdapterReviewResult === false &&
      reviewResultSupplyGate.readiness?.kisPersonalPermissionExternalBlocker === false &&
      reviewResultSupplyGate.readiness?.ownerAdapterReviewResultRecordingResultReviewResultSuppliedNow === false &&
      reviewResultSupplyGate.readiness?.ownerAdapterReviewResultRecordingResultReviewResultReadNow === false &&
      reviewResultSupplyGate.readiness?.ownerAdapterReviewResultReviewRecordedNow === false &&
      ...
      reviewResultSupplyGate.readiness?.currentStepCallsProvider === false &&
      reviewResultSupplyGate.readiness?.currentStepSubmitsOrders === false &&
    ...
    reviewResultGatesReady: missingReviewResultGates.length === 0,
    forbiddenReviewResultContentReady: missingForbiddenReviewResultContent.length === 0,
    architectureDocMentionsRecordingResultReviewResultContract:
      architectureDoc.includes("Trading Live-Guarded Owner Adapter Review Result Recording Result Review Result Contract") &&
      architectureDoc.includes("live_guarded_owner_adapter_review_result_recording_result_review_result"),
  ...
  const readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewResultContract =
    ...
    checks.reviewResultGatesReady &&
    checks.forbiddenReviewResultContentReady &&
    checks.architectureDocMentionsRecordingResultReviewResultContract &&
  ...
  return stableJson({
    ...
    step: "Step 116-7J",
    scope: "live_guarded_owner_adapter_review_result_recording_result_review_result",
    sourceFiles: {
      liveGuardedOwnerAdapterReviewResultRecordingResultReviewResultSupplyGate: REVIEW_RESULT_SUPPLY_GATE_PATH,
    ...
    reviewResultContractBoundary: {
      ...
      nextAllowedAction:
        "after the owner-supplied redacted recording-result review result is separately reviewed, open a dedicated private-worker implementation review boundary",
      reviewResultGates,
      forbiddenReviewResultContent,
    ...
    readiness: {
      status: readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewResultContract
        ? "live_guarded_owner_adapter_review_result_recording_result_review_result_contract_ready_pending_owner_review_result"
        : "blocked_before_live_guarded_owner_adapter_review_result_recording_result_review_result_contract",
      readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewResultContract,
      ...
      blockers: [
        ...(checks.reviewResultSupplyGateReady
          ? []
          : ["live_guarded_owner_adapter_review_result_recording_result_review_result_supply_gate_not_ready"]),
        ...(checks.reviewPreflightStillReady
          ? []
        ...
        ...missingReviewResultGates.map(
          (gate) => `missing_owner_adapter_review_result_recording_result_review_result_gate_${gate}`,
        ),
        ...missingForbiddenReviewResultContent.map(
          (content) => `missing_forbidden_owner_adapter_review_result_recording_result_review_result_content_${content}`,
        ),
        ...(checks.architectureDocMentionsRecordingResultReviewResultContract
          ? []
          : ["architecture_doc_missing_owner_adapter_review_result_recording_result_review_result"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log("[generate-trading-live-guarded-owner-adapter-review-result-recording-result-review-result-contract] ok");
    console.log(
      `[generate-trading-live-guarded-owner-adapter-review-result-recording-result-review-result-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    `[generate-trading-live-guarded-owner-adapter-review-result-recording-result-review-result-contract] wrote ${CONTRACT_PATH}`,
  );
}
