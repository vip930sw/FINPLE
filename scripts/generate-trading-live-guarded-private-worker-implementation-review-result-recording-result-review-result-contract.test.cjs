...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-review-result-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-review-result-contract-"));
...
test("passes with current private worker recording-result review result gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /private_worker_implementation_review_result_recording_result_review_result/);
});

test("opens review result contract without reading, recording, implementing, calls, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(
    report.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultReviewResult,
    true,
  );
  ...
  assert.equal(report.currentState.ownerPrivateWorkerImplementationReviewResultRecordingResultReviewResultSuppliedNow, false);
  assert.equal(report.currentState.ownerPrivateWorkerImplementationReviewResultRecordingResultReviewResultReadNow, false);
...
test("blocks if review result supply gate no longer passes", () => {
  ...
  supplyGate.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGate =
    false;
  supplyGate.readiness.ownerPrivateWorkerImplementationReviewResultRecordingResultReviewResultSuppliedNow = true;
  supplyGate.readiness.ownerPrivateWorkerImplementationReviewResultRecordingResultReviewResultReadNow = true;
  writeJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_review_result_recording_result_review_result_supply_gate_contract.json",
    supplyGate,
