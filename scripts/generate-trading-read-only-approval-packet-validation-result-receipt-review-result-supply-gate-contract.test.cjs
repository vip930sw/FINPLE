...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-packet-validation-result-receipt-review-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-approval-review-result-supply-"));
...
test("passes with current read-only approval validation receipt review result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /read_only_approval_packet_validation_result_receipt_review_result_supply_gate_contract\.json/);
});

test("opens supply boundary without accepting, reading, or recording owner review material", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalValidationReceiptReviewResultSupply, true);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewAfterReviewResult, false);
...
test("blocks if the review result contract starts recording or reading too early", () => {
  const workspace = makeWorkspace();
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalValidationReceiptReviewResultSupply, false);
  assert.match(report.readiness.blockers.join("|"), /approval_validation_receipt_review_result_not_ready/);
});
