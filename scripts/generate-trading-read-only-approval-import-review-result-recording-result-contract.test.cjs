...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-recording-result-contract.cjs",
);
const CONTRACT = "trading_lab_step116_read_only_approval_import_review_result_recording_result_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-import-review-recording-result-"));
...
test("passes with current read-only approval import review result recording result contract", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /read_only_approval_import_review_result_recording_result_contract\.json/);
});

test("records only the recording result contract boundary while import, provider, route, UI, DB, and order work stay blocked", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResult, true);
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalImportImplementationReview, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultRecordedNow, false);
...
test("blocks if recording result supply gate starts reading or recording too early", () => {
  const workspace = makeWorkspace();
  ...
  supplyGate.readiness.currentStepReadsRecordingResult = true;
  supplyGate.readiness.currentStepRecordsRecordingResult = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResult, false);
  assert.match(
