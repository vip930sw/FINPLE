...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-recording-result-review-result-recording-preflight-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-import-review-result-recording-preflight-"));
...
test("passes with current read-only approval import review-result recording preflight", () => {
  ...
  assert.match(
    result.stdout,
    /read_only_approval_import_review_result_recording_result_review_result_recording_preflight_contract\.json/,
  );
});

test("records only the hash-only recording preflight while import, provider, route, UI, DB, and order work stay blocked", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(
    report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingPreflight,
    true,
  );
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalImportImplementationReview, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultAcceptedNow, false);
...
test("blocks if the review-result contract starts reading or recording too early", () => {
  const workspace = makeWorkspace();
