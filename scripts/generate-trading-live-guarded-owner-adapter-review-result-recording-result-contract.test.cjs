...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-owner-adapter-review-result-recording-result-contract.cjs",
);
const CONTRACT = "trading_lab_step116_live_guarded_owner_adapter_review_result_recording_result_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-owner-adapter-recording-result-"));
...
test("passes with current owner adapter review result recording result contract", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /live_guarded_owner_adapter_review_result_recording_result_contract\.json/);
});
...
test("opens recording result contract without reading, recording, implementing, calls, or orders", () => {
  ...
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultContract, true);
  ...
  assert.equal(report.currentState.ownerAdapterReviewResultRecordingResultSuppliedNow, false);
...
test("blocks if recording result supply gate is no longer ready", () => {
  const workspace = makeWorkspace();
  const supply = readJson(
    workspace,
    "trading_lab_step116_live_guarded_owner_adapter_review_result_recording_result_supply_gate_contract.json",
  );
  supply.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultSupplyGate = false;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultContract, false);
  assert.match(
...
test("blocks if owner adapter review result supply gate starts reading or recording result", () => {
  const workspace = makeWorkspace();
  const supply = readJson(
    workspace,
    "trading_lab_step116_live_guarded_owner_adapter_review_result_supply_gate_contract.json",
  ...
  supply.readiness.currentStepReadsAdapterReviewResult = true;
  supply.readiness.currentStepRecordsAdapterReviewResult = true;
