...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-review-result-recording-preflight-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-review-result-recording-"));
...
test("passes with current private worker implementation review result recording preflight", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /private_worker_implementation_review_result_recording_preflight_contract\.json/);
});
...
test("opens recording preflight without accepting, reading, recording, implementing, calls, or orders", () => {
  ...
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingPreflight, true);
  ...
  assert.equal(report.currentState.ownerPrivateWorkerImplementationReviewResultAcceptedNow, false);
...
test("blocks if worker review result supply gate starts accepting or recording result", () => {
  const workspace = makeWorkspace();
  const supply = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_review_result_supply_gate_contract.json",
  );
  supply.readiness.readyForPrivateWorkerImplementationAfterReviewResult = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingPreflight, false);
  assert.match(
