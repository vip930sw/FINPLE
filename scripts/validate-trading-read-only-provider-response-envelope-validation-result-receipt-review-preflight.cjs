  ...
  if (!isPlainObject(boundary)) {
    errors.push(
      makeError(
        ...
        "futureValidationResultReceiptReviewBoundary",
        "future response validation result receipt review boundary must be an object",
      ),
    );
    return;
  }
...
function validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewPreflight(contract) {
  const errors = [];
...
function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/validate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight.cjs --contract <path>",
    );
    return;
  }
  if (args.unknown) {
  ...
  const result = validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewPreflight(contract);
