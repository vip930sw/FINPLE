...
for (const file of [...REQUIRED_FILES, ...UNTOUCHED_STEP_191_TO_193_SERVICE_FILES]) {
...
for (const file of UNTOUCHED_STEP_191_TO_193_SERVICE_FILES) {
  const source = read(file);
  assert(!source.includes("tradingAiMlContractPrimitives"), `Step191-193 service must not be migrated in Step201: ${file}`);
  assert(!source.includes("Step201"), `Step191-193 service must not know Step201: ${file}`);
}
