...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-packet-validation-result-receipt-validator-fixtures.cjs",
);
const CONTRACT = "trading_lab_step116_read_only_approval_packet_validation_result_receipt_validator_fixtures.json";
const RECEIPT_CONTRACT = "trading_lab_step116_read_only_approval_packet_validation_result_receipt.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-read-only-approval-validation-result-fixtures-"));
...
test("passes with current read-only approval packet validation result receipt validator fixtures", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /trading_lab_step116_read_only_approval_packet_validation_result_receipt_validator_fixtures\.json/);
});
...
test("records synthetic receipt validator fixtures while keeping receipt recording, imports, provider calls, UI, DB, and orders blocked", () => {
  ...
  assert.equal(report.readiness.readyForReadOnlyApprovalPacketValidationResultReceiptValidatorRegression, true);
...
test("keeps fixture catalog redacted and validates expected local failures", () => {
  ...
  assert.match(invalidIds.join("|"), /approval_packet_imported_flag_enabled/);
  ...
  assert.doesNotMatch(serialized, /rawAccountIdentifier|rawOperatorName|rawEvidenceText|rawRevocationPlan/);
});
...
test("rejects stale fixtures if receipt, provider, or order flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_approval_packet_validation_result_receipt_validator_fixtures\.json is out of date/,
  );
});
