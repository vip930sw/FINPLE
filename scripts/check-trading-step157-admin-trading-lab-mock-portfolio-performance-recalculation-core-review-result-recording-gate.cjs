...
const REQUIRED_FILES = [
  ...
  "scripts/check-trading-step157-admin-trading-lab-mock-portfolio-performance-recalculation-core-review-result-recording-gate.test.cjs",
  "scripts/check-trading-step156-admin-trading-lab-mock-portfolio-performance-recalculation-core-preflight.cjs",
];

const REQUIRED_SNIPPETS = [
  ["server/src/services/tradingAdminLabDashboardShell.js", "STEP157_ADMIN_TRADING_LAB_MOCK_PORTFOLIO_PERFORMANCE_RECALCULATION_CORE_REVIEW_RESULT_FLAGS"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_PORTFOLIO_PERFORMANCE_RECALCULATION_CORE_REVIEW_RESULT_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_PERFORMANCE_CORE_REVIEW_RECEIPT_SCHEMA"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_PERFORMANCE_CORE_REVIEW_DECISION_SUMMARY_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_MOCK_PERFORMANCE_CORE_POLICY_REVIEW_SUMMARY_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "validateTradingLabMockPortfolioPerformanceRecalculationCoreReviewResult"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabMockPortfolioPerformanceRecalculationCoreReviewResult"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabMockPortfolioPerformanceRecalculationCoreReviewResultRecordingGate"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildAdminTradingLabMockPortfolioPerformanceRecalculationCoreReviewResultStatus"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "admin_only_trading_lab_mock_portfolio_performance_recalculation_core_review_result_fail_closed"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "mock_performance_core_review_recorded"],
  ...
  ["server/src/routes/adminTradingReadinessRoutes.js", "buildAdminTradingLabMockPortfolioPerformanceRecalculationCoreReviewResultStatus"],
  ["server/src/routes/adminTradingReadinessRoutes.js", 'router.get("/trading-lab-mock-portfolio-performance-recalculation-core-review-result"'],
  ["server/src/routes/adminTradingReadinessRoutes.js", "requireAdminAccess"],
  ["server/src/routes/adminTradingReadinessRoutes.test.js", "trading-lab-mock-portfolio-performance-recalculation-core-review-result"],
  ["src/components/portfolio/services/serverPortfolioService.js", "fetchAdminTradingLabMockPortfolioPerformanceRecalculationCoreReviewResultStatus"],
  ["src/components/portfolio/services/serverPortfolioService.js", "\"/admin/trading-readiness/trading-lab-mock-portfolio-performance-recalculation-core-review-result\""],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabMockPortfolioPerformanceRecalculationCoreReviewResultStatus"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabMockPerformanceCoreReviewCards"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabMockPerformanceCoreReviewList"],
  ["src/components/TradingReadinessPanel.jsx", "Mock portfolio performance recalculation core review result"],
  ["src/components/TradingReadinessPanel.jsx", "does not update actual cash, positions, portfolio ledger, or performance records"],
  ["src/App.css", ".tradingLabMockPerformanceCoreReviewCards"],
  ["src/App.css", ".tradingLabMockPerformanceCoreReviewList"],
  ["package.json", "check:trading-step157-admin-trading-lab-mock-portfolio-performance-recalculation-core-review-result-recording-gate"],
];
...
function main() {
  ...
  if (!/router\.get\("\/trading-lab-mock-portfolio-performance-recalculation-core-review-result"[\s\S]*requireAdminAccess[\s\S]*buildAdminTradingLabMockPortfolioPerformanceRecalculationCoreReviewResultStatus/.test(routeText)) {
    fail("mock portfolio performance recalculation core review result endpoint must stay admin-only and read-only");
  }
  ...
  const publicExposureTerms = [
    "tradingLabMockPortfolioPerformanceRecalculationCoreReviewResultStatus",
    "trading-lab-mock-portfolio-performance-recalculation-core-review-result",
  ...
  if (accountExposure.length > 0) fail(`mock performance core review result must not be exposed on /mypage: ${accountExposure.join(", ")}`);
  const appExposure = publicExposureTerms.filter((term) => appText.includes(term));
  if (appExposure.length > 0) fail(`mock performance core review result must not be exposed on homepage or public router: ${appExposure.join(", ")}`);
  ...
  const touchedScenarioFiles = scenarioFiles.filter((filePath) => fs.existsSync(filePath) && readText(filePath).includes("Step 157"));
  if (touchedScenarioFiles.length > 0) fail(`scenario runtime files must remain untouched: ${touchedScenarioFiles.join(", ")}`);

  console.log("[check-trading-step157-admin-trading-lab-mock-portfolio-performance-recalculation-core-review-result-recording-gate] ok");
}
