...
const CONTRACT_VERSION =
  "trading-lab-step116-live-guarded-private-worker-implementation-boundary-review-result-recording-result-review-result-supply-gate-v0.1";
const AUDITED_AT = "2026-07-02T00:00:00Z";
const REQUIRED_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_GATES = [
...
const FORBIDDEN_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_CONTENT = [
...
function buildContract() {
  ...
  const boundaryReviewResultRecordingResultReviewResultSupplyGates = [
    ...REQUIRED_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_GATES,
  ];
  const forbiddenBoundaryReviewResultRecordingResultReviewResultSupplyContent = [
    ...FORBIDDEN_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_CONTENT,
  ];
  const missingBoundaryReviewResultRecordingResultReviewResultSupplyGates = missingValues(
    boundaryReviewResultRecordingResultReviewResultSupplyGates,
    REQUIRED_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_GATES,
  );
  const missingForbiddenBoundaryReviewResultRecordingResultReviewResultSupplyContent = missingValues(
    forbiddenBoundaryReviewResultRecordingResultReviewResultSupplyContent,
    FORBIDDEN_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_CONTENT,
  );
  ...
  const checks = {
    contractOnly: true,
    privateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultSupplyGateOnly: true,
    ...
    boundaryReviewResultRecordingResultReviewResultSupplyGatesReady:
      missingBoundaryReviewResultRecordingResultReviewResultSupplyGates.length === 0,
    forbiddenBoundaryReviewResultRecordingResultReviewResultSupplyContentReady:
      missingForbiddenBoundaryReviewResultRecordingResultReviewResultSupplyContent.length === 0,
    architectureDocMentionsPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultSupplyGate:
      architectureDoc.includes(
        "Trading Live-Guarded Private Worker Implementation Boundary Review Result Recording Result Review Result Supply Gate",
      ) &&
      architectureDoc.includes(
        "live_guarded_private_worker_implementation_boundary_review_result_recording_result_review_result_supply_gate",
      ),
  ...
  const readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultSupplyGate =
    ...
    checks.boundaryReviewResultRecordingResultReviewResultSupplyGatesReady &&
    checks.forbiddenBoundaryReviewResultRecordingResultReviewResultSupplyContentReady &&
    checks.architectureDocMentionsPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultSupplyGate &&
  ...
  return stableJson({
    ...
    step: "Step 116-7Z",
    scope: "live_guarded_private_worker_implementation_boundary_review_result_recording_result_review_result_supply_gate",
    sourceFiles: {
    ...
    privateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultSupplyGate: {
      ...
      nextAllowedAction:
        "after the owner supplies a redacted boundary review recording-result review result outside repo commits, add a separate hash-only review result contract boundary",
      boundaryReviewResultRecordingResultReviewResultSupplyGates,
      forbiddenBoundaryReviewResultRecordingResultReviewResultSupplyContent,
    ...
    checks,
    evidence: {
      missingBoundaryReviewResultRecordingResultReviewResultSupplyGates,
      missingForbiddenBoundaryReviewResultRecordingResultReviewResultSupplyContent,
    ...
    readiness: {
      status: readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultSupplyGate
        ? "live_guarded_private_worker_implementation_boundary_review_result_recording_result_review_result_supply_gate_ready_pending_owner_review_result"
        : "blocked_before_live_guarded_private_worker_implementation_boundary_review_result_recording_result_review_result_supply_gate",
      readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultSupplyGate,
      ...
      blockers: [
        ...
        ...missingBoundaryReviewResultRecordingResultReviewResultSupplyGates.map(
          (gate) =>
            `missing_private_worker_implementation_boundary_review_result_recording_result_review_result_supply_gate_${gate}`,
        ),
        ...missingForbiddenBoundaryReviewResultRecordingResultReviewResultSupplyContent.map(
          (content) =>
            `missing_forbidden_private_worker_implementation_boundary_review_result_recording_result_review_result_supply_content_${content}`,
        ),
        ...(checks.architectureDocMentionsPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultSupplyGate
          ? []
          : [
              "architecture_doc_missing_private_worker_implementation_boundary_review_result_recording_result_review_result_supply_gate",
            ]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log(
      "[generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-review-result-supply-gate-contract] ok",
    );
    console.log(
      `[generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-review-result-supply-gate-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    `[generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-review-result-supply-gate-contract] wrote ${CONTRACT_PATH}`,
  );
}
