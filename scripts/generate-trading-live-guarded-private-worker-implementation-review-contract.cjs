...
const CONTRACT_VERSION = "trading-lab-step116-live-guarded-private-worker-implementation-review-v0.1";
const AUDITED_AT = "2026-07-02T00:00:00Z";
const REQUIRED_PRIVATE_WORKER_REVIEW_GATES = [
  ...
  "private_worker_implementation_review_required_later",
  ...
  "hash_only_worker_review_required",
  "kill_switch_before_worker_start",
  "risk_gate_before_request_signing",
  "idempotency_key_required",
];
const FORBIDDEN_PRIVATE_WORKER_REVIEW_CONTENT = [
...
function buildContract() {
  ...
  const privateWorkerReviewGates = [...REQUIRED_PRIVATE_WORKER_REVIEW_GATES];
  const forbiddenPrivateWorkerReviewContent = [...FORBIDDEN_PRIVATE_WORKER_REVIEW_CONTENT];
  const missingPrivateWorkerReviewGates = missingValues(privateWorkerReviewGates, REQUIRED_PRIVATE_WORKER_REVIEW_GATES);
  const missingForbiddenPrivateWorkerReviewContent = missingValues(
    forbiddenPrivateWorkerReviewContent,
    FORBIDDEN_PRIVATE_WORKER_REVIEW_CONTENT,
  );
  const forbiddenArtifacts = forbiddenRuntimeArtifacts();

  const checks = {
    contractOnly: true,
    privateWorkerImplementationReviewOnly: true,
    privateWorkerPreflightReady:
    ...
    ownerAdapterReviewResultBoundaryReady:
      ...
      ownerAdapterReviewResult.readiness?.ownerAdapterReviewResultRecordingResultReviewResultReadNow === false &&
      ownerAdapterReviewResult.readiness?.ownerAdapterReviewResultReviewRecordedNow === false &&
    ...
    privateWorkerReviewGatesReady: missingPrivateWorkerReviewGates.length === 0,
    forbiddenPrivateWorkerReviewContentReady: missingForbiddenPrivateWorkerReviewContent.length === 0,
    architectureDocMentionsPrivateWorkerImplementationReview:
      architectureDoc.includes("Trading Live-Guarded Private Worker Implementation Review") &&
      architectureDoc.includes("live_guarded_private_worker_implementation_review"),
    noRuntimeArtifacts: forbiddenArtifacts.length === 0,
    kisPersonalPermissionExternalBlocker: false,
    ownerPrivateWorkerImplementationReviewSuppliedNow: false,
    ownerPrivateWorkerImplementationReviewReadNow: false,
    privateWorkerImplementationReviewRecordedNow: false,
  ...
  const readyForLiveGuardedPrivateWorkerImplementationReview =
    checks.privateWorkerPreflightReady &&
    ...
    checks.privateWorkerReviewGatesReady &&
    checks.forbiddenPrivateWorkerReviewContentReady &&
    checks.architectureDocMentionsPrivateWorkerImplementationReview &&
  ...
  return stableJson({
    ...
    step: "Step 116-7K",
    scope: "live_guarded_private_worker_implementation_review",
    sourceFiles: {
    ...
    privateWorkerImplementationReviewBoundary: {
      ...
      nextAllowedAction:
        "after an owner-supplied redacted private-worker implementation review result is supplied and recorded, prepare a separate private-worker implementation pre-commit boundary",
      privateWorkerReviewGates,
      forbiddenPrivateWorkerReviewContent,
      futureReviewRules: [
        "owner_supplied_redacted_worker_review_only",
        ...
        "worker_review_must_not_open_provider_calls",
        "worker_review_must_not_open_order_submission",
      ],
    },
    checks,
    evidence: {
      missingPrivateWorkerReviewGates,
      missingForbiddenPrivateWorkerReviewContent,
    ...
    readiness: {
      status: readyForLiveGuardedPrivateWorkerImplementationReview
        ? "live_guarded_private_worker_implementation_review_ready_pending_owner_worker_review_result"
        : "blocked_before_live_guarded_private_worker_implementation_review",
      readyForLiveGuardedPrivateWorkerImplementationReview,
      readyForPrivateWorkerImplementationAfterReview: false,
      kisPersonalPermissionExternalBlocker: false,
      ownerPrivateWorkerImplementationReviewSuppliedNow: false,
      ownerPrivateWorkerImplementationReviewReadNow: false,
      privateWorkerImplementationReviewRecordedNow: false,
      ...
      blockers: [
        ...(checks.privateWorkerPreflightReady
          ? []
          : ["live_guarded_private_worker_implementation_preflight_not_ready"]),
        ...(checks.ownerAdapterReviewResultBoundaryReady
          ? []
        ...
        ...missingPrivateWorkerReviewGates.map((gate) => `missing_private_worker_implementation_review_gate_${gate}`),
        ...missingForbiddenPrivateWorkerReviewContent.map(
          (content) => `missing_forbidden_private_worker_implementation_review_content_${content}`,
        ),
        ...(checks.architectureDocMentionsPrivateWorkerImplementationReview
          ? []
          : ["architecture_doc_missing_private_worker_implementation_review"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log("[generate-trading-live-guarded-private-worker-implementation-review-contract] ok");
    console.log(`[generate-trading-live-guarded-private-worker-implementation-review-contract] contract=${CONTRACT_PATH}`);
    return;
  }

  fs.mkdirSync(path.dirname(CONTRACT_PATH), { recursive: true });
  fs.writeFileSync(CONTRACT_PATH, expected);
  console.log(`[generate-trading-live-guarded-private-worker-implementation-review-contract] wrote ${CONTRACT_PATH}`);
}
