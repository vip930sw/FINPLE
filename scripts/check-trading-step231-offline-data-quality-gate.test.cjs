...
test("Step231 checker passes and leaves the working tree unchanged", () => {
  const before = execFileSync("git", ["status", "--short"], { encoding: "utf8" });
  const output = execFileSync(process.execPath, ["scripts/check-trading-step231-offline-data-quality-gate.cjs"], {
    encoding: "utf8",
  });
  const after = execFileSync("git", ["status", "--short"], { encoding: "utf8" });

  assert.equal(after, before);
  assert.match(output, /\[check-trading-step231-offline-data-quality-gate\] ok/);
  assert.match(output, /"gateMode": "offline_data_quality"/);
  assert.match(output, /"decision": "block_offline_promotion"/);
  assert.match(output, /"actualLiveTradingReady": false/);
  assert.match(output, new RegExp(`"step228BaselineCommit": "${BASELINE_COMMIT}"`));
});

test("Step231 package script runs through the dedicated checker and tests only", () => {
  const packageJson = fs.readFileSync("package.json", "utf8");

  assert.match(packageJson, /check:trading-step231-offline-data-quality-gate/);
  assert.match(packageJson, /scripts\/check-trading-step231-offline-data-quality-gate\.cjs/);
  assert.match(packageJson, /server\/src\/services\/tradingAiMlDatasetQualityGate\.test\.js/);
  assert.match(packageJson, /scripts\/check-trading-step231-offline-data-quality-gate\.test\.cjs/);
  assert.match(packageJson, /check:trading-step230-offline-dataset-quality-batch-summary/);
});

test("Step231 keeps the Step228 snapshot counts and baseline unchanged", () => {
...
test("Step231 dirty file boundary excludes Step229 Step230 registries runtime UI DB and scenario assets", () => {
