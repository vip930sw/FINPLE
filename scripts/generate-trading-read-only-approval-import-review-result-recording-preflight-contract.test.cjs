...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-recording-preflight-contract.cjs",
);
const CONTRACT = "trading_lab_step116_read_only_approval_import_review_result_recording_preflight_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-import-review-result-recording-"));
...
test("passes with current read-only approval import review result recording preflight", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /read_only_approval_import_review_result_recording_preflight_contract\.json/);
});

test("opens recording preflight without accepting, reading, recording, importing, provider, route, UI, DB, or order work", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingPreflight, true);
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalImportImplementationReview, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultAcceptedNow, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultReadNow, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultRecordedNow, false);
...
test("blocks if import review result supply gate starts reading or recording too early", () => {
  const workspace = makeWorkspace();
  const supply = readJson(workspace, "trading_lab_step116_read_only_approval_import_review_result_supply_gate_contract.json");
  supply.readiness.readyForFutureReadOnlyApprovalImportImplementationReview = true;
  supply.readiness.ownerRedactedApprovalImportReviewResultSuppliedNow = true;
  supply.readiness.currentStepReadsImportReviewResult = true;
  supply.readiness.currentStepRecordsImportReviewResult = true;
  supply.readiness.importImplementationAllowedNow = true;
  writeJson(workspace, "trading_lab_step116_read_only_approval_import_review_result_supply_gate_contract.json", supply);

  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingPreflight, false);
  assert.match(report.readiness.blockers.join("|"), /read_only_approval_import_review_result_supply_gate_not_ready/);
});
...
test("blocks if private result, importer, provider implementation, route, UI, or scenario artifacts appear", () => {
  ...
  const files = [
    ...
    path.join(workspace, "data", "private", "trading", "read_only_approval_import_review_result.redacted.json"),
