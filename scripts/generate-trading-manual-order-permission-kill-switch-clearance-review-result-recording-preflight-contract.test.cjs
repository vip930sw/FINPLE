...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-kill-switch-clearance-review-result-recording-preflight-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-kill-switch-review-result-recording-"));
...
test("passes with current kill-switch clearance review result recording preflight", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /kill_switch_clearance_review_result_recording_preflight_contract\.json/);
});

test("opens recording preflight without accepting, reading, recording, clearing, or ordering", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewResultRecordingPreflight, true);
  assert.equal(report.readiness.readyForKillSwitchClearanceReviewResultContract, false);
...
test("blocks if result supply gate is no longer ready", () => {
  ...
  supplyGate.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewResultSupply = false;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewResultRecordingPreflight, false);
  assert.match(
