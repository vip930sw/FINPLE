...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-validation-receipt-recording-preflight-contract.cjs",
);
const CONTRACT = "trading_lab_step116_manual_order_permission_validation_receipt_recording_preflight_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-receipt-recording-preflight-"));
...
test("passes with current validation receipt recording preflight", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /manual_order_permission_validation_receipt_recording_preflight_contract\.json/);
});

test("opens receipt recording preflight without reading validation result, recording receipt, or enabling trading", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionValidationReceiptRecordingAfterExecution, true);
  assert.equal(report.readiness.readyForKillSwitchClearanceReviewAfterValidationReceipt, true);
...
test("blocks if local validation execution preflight is no longer ready", () => {
  ...
  preflight.readiness.readyForOwnerLocalValidationExecutionAfterPath = false;
  writeJson(
    workspace,
    "trading_lab_step116_manual_order_permission_local_validation_execution_preflight_contract.json",
    preflight,
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionValidationReceiptRecordingAfterExecution, false);
  assert.match(report.readiness.blockers.join("|"), /local_validation_execution_preflight_not_ready/);
  assert.equal(report.readiness.orderSubmissionAllowed, false);
});
...
test("blocks if receipt review preflight starts reading or recording receipts early", () => {
  ...
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionValidationReceiptRecordingAfterExecution, false);
  assert.match(report.readiness.blockers.join("|"), /validation_result_receipt_review_preflight_not_ready/);
});
