...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-recording-result-review-preflight-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-import-recording-result-review-preflight-"));
...
test("passes with current read-only approval import review result recording result review preflight", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /read_only_approval_import_review_result_recording_result_review_preflight_contract\.json/);
});

test("opens only a future review preflight while import, provider, route, UI, DB, and order work stay blocked", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewPreflight, true);
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalImportImplementationReview, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewAcceptedNow, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewReadNow, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewRecordedNow, false);
  assert.equal(report.currentState.currentStepReadsRecordingResult, false);
  assert.equal(report.currentState.currentStepRecordsReview, false);
...
test("blocks if recording result contract starts reading or recording too early", () => {
  const workspace = makeWorkspace();
  const recordingResult = readJson(
    workspace,
    "trading_lab_step116_read_only_approval_import_review_result_recording_result_contract.json",
  );
  recordingResult.readiness.readyForFutureReadOnlyApprovalImportImplementationReview = true;
  recordingResult.readiness.currentStepReadsRecordingResult = true;
  recordingResult.readiness.currentStepRecordsRecordingResult = true;
  recordingResult.readiness.providerCallsAllowed = true;
  writeJson(
    workspace,
    "trading_lab_step116_read_only_approval_import_review_result_recording_result_contract.json",
    recordingResult,
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewPreflight, false);
  assert.match(report.readiness.blockers.join("|"), /read_only_approval_import_review_result_recording_result_contract_not_ready/);
});
...
test("blocks if private review, importer, provider implementation, route, UI, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
    path.join(workspace, "data", "private", "trading", "read_only_approval.redacted.json"),
    path.join(workspace, "data", "private", "trading", "read_only_approval_import_review_recording_result_review.redacted.json"),
