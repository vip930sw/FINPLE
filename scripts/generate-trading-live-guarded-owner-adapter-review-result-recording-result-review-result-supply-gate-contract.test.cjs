...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-owner-adapter-review-result-recording-result-review-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-owner-adapter-recording-result-review-result-supply-"));
...
test("passes with current owner adapter review result recording result review result supply gate", () => {
  ...
  assert.match(
    result.stdout,
    /live_guarded_owner_adapter_review_result_recording_result_review_result_supply_gate_contract\.json/,
  );
});
...
test("opens review result supply gate without reading, recording, implementing, calls, or orders", () => {
  ...
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewResultSupplyGate, true);
...
test("blocks if review preflight is no longer ready", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_live_guarded_owner_adapter_review_result_recording_result_review_preflight_contract.json",
  );
  preflight.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewPreflight = false;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewResultSupplyGate, false);
  assert.match(
...
test("blocks if recording result contract starts reading or recording result", () => {
  ...
  contract.readiness.ownerAdapterReviewResultRecordedNow = true;
  writeJson(
    workspace,
    "trading_lab_step116_live_guarded_owner_adapter_review_result_recording_result_contract.json",
    contract,
