...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight-validator-fixtures.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-provider-response-receipt-review-fixtures-"));
...
test("passes with current read-only provider response receipt review preflight validator fixtures", () => {
...
test("records synthetic review preflight validator fixtures while keeping receipt reads, provider calls, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForResponseEnvelopeValidationResultReceiptReviewPreflightValidatorRegression, true);
...
test("rejects stale fixtures if receipt, provider, or order flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_provider_response_envelope_validation_result_receipt_review_preflight_validator_fixtures\.json is out of date/,
  );
});
