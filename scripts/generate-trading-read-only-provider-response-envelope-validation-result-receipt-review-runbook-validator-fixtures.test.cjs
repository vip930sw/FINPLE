...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-validator-fixtures.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-provider-response-receipt-review-runbook-fixtures-"));
...
test("passes with current read-only provider response receipt review runbook validator fixtures", () => {
...
test("records synthetic validator fixtures while keeping receipt reads, provider calls, routes, UI, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForResponseReviewRunbookValidatorFixtureRegression, true);
...
test("keeps fixture catalog redacted and validates expected local failures", () => {
  ...
  assert.match(invalidIds.join("|"), /validator_run_enabled/);
  assert.match(invalidIds.join("|"), /receipt_read_enabled/);
  assert.match(invalidIds.join("|"), /receipt_record_enabled/);
  assert.match(invalidIds.join("|"), /provider_call_action_enabled/);
  assert.match(invalidIds.join("|"), /missing_review_assertion/);
  assert.match(invalidIds.join("|"), /changed_future_receipt_path/);
  assert.match(invalidIds.join("|"), /changed_validator_command/);
  assert.match(invalidIds.join("|"), /changed_review_preflight_command/);
...
test("rejects stale fixtures if receipt read or trading flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_provider_response_envelope_validation_result_receipt_review_runbook_validator_fixtures\.json is out of date/,
  );
});

test("blocks if upstream response review runbook opens too early", () => {
  const workspace = makeWorkspace();
