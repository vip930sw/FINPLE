...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-provider-call-authorization-preflight-validator-fixtures.cjs",
);
const CONTRACT = "trading_lab_step116_read_only_provider_call_authorization_preflight_validator_fixtures.json";
const CALL_AUTHORIZATION_PREFLIGHT = "trading_lab_step116_read_only_provider_call_authorization_preflight.json";
const VALIDATOR = "validate-trading-read-only-provider-call-authorization-preflight.cjs";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-call-auth-validator-fixtures-"));
  const processedDir = path.join(workspace, "data", "processed");
  fs.mkdirSync(processedDir, { recursive: true });
  fs.copyFileSync(path.join("data", "processed", CONTRACT), path.join(processedDir, CONTRACT));
  fs.copyFileSync(
    path.join("data", "processed", CALL_AUTHORIZATION_PREFLIGHT),
    path.join(processedDir, CALL_AUTHORIZATION_PREFLIGHT),
  );
...
test("passes with current read-only provider call authorization preflight validator fixtures", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /trading_lab_step116_read_only_provider_call_authorization_preflight_validator_fixtures\.json/);
});

test("generation keeps provider calls, routes, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForReadOnlyProviderCallAuthorizationPreflightValidatorFixtureRegression, true);
  assert.equal(report.readiness.currentStepAuthorizesProviderCalls, false);
  assert.equal(report.readiness.currentStepCallsProvider, false);
  assert.equal(report.readiness.currentStepRefreshesToken, false);
...
test("rejects stale fixtures if trading flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_provider_call_authorization_preflight_validator_fixtures\.json is out of date/,
  );
});

test("blocks if upstream call authorization preflight opens too early", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(workspace, CALL_AUTHORIZATION_PREFLIGHT);
  preflight.readiness.readyForFutureReadOnlyProviderCallAuthorizationReview = true;
  preflight.readiness.providerCallAuthorizationAllowedNow = true;
  preflight.readiness.providerCallsAllowed = true;
  writeJson(workspace, CALL_AUTHORIZATION_PREFLIGHT, preflight);

  const result = runFixtures(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.checks.callAuthorizationPreflightReady, false);
  assert.equal(report.readiness.readyForReadOnlyProviderCallAuthorizationPreflightValidatorFixtureRegression, false);
