...
function makeContractFile(contract) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "finple-validation-result-receipt-review-preflight-validator-"));
  const filePath = path.join(dir, "validation-result-receipt-review-preflight.json");
...
test("validates the current validation result receipt review preflight", () => {
...
test("rejects receipt, provider, or order boundary actions enabled now", () => {
  const contract = currentContract();
  ...
  contract.futureValidationResultReceiptReviewBoundary.currentStepImportsPermissionPacket = true;
...
test("rejects incomplete forbidden review content and changed future receipt path", () => {
  const contract = currentContract();
...
test("rejects trading allow flags and raw secret-shaped values without echoing the raw value", () => {
  ...
  contract.currentState.permissionPacketImportedNow = true;
