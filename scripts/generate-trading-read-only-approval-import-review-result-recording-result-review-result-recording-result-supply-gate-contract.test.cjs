...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-recording-result-review-result-recording-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-import-review-result-recording-result-supply-gate-"));
...
test("passes with current read-only approval import review-result recording-result supply gate", () => {
  ...
  assert.match(
    result.stdout,
    /read_only_approval_import_review_result_recording_result_review_result_recording_result_supply_gate_contract\.json/,
  );
});

test("opens only the supply gate while import, provider, route, UI, DB, and order work stay blocked", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(
    report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingResultSupplyGate,
    true,
  );
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalImportImplementationReview, false);
  assert.equal(
    report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordingResultSuppliedNow,
    false,
  );
  assert.equal(
    report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordingResultReadNow,
    false,
  );
  assert.equal(
    report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordingResultRecordedNow,
    false,
  );
...
test("blocks if recording preflight starts accepting or recording too early", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_read_only_approval_import_review_result_recording_result_review_result_recording_preflight_contract.json",
  );
  preflight.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingPreflight = false;
  preflight.readiness.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultAcceptedNow = true;
  preflight.readiness.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordedNow = true;
...
test("blocks if review-result contract starts reading or recording too early", () => {
  const workspace = makeWorkspace();
  const reviewResult = readJson(
    workspace,
    "trading_lab_step116_read_only_approval_import_review_result_recording_result_review_result_contract.json",
  );
  reviewResult.readiness.readyForFutureReadOnlyApprovalImportImplementationReview = true;
  reviewResult.readiness.currentStepReadsReviewResult = true;
  reviewResult.readiness.currentStepRecordsReviewResult = true;
...
test("blocks if private recording result, importer, provider implementation, route, UI, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
