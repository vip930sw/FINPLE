...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-supply-gate-contract.cjs",
);
const CONTRACT = "trading_lab_step116_read_only_approval_import_review_result_supply_gate_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-import-review-result-supply-"));
...
test("passes with current read-only approval import review result supply gate", () => {
  ...
  assert.match(result.stdout, /read_only_approval_import_review_result_supply_gate_contract\.json/);
});

test("opens supply boundary without reading review result, packet, provider, route, UI, or DB", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultSupply, true);
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalImportImplementationReview, false);
  assert.equal(report.currentState.ownerRedactedApprovalImportReviewResultSuppliedNow, false);
  assert.equal(report.currentState.currentStepReadsImportReviewResult, false);
  assert.equal(report.currentState.currentStepRecordsImportReviewResult, false);
...
test("blocks if import review preflight starts reading or recording too early", () => {
  const workspace = makeWorkspace();
  const importReview = readJson(workspace, "trading_lab_step116_read_only_approval_import_review_preflight_contract.json");
  importReview.readiness.currentStepReadsReviewResult = true;
  importReview.readiness.currentStepRecordsReviewResult = true;
  importReview.readiness.approvalPacketImportedNow = true;
  writeJson(workspace, "trading_lab_step116_read_only_approval_import_review_preflight_contract.json", importReview);

  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultSupply, false);
  assert.match(report.readiness.blockers.join("|"), /read_only_approval_import_review_preflight_not_ready/);
});
...
test("blocks if private packet, importer, provider implementation, route, UI, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
