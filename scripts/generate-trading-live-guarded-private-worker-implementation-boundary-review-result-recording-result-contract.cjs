...
const CONTRACT_VERSION =
  "trading-lab-step116-live-guarded-private-worker-implementation-boundary-review-result-recording-result-v0.1";
const AUDITED_AT = "2026-07-02T00:00:00Z";
const REQUIRED_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_GATES = [
...
const FORBIDDEN_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_CONTENT = [
...
function buildContract() {
  ...
  const boundaryReviewResultRecordingResultGates = [...REQUIRED_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_GATES];
  const forbiddenBoundaryReviewResultRecordingResultContent = [
    ...FORBIDDEN_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_CONTENT,
  ];
  const missingBoundaryReviewResultRecordingResultGates = missingValues(
    boundaryReviewResultRecordingResultGates,
    REQUIRED_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_GATES,
  );
  const missingForbiddenBoundaryReviewResultRecordingResultContent = missingValues(
    forbiddenBoundaryReviewResultRecordingResultContent,
    FORBIDDEN_BOUNDARY_REVIEW_RESULT_RECORDING_RESULT_CONTENT,
  );
  ...
  const checks = {
    contractOnly: true,
    privateWorkerImplementationBoundaryReviewResultRecordingResultContractOnly: true,
    recordingResultSupplyGateReady:
      ...
      recordingResultSupplyGate.readiness?.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultSuppliedNow ===
        false &&
      recordingResultSupplyGate.readiness?.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultReadNow ===
        false &&
      ...
      recordingResultSupplyGate.readiness?.currentStepStartsWorkerRuntime === false &&
      failClosed(recordingResultSupplyGate),
    ...
    boundaryReviewResultRecordingResultGatesReady: missingBoundaryReviewResultRecordingResultGates.length === 0,
    forbiddenBoundaryReviewResultRecordingResultContentReady:
      missingForbiddenBoundaryReviewResultRecordingResultContent.length === 0,
    architectureDocMentionsPrivateWorkerImplementationBoundaryReviewResultRecordingResult:
      architectureDoc.includes(
        "Trading Live-Guarded Private Worker Implementation Boundary Review Result Recording Result Contract",
      ) &&
      architectureDoc.includes("live_guarded_private_worker_implementation_boundary_review_result_recording_result"),
  ...
  const readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultContract =
    ...
    checks.boundaryReviewResultRecordingResultGatesReady &&
    checks.forbiddenBoundaryReviewResultRecordingResultContentReady &&
    checks.architectureDocMentionsPrivateWorkerImplementationBoundaryReviewResultRecordingResult &&
  ...
  return stableJson({
    ...
    step: "Step 116-7X",
    scope: "live_guarded_private_worker_implementation_boundary_review_result_recording_result",
    sourceFiles: {
    ...
    privateWorkerImplementationBoundaryReviewResultRecordingResultBoundary: {
      ...
      nextAllowedAction:
        "after the recording-result contract boundary is reviewed, add a separate private-worker implementation boundary review result recording result review preflight",
      boundaryReviewResultRecordingResultGates,
      forbiddenBoundaryReviewResultRecordingResultContent,
    ...
    checks,
    evidence: {
      missingBoundaryReviewResultRecordingResultGates,
      missingForbiddenBoundaryReviewResultRecordingResultContent,
    ...
    readiness: {
      status: readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultContract
        ? "live_guarded_private_worker_implementation_boundary_review_result_recording_result_contract_ready_pending_owner_recording_result"
        : "blocked_before_live_guarded_private_worker_implementation_boundary_review_result_recording_result_contract",
      readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultContract,
      ...
      blockers: [
        ...
        ...missingBoundaryReviewResultRecordingResultGates.map(
          (gate) => `missing_private_worker_implementation_boundary_review_result_recording_result_gate_${gate}`,
        ),
        ...missingForbiddenBoundaryReviewResultRecordingResultContent.map(
          (content) =>
            `missing_forbidden_private_worker_implementation_boundary_review_result_recording_result_content_${content}`,
        ),
        ...(checks.architectureDocMentionsPrivateWorkerImplementationBoundaryReviewResultRecordingResult
          ? []
          : ["architecture_doc_missing_private_worker_implementation_boundary_review_result_recording_result"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log(
      "[generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-contract] ok",
    );
    console.log(
      `[generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    `[generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-contract] wrote ${CONTRACT_PATH}`,
  );
}
