...
const CONTRACT_VERSION =
  "trading-lab-step116-live-guarded-private-worker-implementation-review-result-recording-preflight-v0.1";
const AUDITED_AT = "2026-07-02T00:00:00Z";
const REQUIRED_WORKER_REVIEW_RESULT_RECORDING_PREFLIGHT_GATES = [
...
const FORBIDDEN_WORKER_REVIEW_RESULT_RECORDING_PREFLIGHT_CONTENT = [
...
function buildContract() {
  ...
  const workerReviewResultRecordingPreflightGates = [
    ...REQUIRED_WORKER_REVIEW_RESULT_RECORDING_PREFLIGHT_GATES,
  ];
  const forbiddenWorkerReviewResultRecordingPreflightContent = [
    ...FORBIDDEN_WORKER_REVIEW_RESULT_RECORDING_PREFLIGHT_CONTENT,
  ];
  const missingWorkerReviewResultRecordingPreflightGates = missingValues(
    workerReviewResultRecordingPreflightGates,
    REQUIRED_WORKER_REVIEW_RESULT_RECORDING_PREFLIGHT_GATES,
  );
  const missingForbiddenWorkerReviewResultRecordingPreflightContent = missingValues(
    forbiddenWorkerReviewResultRecordingPreflightContent,
    FORBIDDEN_WORKER_REVIEW_RESULT_RECORDING_PREFLIGHT_CONTENT,
  );
  ...
  const checks = {
    contractOnly: true,
    privateWorkerImplementationReviewResultRecordingPreflightOnly: true,
    workerReviewResultSupplyGateReady:
      ...
      workerReviewResultSupplyGate.readiness?.currentStepRecordsPrivatePath === false &&
      workerReviewResultSupplyGate.readiness?.currentStepRecordsRawValues === false &&
      workerReviewResultSupplyGate.readiness?.currentStepOpensPrivateWorkerImplementation === false &&
      workerReviewResultSupplyGate.readiness?.currentStepImplementsPrivateWorker === false &&
    ...
    workerReviewResultRecordingPreflightGatesReady:
      missingWorkerReviewResultRecordingPreflightGates.length === 0,
    forbiddenWorkerReviewResultRecordingPreflightContentReady:
      missingForbiddenWorkerReviewResultRecordingPreflightContent.length === 0,
    architectureDocMentionsPrivateWorkerImplementationReviewResultRecordingPreflight:
      architectureDoc.includes(
        "Trading Live-Guarded Private Worker Implementation Review Result Recording Preflight",
      ) &&
      architectureDoc.includes("live_guarded_private_worker_implementation_review_result_recording_preflight"),
    noRuntimeArtifacts: forbiddenArtifacts.length === 0,
    kisPersonalPermissionExternalBlocker: false,
    ownerPrivateWorkerImplementationReviewResultAcceptedNow: false,
  ...
  const readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingPreflight =
    checks.workerReviewResultSupplyGateReady &&
    ...
    checks.workerReviewResultRecordingPreflightGatesReady &&
    checks.forbiddenWorkerReviewResultRecordingPreflightContentReady &&
    checks.architectureDocMentionsPrivateWorkerImplementationReviewResultRecordingPreflight &&
  ...
  return stableJson({
    ...
    step: "Step 116-7M",
    scope: "live_guarded_private_worker_implementation_review_result_recording_preflight",
    sourceFiles: {
    ...
    privateWorkerImplementationReviewResultRecordingPreflightBoundary: {
      ...
      nextAllowedAction:
        "after the owner supplies a redacted private-worker implementation review result outside repo commits, add a separate hash-only review result recording result supply gate",
      workerReviewResultRecordingPreflightGates,
      forbiddenWorkerReviewResultRecordingPreflightContent,
    ...
    checks,
    evidence: {
      missingWorkerReviewResultRecordingPreflightGates,
      missingForbiddenWorkerReviewResultRecordingPreflightContent,
    ...
    readiness: {
      status: readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingPreflight
        ? "live_guarded_private_worker_implementation_review_result_recording_preflight_ready_pending_owner_worker_review_result"
        : "blocked_before_live_guarded_private_worker_implementation_review_result_recording_preflight",
      readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingPreflight,
      ...
      blockers: [
        ...
        ...missingWorkerReviewResultRecordingPreflightGates.map(
          (gate) => `missing_private_worker_implementation_review_result_recording_preflight_gate_${gate}`,
        ),
        ...missingForbiddenWorkerReviewResultRecordingPreflightContent.map(
          (content) =>
            `missing_forbidden_private_worker_implementation_review_result_recording_preflight_content_${content}`,
        ),
        ...(checks.architectureDocMentionsPrivateWorkerImplementationReviewResultRecordingPreflight
          ? []
          : ["architecture_doc_missing_private_worker_implementation_review_result_recording_preflight"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log(
      "[generate-trading-live-guarded-private-worker-implementation-review-result-recording-preflight-contract] ok",
    );
    console.log(
      `[generate-trading-live-guarded-private-worker-implementation-review-result-recording-preflight-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    `[generate-trading-live-guarded-private-worker-implementation-review-result-recording-preflight-contract] wrote ${CONTRACT_PATH}`,
  );
}
