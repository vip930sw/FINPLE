...
const SCRIPT_PATH = path.resolve("scripts", "generate-trading-read-only-approval-import-review-preflight-contract.cjs");
const CONTRACT = "trading_lab_step116_read_only_approval_import_review_preflight_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-approval-import-review-preflight-"));
...
test("passes with current read-only approval import review preflight", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /read_only_approval_import_review_preflight_contract\.json/);
});

test("opens import review preflight without importing approval evidence or authorizing provider calls", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewPreflight, true);
  ...
  assert.equal(report.currentState.ownerRedactedApprovalValidationReceiptReviewResultSuppliedNow, false);
...
test("blocks if review result supply gate is no longer ready", () => {
  ...
  supplyGate.readiness.readyForReadOnlyApprovalValidationReceiptReviewResultSupply = false;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewPreflight, false);
  assert.match(report.readiness.blockers.join("|"), /approval_validation_receipt_review_result_supply_gate_not_ready/);
});

test("blocks if import implementation or provider call authorization opens too early", () => {
  const workspace = makeWorkspace();
  ...
  approvalImport.readiness.ownerPacketReadAllowedNow = true;
