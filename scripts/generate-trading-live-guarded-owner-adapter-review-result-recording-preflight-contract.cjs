...
const CONTRACT_VERSION = "trading-lab-step116-live-guarded-owner-adapter-review-result-recording-preflight-v0.1";
...
function buildContract() {
  ...
  const preflightGates = [...REQUIRED_RECORDING_PREFLIGHT_GATES];
  const forbiddenPreflightContent = [...FORBIDDEN_RECORDING_PREFLIGHT_CONTENT];
  const missingPreflightGates = missingValues(preflightGates, REQUIRED_RECORDING_PREFLIGHT_GATES);
  ...
  const checks = {
    contractOnly: true,
    ownerAdapterReviewResultRecordingPreflightOnly: true,
    ownerAdapterReviewResultSupplyGateReady:
      ...
      ownerAdapterReviewResultSupplyGate.readiness?.kisPersonalPermissionExternalBlocker === false &&
      ...
      ownerAdapterReviewResultSupplyGate.readiness?.currentStepOpensPrivateWorkerImplementation === false &&
      ownerAdapterReviewResultSupplyGate.readiness?.currentStepCallsProvider === false &&
      ownerAdapterReviewResultSupplyGate.readiness?.currentStepSubmitsOrders === false &&
    ...
    architectureDocMentionsOwnerAdapterReviewResultRecordingPreflight:
      architectureDoc.includes("Trading Live-Guarded Owner Adapter Review Result Recording Preflight") &&
      architectureDoc.includes("live_guarded_owner_adapter_review_result_recording_preflight"),
    noRuntimeArtifacts: forbiddenArtifacts.length === 0,
    kisPersonalPermissionExternalBlocker: false,
    ownerAdapterReviewResultAcceptedNow: false,
    ownerAdapterReviewResultReadNow: false,
  ...
  const readyForLiveGuardedOwnerAdapterReviewResultRecordingPreflight =
    checks.ownerAdapterReviewResultSupplyGateReady &&
    ...
    checks.architectureDocMentionsOwnerAdapterReviewResultRecordingPreflight &&
  ...
  return stableJson({
    ...
    step: "Step 116-7E",
    scope: "live_guarded_owner_adapter_review_result_recording_preflight",
    sourceFiles: {
    ...
    readiness: {
      status: readyForLiveGuardedOwnerAdapterReviewResultRecordingPreflight
        ? "live_guarded_owner_adapter_review_result_recording_preflight_ready_pending_owner_result"
        : "blocked_before_live_guarded_owner_adapter_review_result_recording_preflight",
      readyForLiveGuardedOwnerAdapterReviewResultRecordingPreflight,
      ...
      blockers: [
        ...
        ...missingPreflightGates.map((gate) => `missing_owner_adapter_review_result_recording_preflight_gate_${gate}`),
        ...missingForbiddenPreflightContent.map(
          (content) => `missing_forbidden_owner_adapter_review_result_recording_preflight_content_${content}`,
        ),
        ...(checks.architectureDocMentionsOwnerAdapterReviewResultRecordingPreflight
          ? []
          : ["architecture_doc_missing_owner_adapter_review_result_recording_preflight"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log("[generate-trading-live-guarded-owner-adapter-review-result-recording-preflight-contract] ok");
    console.log(
      `[generate-trading-live-guarded-owner-adapter-review-result-recording-preflight-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    `[generate-trading-live-guarded-owner-adapter-review-result-recording-preflight-contract] wrote ${CONTRACT_PATH}`,
  );
}
