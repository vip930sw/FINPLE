...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-snapshot-normalization-validator-fixtures.cjs",
);
const CONTRACT = "trading_lab_step116_read_only_snapshot_normalization_validator_fixtures.json";
const SNAPSHOT_NORMALIZATION_CONTRACT = "trading_lab_step116_read_only_snapshot_normalization_contract.json";
const RESPONSE_ENVELOPE_VALIDATOR_FIXTURES =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-snapshot-normalization-fixtures-"));
  const processedDir = path.join(workspace, "data", "processed");
  fs.mkdirSync(processedDir, { recursive: true });
  for (const fileName of [CONTRACT, SNAPSHOT_NORMALIZATION_CONTRACT, RESPONSE_ENVELOPE_VALIDATOR_FIXTURES]) {
...
test("passes with current snapshot normalization validator fixtures", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /trading_lab_step116_read_only_snapshot_normalization_validator_fixtures\.json/);
});

test("records synthetic normalized snapshot fixtures while keeping provider calls, routes, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForSnapshotNormalizationFixtureRegression, true);
  assert.equal(report.readiness.rawPayloadStored, false);
...
test("keeps fixture catalog redacted and validates expected pass/fail outcomes", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  ...
  assert.equal(report.validation.evidence.invalidFixturesMatchExpectedResults, true);
  assert.equal(report.validation.validSyntheticSnapshot.rawPayloadStored, false);
  assert.match(invalidIds.join("|"), /missing_value_hash/);
  assert.match(invalidIds.join("|"), /unknown_snapshot_type/);
  assert.match(invalidIds.join("|"), /raw_payload_stored/);
...
test("rejects stale fixtures if provider call flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_snapshot_normalization_validator_fixtures\.json is out of date/,
  );
});

test("blocks if snapshot normalization contract or response fixtures open too early", () => {
  const workspace = makeWorkspace();
  const normalization = readJson(workspace, SNAPSHOT_NORMALIZATION_CONTRACT);
  const responseFixtures = readJson(workspace, RESPONSE_ENVELOPE_VALIDATOR_FIXTURES);
  normalization.readiness.providerCallsAllowed = true;
  responseFixtures.readiness.orderSubmissionAllowed = true;
  writeJson(workspace, SNAPSHOT_NORMALIZATION_CONTRACT, normalization);
  writeJson(workspace, RESPONSE_ENVELOPE_VALIDATOR_FIXTURES, responseFixtures);
  ...
  assert.equal(report.checks.responseEnvelopeValidatorFixturesReady, false);
  assert.match(report.readiness.blockers.join("|"), /snapshot_normalization_contract_not_ready/);
  assert.match(report.readiness.blockers.join("|"), /response_envelope_validator_fixtures_not_ready/);
});

test("blocks if normalizer service, provider service, route, UI, private packet, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
    path.join(workspace, "server", "src", "services", "trading", "readOnlySnapshotNormalizer.js"),
