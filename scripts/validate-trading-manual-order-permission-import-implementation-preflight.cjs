...
function validateFutureBoundary(errors, boundary) {
  if (!isPlainObject(boundary)) {
    errors.push(
      makeError(
        ...
        "futureManualOrderPermissionImportImplementationBoundary",
        "future import implementation boundary must be an object",
      ),
    );
    return;
  }
  if (boundary.futureImportServicePath !== FUTURE_IMPORT_SERVICE_PATH) {
    errors.push(
      makeError(
        "invalid_future_import_service_path",
        "futureManualOrderPermissionImportImplementationBoundary.futureImportServicePath",
        "future import service path must stay fixed",
      ),
    );
  }
  if (boundary.futurePermissionPacketPath !== FUTURE_PERMISSION_PACKET_PATH) {
    errors.push(
      makeError(
        ...
        "futureManualOrderPermissionImportImplementationBoundary.futurePermissionPacketPath",
        "future permission packet path must stay fixed",
      ),
    );
  }
  for (const flag of [
    "currentStepReadsPrivatePacket",
    "currentStepWritesPrivatePacket",
    "currentStepImportsPacket",
    ...
    if (boundary[flag] !== false) {
      errors.push(
        makeError(
          ...
          `futureManualOrderPermissionImportImplementationBoundary.${flag}`,
          `${flag} must be false`,
        ),
      );
    }
  ...
  pushMissingArrayErrors(
    ...
    "futureManualOrderPermissionImportImplementationBoundary.reviewGates",
    "missing_review_gate",
  );
  pushMissingArrayErrors(
    errors,
    ...
    "futureManualOrderPermissionImportImplementationBoundary.implementationRules",
  ...
  pushMissingArrayErrors(
    ...
    "futureManualOrderPermissionImportImplementationBoundary.forbiddenPreflightContent",
    "missing_forbidden_preflight_content",
  );
}

function validateManualOrderPermissionImportImplementationPreflight(contract) {
  const errors = [];
  ...
  validateFutureBoundary(errors, contract.futureManualOrderPermissionImportImplementationBoundary);
  ...
  for (const containerName of ["currentState", "checks", "readiness"]) {
    ...
    for (const flag of [
      "importImplementationAllowedNow",
      "ownerPacketReadAllowedNow",
...
function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/validate-trading-manual-order-permission-import-implementation-preflight.cjs --contract <path>",
    );
    return;
  }
  if (args.unknown) {
  ...
  const result = validateManualOrderPermissionImportImplementationPreflight(contract);
