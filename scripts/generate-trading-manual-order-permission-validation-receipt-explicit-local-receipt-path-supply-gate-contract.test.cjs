...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-validation-receipt-explicit-local-receipt-path-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-validation-receipt-path-supply-gate-"));
...
test("passes with current validation receipt explicit local receipt path supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /manual_order_permission_validation_receipt_explicit_local_receipt_path_supply_gate_contract\.json/);
});

test("opens receipt path supply gate without reading or recording a receipt", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForOwnerExplicitLocalValidationReceiptPathSupply, true);
  assert.equal(report.readiness.readyForFutureManualOrderPermissionValidationReceiptPathIntake, true);
  assert.equal(report.currentState.ownerLocalValidationReceiptPathSuppliedNow, false);
  assert.equal(report.currentState.ownerValidationReceiptSuppliedNow, false);
...
test("blocks if validation execution result supply gate is no longer ready", () => {
  const workspace = makeWorkspace();
  const supplyGate = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_validation_execution_result_supply_gate_contract.json",
  );
  supplyGate.readiness.readyForOwnerValidationExecutionResultSupply = false;
  writeJson(
    workspace,
    "trading_lab_step116_manual_order_permission_validation_execution_result_supply_gate_contract.json",
    supplyGate,
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForOwnerExplicitLocalValidationReceiptPathSupply, false);
  assert.match(report.readiness.blockers.join("|"), /validation_execution_result_supply_gate_not_ready/);
});
