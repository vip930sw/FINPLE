...
const CHECK_PATH = "scripts/check-trading-step150-admin-trading-lab-mock-portfolio-ledger-update-review-result-recording-gate.cjs";

test("Step 150 mock portfolio ledger update review result checker passes", () => {
  const output = execFileSync(process.execPath, [CHECK_PATH], { encoding: "utf8" });
  assert.match(output, /\[check-trading-step150-admin-trading-lab-mock-portfolio-ledger-update-review-result-recording-gate\] ok/);
});

test("Step 150 checker guards endpoint, UI scope, no-go terms, and scenario files", () => {
