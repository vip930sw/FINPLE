...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-kill-switch-clearance-review-result-receipt-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-kill-switch-review-result-receipt-"));
...
test("passes with current kill-switch clearance review result receipt contract", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /kill_switch_clearance_review_result_receipt_contract\.json/);
});

test("opens receipt boundary without reading, recording, clearing, opening risk gate, or ordering", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewResultReceipt, true);
  assert.equal(report.readiness.readyForRiskGateClearanceReviewAfterKillSwitch, false);
  assert.equal(report.currentState.ownerKillSwitchClearanceReviewResultReceiptRecordedNow, false);
  assert.equal(report.currentState.currentStepReadsClearanceResult, false);
  assert.equal(report.currentState.currentStepRecordsClearanceResultReceipt, false);
...
test("blocks if kill-switch clearance review result contract is no longer ready", () => {
  const workspace = makeWorkspace();
  const reviewResult = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_kill_switch_clearance_review_result_contract.json",
  );
  reviewResult.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewResultContract = false;
  reviewResult.readiness.currentStepRecordsClearanceResult = true;
  reviewResult.readiness.currentStepClearsKillSwitch = true;
  writeJson(
    workspace,
    "trading_lab_step116_manual_order_permission_kill_switch_clearance_review_result_contract.json",
    reviewResult,
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewResultReceipt, false);
  assert.match(
...
test("blocks if recording preflight starts reading or recording the owner result", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_kill_switch_clearance_review_result_recording_preflight_contract.json",
  );
  preflight.readiness.currentStepReadsClearanceResult = true;
...
test("blocks if kill-switch or risk-gate contracts start enabling runtime", () => {
  const workspace = makeWorkspace();
