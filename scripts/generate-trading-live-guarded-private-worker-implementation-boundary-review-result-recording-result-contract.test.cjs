...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-boundary-recording-result-"));
...
test("passes with current private worker implementation boundary review result recording result contract", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /private_worker_implementation_boundary_review_result_recording_result_contract\.json/);
});

test("opens recording result contract without accepting, reading, recording, implementing, calls, runtime, routes, UI, DB, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultContract, true);
...
test("blocks if recording result supply gate no longer passes", () => {
  ...
  supplyGate.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultSupplyGate = false;
  supplyGate.readiness.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultSuppliedNow = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultRecordingResultContract, false);
  assert.match(
