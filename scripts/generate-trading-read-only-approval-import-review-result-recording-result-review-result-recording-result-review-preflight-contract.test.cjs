...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-recording-result-review-result-recording-result-review-preflight-contract.cjs",
);
const CONTRACT =
...
test("passes with current read-only approval import recording-result review preflight", () => {
  ...
  assert.match(
    result.stdout,
    /read_only_approval_import_review_result_recording_result_review_result_recording_result_review_preflight_contract\.json/,
  );
});

test("opens only the review preflight while import, provider, route, UI, DB, and order work stay blocked", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(
    report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingResultReviewPreflight,
    true,
  );
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalImportImplementationReview, false);
  assert.equal(
    report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordingResultReviewAcceptedNow,
    false,
  );
  assert.equal(
    report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordingResultReviewReadNow,
    false,
  );
  assert.equal(
    report.currentState.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordingResultReviewRecordedNow,
    false,
  );
...
test("blocks if recording-result contract starts recording too early", () => {
  const workspace = makeWorkspace();
  const contract = readJson(
    workspace,
    "trading_lab_step116_read_only_approval_import_review_result_recording_result_review_result_recording_result_contract.json",
  );
  contract.readiness.readyForFutureReadOnlyApprovalImportImplementationReview = true;
  contract.readiness.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordingResultRecordedNow = true;
  contract.readiness.providerCallsAllowed = true;
  writeJson(
    workspace,
    "trading_lab_step116_read_only_approval_import_review_result_recording_result_review_result_recording_result_contract.json",
    contract,
