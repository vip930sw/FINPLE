...
function makeReceiptFile(receipt) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "finple-provider-response-validation-result-receipt-"));
  const filePath = path.join(dir, "response_validation_result_receipt.redacted.json");
...
test("validates a redacted read-only provider response envelope validation result receipt", () => {
...
test("rejects missing fields and unknown fields", () => {
  ...
  delete receipt.responseEnvelopeShapeHash;
...
test("rejects malformed ids, hashes, timestamps, statuses, and redaction versions", () => {
  const result = runValidator([
    ...
    makeReceiptFile(
      validReceipt({
        responseValidationReceiptId: "receipt_123",
        validationStatus: "approved",
        validatedAt: "2026-06-29",
        validatorVersionHash: "not-a-hash",
        responseEnvelopeShapeHash: "shape",
  ...
  assert.equal(result.status, 1);
  assert.match(codes, /invalid_response_validation_receipt_id/);
...
test("rejects enabled envelope, raw response, provider, order, runtime, and UI flags", () => {
  const result = runValidator([
    "--receipt",
    makeReceiptFile(
      validReceipt({
        envelopePathRecorded: true,
        rawResponseRecorded: true,
        providerPayloadRecorded: true,
  ...
  const result = runValidator([
    ...
    makeReceiptFile(
      validReceipt({
        ...
        responseValidationReceiptId: "response_validation_receipt_1234567890",
      }),
    ),
  ]);
