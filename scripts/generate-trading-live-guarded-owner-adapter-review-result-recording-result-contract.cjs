...
const CONTRACT_VERSION = "trading-lab-step116-live-guarded-owner-adapter-review-result-recording-result-v0.1";
...
function buildContract() {
  ...
  const checks = {
    contractOnly: true,
    ownerAdapterReviewResultRecordingResultContractOnly: true,
    recordingResultSupplyGateReady:
      ...
      recordingResultSupplyGate.readiness?.readyForPrivateWorkerImplementationAfterOwnerAdapterReviewResult === false &&
      recordingResultSupplyGate.readiness?.kisPersonalPermissionExternalBlocker === false &&
      ...
      recordingResultSupplyGate.readiness?.currentStepCallsProvider === false &&
      recordingResultSupplyGate.readiness?.currentStepSubmitsOrders === false &&
    ...
    architectureDocMentionsOwnerAdapterReviewResultRecordingResult:
      architectureDoc.includes("Trading Live-Guarded Owner Adapter Review Result Recording Result Contract") &&
      architectureDoc.includes("live_guarded_owner_adapter_review_result_recording_result"),
  ...
  const readyForLiveGuardedOwnerAdapterReviewResultRecordingResultContract =
    ...
    checks.architectureDocMentionsOwnerAdapterReviewResultRecordingResult &&
  ...
  return stableJson({
    ...
    step: "Step 116-7G",
    scope: "live_guarded_owner_adapter_review_result_recording_result",
    sourceFiles: {
    ...
    readiness: {
      status: readyForLiveGuardedOwnerAdapterReviewResultRecordingResultContract
        ? "live_guarded_owner_adapter_review_result_recording_result_contract_ready_pending_owner_recording_result"
        : "blocked_before_live_guarded_owner_adapter_review_result_recording_result_contract",
      readyForLiveGuardedOwnerAdapterReviewResultRecordingResultContract,
      ...
      blockers: [
        ...
        ...missingRecordingResultGates.map((gate) => `missing_owner_adapter_review_result_recording_result_gate_${gate}`),
        ...missingForbiddenRecordingResultContent.map(
          (content) => `missing_forbidden_owner_adapter_review_result_recording_result_content_${content}`,
        ),
        ...(checks.architectureDocMentionsOwnerAdapterReviewResultRecordingResult
          ? []
          : ["architecture_doc_missing_owner_adapter_review_result_recording_result"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log("[generate-trading-live-guarded-owner-adapter-review-result-recording-result-contract] ok");
    console.log(
      `[generate-trading-live-guarded-owner-adapter-review-result-recording-result-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    `[generate-trading-live-guarded-owner-adapter-review-result-recording-result-contract] wrote ${CONTRACT_PATH}`,
  );
}
