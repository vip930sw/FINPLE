...
const {
  validateTradingManualOrderPermissionValidationResultReceipt,
} = require("./validate-trading-manual-order-permission-validation-result-receipt.cjs");
...
const CONTRACT_VERSION =
  "trading-lab-step116-manual-order-permission-validation-result-receipt-validator-fixtures-v0.1";
...
const REQUIRED_INVALID_FIXTURE_IDS = [
  ...
  "permission_packet_imported_flag_enabled",
...
function validSyntheticValidationResultReceipt() {
  return {
    ...
    packetShapeHash: "hmac-sha256:fixture_packet_shape_hash_123456",
    errorCodeHashes: [],
    redactionVersion: "v1",
...
function invalidSyntheticFixtures(baseReceipt) {
  return [
    { id: "missing_required_field", expectedErrorCodes: ["missing_required_field"], receipt: omitField(baseReceipt, "packetShapeHash") },
    ...
    { id: "malformed_hash_field", expectedErrorCodes: ["malformed_hash_field"], receipt: { ...baseReceipt, packetShapeHash: "not-a-labelled-hash" } },
    {
    ...
    {
      id: "permission_packet_imported_flag_enabled",
      expectedErrorCodes: ["forbidden_flag_enabled"],
      receipt: { ...baseReceipt, permissionPacketImportedNow: true },
    },
    ...
    },
    {
      ...
      expectedErrorCodes: ["forbidden_string_value"],
      receipt: { ...baseReceipt, validatorVersionHash: "sha256:manual_order_permission.redacted.json_marker" },
    },
  ];
}

function fixtureValidationEvidence(validReceipt, invalidFixtures) {
  const validResult = validateTradingManualOrderPermissionValidationResultReceipt(validReceipt);
  const invalidResults = invalidFixtures.map((fixture) => {
    const result = validateTradingManualOrderPermissionValidationResultReceipt(fixture.receipt);
...
function buildContract() {
  ...
  const syntheticValidReceipt = validSyntheticValidationResultReceipt();
  ...
  const checks = {
    ...
    validatorExportsLocalValidation:
      validatorSource.includes("validateTradingManualOrderPermissionValidationResultReceipt") &&
    ...
    architectureDocMentionsReceiptValidatorFixtures:
      architectureDoc.includes("Trading Manual Order Permission Validation Result Receipt Validator Fixtures") &&
      architectureDoc.includes("manual_order_permission_validation_result_receipt_validator_fixtures"),
  ...
  const readyForManualOrderPermissionValidationResultReceiptValidatorRegression =
  ...
  return stableJson({
    ...
    step: "Step 116-4N",
    scope: "manual_order_permission_validation_result_receipt_validator_fixtures",
    sourceFiles: {
    ...
    validation: {
      redactionBoundary:
        "synthetic validation-result receipt fixtures only; no real receipt, private packet path, raw account, operator, credential, provider payload, order payload, token, app key, app secret, execution, fill, or order confirmation content",
    ...
    readiness: {
      status: readyForManualOrderPermissionValidationResultReceiptValidatorRegression
        ? "fixtures_ready_for_manual_order_permission_validation_result_receipt_validator_regression"
        : "blocked_before_manual_order_permission_validation_result_receipt_validator_fixture_regression",
      readyForManualOrderPermissionValidationResultReceiptValidatorRegression,
      fixturesOnly: true,
      validationReceiptRecordedNow: false,
      packetPathRecorded: false,
      rawValuesRecorded: false,
      ...
      blockers: [
        ...
        ...(checks.architectureDocMentionsReceiptValidatorFixtures
          ? []
          : ["architecture_doc_missing_manual_order_permission_validation_result_receipt_validator_fixtures"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-manual-order-permission-validation-result-receipt-validator-fixtures.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-manual-order-permission-validation-result-receipt-validator-fixtures.cjs`,
      );
    }
    console.log("[generate-trading-manual-order-permission-validation-result-receipt-validator-fixtures] ok");
    console.log(
      `[generate-trading-manual-order-permission-validation-result-receipt-validator-fixtures] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-manual-order-permission-validation-result-receipt-validator-fixtures] wrote contract");
  console.log(
    `[generate-trading-manual-order-permission-validation-result-receipt-validator-fixtures] readyForManualOrderPermissionValidationResultReceiptValidatorRegression=${parsed.readiness.readyForManualOrderPermissionValidationResultReceiptValidatorRegression}`,
  );
}
