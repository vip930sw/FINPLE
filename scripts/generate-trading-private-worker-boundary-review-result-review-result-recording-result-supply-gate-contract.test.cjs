...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-private-worker-boundary-review-result-review-result-recording-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
test("passes with current boundary review-result recording result supply gate", () => {
  ...
  assert.match(
    result.stdout,
    /private_worker_boundary_review_result_review_result_recording_result_supply_gate/,
  );
});
...
test("blocks if architecture doc does not mention recording result supply gate", () => {
  const workspace = makeWorkspace();
  const docTarget = path.join(workspace, DOC_PATH);
