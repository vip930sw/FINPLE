...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-kill-switch-clearance-review-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-kill-switch-review-result-supply-"));
...
test("passes with current kill-switch clearance review result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /kill_switch_clearance_review_result_supply_gate_contract\.json/);
});

test("opens result supply gate without accepting, recording, clearing, or ordering", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewResultSupply, true);
  assert.equal(report.readiness.readyForKillSwitchClearanceReviewResultRecording, false);
  ...
  assert.equal(report.currentState.currentStepAcceptsClearanceResult, false);
...
test("blocks if kill-switch clearance review preflight is no longer ready", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_kill_switch_clearance_review_preflight_contract.json",
  );
  preflight.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewPreflight = false;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionKillSwitchClearanceReviewResultSupply, false);
  assert.match(
