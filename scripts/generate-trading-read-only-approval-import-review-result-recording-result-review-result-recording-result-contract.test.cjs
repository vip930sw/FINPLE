...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-recording-result-review-result-recording-result-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-import-recording-result-contract-"));
...
test("passes with current read-only approval import recording-result contract", () => {
  ...
  assert.match(
    result.stdout,
    /read_only_approval_import_review_result_recording_result_review_result_recording_result_contract\.json/,
  );
});

test("records only the contract boundary while import, provider, route, UI, DB, and order work stay blocked", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingResult, true);
...
test("blocks if supply gate starts reading or recording too early", () => {
  const workspace = makeWorkspace();
  const supplyGate = readJson(
    workspace,
    "trading_lab_step116_read_only_approval_import_review_result_recording_result_review_result_recording_result_supply_gate_contract.json",
  );
  supplyGate.readiness.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordingResultReadNow = true;
  supplyGate.readiness.ownerRedactedApprovalImportReviewResultRecordingResultReviewResultRecordingResultRecordedNow = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingResult, false);
  assert.match(
