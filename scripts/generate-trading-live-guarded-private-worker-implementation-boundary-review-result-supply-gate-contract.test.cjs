...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-boundary-review-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-boundary-review-result-supply-"));
...
test("passes with current private worker implementation boundary review result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /private_worker_implementation_boundary_review_result_supply_gate_contract\.json/);
});

test("opens boundary review result supply gate without reading, recording, implementing, calls, routes, UI, DB, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultSupplyGate, true);
  ...
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryReviewResultSuppliedNow, false);
...
test("blocks if boundary review starts accepting results or opening worker implementation", () => {
  const workspace = makeWorkspace();
  ...
  review.readiness.ownerPrivateWorkerImplementationBoundaryReviewSuppliedNow = true;
  review.readiness.privateWorkerImplementationBoundaryReviewResultRecordedNow = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryReviewResultSupplyGate, false);
  assert.match(
