...
const SCRIPT_PATH = path.resolve("scripts", "check-trading-step121-risk-kill-switch-review-result-recording-gate.cjs");
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-step121-check-"));
  [
...
test("passes with current Step 121 admin-only review result recording gate", () => {
  ...
  assert.match(result.stdout, /step121-risk-kill-switch-review-result-recording-gate/);
});

test("blocks if review result UI appears in mypage source", () => {
  const workspace = makeWorkspace();
  const accountPagePath = path.join(workspace, "src", "components", "AccountPages.jsx");
  fs.appendFileSync(accountPagePath, "\n<TradingReadinessPanel riskKillSwitchReviewResultStatus />\n");
  const result = runCheck(workspace);
...
test("blocks if review result endpoint loses the admin guard", () => {
  const workspace = makeWorkspace();
