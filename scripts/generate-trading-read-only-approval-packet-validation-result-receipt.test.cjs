...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-packet-validation-result-receipt.cjs",
);
const CONTRACT = "trading_lab_step116_read_only_approval_packet_validation_result_receipt.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-read-only-approval-validation-result-receipt-"));
...
test("passes with current read-only approval packet validation result receipt contract", () => {
  const workspace = makeWorkspace();
  const result = runReceipt(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /trading_lab_step116_read_only_approval_packet_validation_result_receipt\.json/);
});

test("records redacted receipt boundary while keeping validation execution, packet paths, raw values, imports, provider calls, routes, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runReceipt(workspace, []);
  ...
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalPacketValidationResultReceiptReview, true);
...
test("keeps receipt schema redacted and requires separate import and provider authorization reviews", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  ...
  assert.doesNotMatch(serialized, /rawAccountIdentifier|rawProviderPayload|rawOrderPayload/);
});
...
test("rejects stale receipt if receipt, import, provider, or order flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_approval_packet_validation_result_receipt\.json is out of date/,
  );
});

test("blocks if upstream validation runbook or import preflight opens too early", () => {
  const workspace = makeWorkspace();
  const runbook = readJson(workspace, "trading_lab_step116_read_only_approval_packet_validation_runbook_contract.json");
  runbook.readiness.currentStepRunsValidator = true;
  writeJson(workspace, "trading_lab_step116_read_only_approval_packet_validation_runbook_contract.json", runbook);
  ...
  assert.match(report.readiness.blockers.join("|"), /read_only_approval_packet_validation_runbook_not_ready/);
  assert.match(report.readiness.blockers.join("|"), /read_only_approval_import_implementation_preflight_not_blocked/);
});

test("blocks if private packet, importer, provider, route, UI, migration, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
