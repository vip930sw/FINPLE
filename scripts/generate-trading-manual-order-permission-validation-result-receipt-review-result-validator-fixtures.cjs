...
const {
  validateManualOrderPermissionValidationResultReceiptReviewResultContract,
} = require("./validate-trading-manual-order-permission-validation-result-receipt-review-result-contract.cjs");
...
const CONTRACT_VERSION =
  "trading-lab-step116-manual-order-permission-validation-result-receipt-review-result-validator-fixtures-v0.1";
    ...
    },
    {
      ...
      contract: {
        ...
        futureValidationResultReceiptReviewResultBoundary: {
          ...baseContract.futureValidationResultReceiptReviewResultBoundary,
      ...
      contract: {
        ...
        futureValidationResultReceiptReviewResultBoundary: {
          ...
          requiredReviewResultFields:
            baseContract.futureValidationResultReceiptReviewResultBoundary.requiredReviewResultFields.filter(
      ...
      contract: {
        ...
        futureValidationResultReceiptReviewResultBoundary: {
          ...
          requiredReviewResultAssertions:
            baseContract.futureValidationResultReceiptReviewResultBoundary.requiredReviewResultAssertions.filter(
              (assertion) => assertion !== "review_result_requires_separate_permission_import_review",
            ),
        },
      },
    },
      ...
      contract: {
        ...
        futureValidationResultReceiptReviewResultBoundary: {
          ...
          forbiddenReviewResultContent:
            baseContract.futureValidationResultReceiptReviewResultBoundary.forbiddenReviewResultContent.filter(
...
function fixtureValidationEvidence(validContract, invalidFixtures) {
  const validResult = validateManualOrderPermissionValidationResultReceiptReviewResultContract(validContract);
  const invalidResults = invalidFixtures.map((fixture) => {
    const result = validateManualOrderPermissionValidationResultReceiptReviewResultContract(fixture.contract);
...
function buildContract() {
  ...
  const checks = {
    ...
    reviewResultContractReady:
      reviewResultContract.readiness?.readyForFutureManualOrderPermissionValidationResultReceiptReviewResult === true &&
      ...
      reviewResultContract.readiness?.permissionPacketImportedNow === false &&
    ...
    validatorExportsLocalValidation:
      validatorSource.includes("validateManualOrderPermissionValidationResultReceiptReviewResultContract") &&
      validatorSource.includes("contract_path_required") &&
      validatorSource.includes("--contract"),
    architectureDocMentionsValidatorFixtures:
      architectureDoc.includes("Trading Manual Order Permission Validation Result Receipt Review Result Validator Fixtures") &&
      architectureDoc.includes("manual_order_permission_validation_result_receipt_review_result_validator_fixtures"),
  ...
  const readyForReviewResultValidatorFixtureRegression =
  ...
  return stableJson({
    ...
    step: "Step 116-4W",
    scope: "manual_order_permission_validation_result_receipt_review_result_validator_fixtures",
    sourceFiles: {
    ...
    validation: {
      redactionBoundary:
        "synthetic review-result validator fixtures only; no real validation receipt, private packet, account, operator, credential, provider payload, order payload, token, app key, app secret, execution, fill, or order confirmation content",
    ...
    readiness: {
      status: readyForReviewResultValidatorFixtureRegression
        ? "fixtures_ready_for_manual_order_permission_validation_result_receipt_review_result_validator_regression"
        : "blocked_before_manual_order_permission_validation_result_receipt_review_result_validator_fixture_regression",
      readyForReviewResultValidatorFixtureRegression,
      fixturesOnly: true,
      currentStepReadsReceipt: false,
      currentStepRecordsReviewResult: false,
      validationReceiptReviewRecordedNow: false,
      ...
      blockers: [
        ...(checks.reviewResultContractReady
          ? []
          : ["manual_order_permission_validation_result_receipt_review_result_contract_not_ready"]),
        ...(checks.validatorExportsLocalValidation
          ? []
          : ["manual_order_permission_validation_result_receipt_review_result_validator_not_ready"]),
        ...(checks.architectureDocMentionsValidatorFixtures
          ? []
          : ["architecture_doc_missing_manual_order_permission_validation_result_receipt_review_result_validator_fixtures"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-manual-order-permission-validation-result-receipt-review-result-validator-fixtures.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-manual-order-permission-validation-result-receipt-review-result-validator-fixtures.cjs`,
      );
    }
    console.log("[generate-trading-manual-order-permission-validation-result-receipt-review-result-validator-fixtures] ok");
    console.log(
      `[generate-trading-manual-order-permission-validation-result-receipt-review-result-validator-fixtures] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-manual-order-permission-validation-result-receipt-review-result-validator-fixtures] wrote contract");
  console.log(
    `[generate-trading-manual-order-permission-validation-result-receipt-review-result-validator-fixtures] readyForReviewResultValidatorFixtureRegression=${parsed.readiness.readyForReviewResultValidatorFixtureRegression}`,
  );
}
