...
const SCRIPT_PATH = path.resolve("scripts", "check-trading-step122-manual-approval-order-draft-preflight-core.cjs");
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-step122-check-"));
  [
...
test("passes with current Step 122 manual approval order draft preflight core", () => {
  const workspace = makeWorkspace();
  const result = runCheck(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /step122-manual-approval-order-draft-preflight-core/);
});

test("blocks if order draft UI appears in mypage source", () => {
  const workspace = makeWorkspace();
  const accountPagePath = path.join(workspace, "src", "components", "AccountPages.jsx");
  fs.appendFileSync(accountPagePath, "\n<TradingReadinessPanel manualApprovalOrderDraftStatus />\n");
  const result = runCheck(workspace);
...
test("blocks if order draft endpoint loses the admin guard", () => {
  const workspace = makeWorkspace();
