...
function validateReviewResultBoundary(errors, boundary) {
  ...
  if (boundary.futureValidationResultReceiptPath !== FUTURE_VALIDATION_RESULT_RECEIPT_PATH) {
    errors.push(
      makeError(
        ...
        "futureValidationResultReceiptReviewResultBoundary.futureValidationResultReceiptPath",
        "future validation result receipt path must stay fixed",
      ),
    );
  }
  for (const flag of ["currentStepReadsReceipt", "currentStepRecordsReviewResult", "currentStepImportsPermissionPacket"]) {
    if (boundary[flag] !== false) {
      errors.push(
        makeError(
          "review_result_action_enabled",
          `futureValidationResultReceiptReviewResultBoundary.${flag}`,
          `${flag} must be false`,
        ),
      );
    }
  ...
  pushMissingArrayErrors(
    ...
    "futureValidationResultReceiptReviewResultBoundary.requiredReviewResultFields",
  ...
  pushMissingArrayErrors(
    ...
    "futureValidationResultReceiptReviewResultBoundary.requiredReviewResultAssertions",
  ...
  pushMissingArrayErrors(
    ...
    "futureValidationResultReceiptReviewResultBoundary.forbiddenReviewResultContent",
    "missing_forbidden_review_result_content",
  );
}

function validateManualOrderPermissionValidationResultReceiptReviewResultContract(contract) {
  const errors = [];
  ...
  validateReviewResultBoundary(errors, contract.futureValidationResultReceiptReviewResultBoundary);
...
function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/validate-trading-manual-order-permission-validation-result-receipt-review-result-contract.cjs --contract <path>",
    );
    return;
  }
  if (args.unknown) {
  ...
  const result = validateManualOrderPermissionValidationResultReceiptReviewResultContract(contract);
