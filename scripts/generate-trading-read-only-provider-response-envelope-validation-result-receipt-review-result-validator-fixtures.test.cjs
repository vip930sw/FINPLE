...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-result-validator-fixtures.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-provider-response-receipt-review-result-fixtures-"));
...
test("passes with current read-only provider response validation receipt review result validator fixtures", () => {
...
test("records synthetic validator fixtures while keeping receipt reads, review-result writes, provider calls, routes, UI, and orders blocked", () => {
  ...
  assert.equal(report.readiness.readyForResponseReviewResultValidatorFixtureRegression, true);
...
test("keeps fixture catalog redacted and validates expected local failures", () => {
  ...
  assert.match(invalidIds.join("|"), /provider_payload_flag_enabled/);
...
test("rejects stale fixtures if receipt read or trading flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_provider_response_envelope_validation_result_receipt_review_result_validator_fixtures\.json is out of date/,
  );
});
