...
const REQUIRED_FILES = [
  ...
  "scripts/check-trading-step138-admin-trading-lab-strategy-draft-clearance-review-result-recording-gate.test.cjs",
];

const REQUIRED_SNIPPETS = [
  ["server/src/services/tradingAdminLabDashboardShell.js", "STEP138_ADMIN_TRADING_LAB_STRATEGY_DRAFT_CLEARANCE_REVIEW_RESULT_FLAGS"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_STRATEGY_DRAFT_CLEARANCE_REVIEW_RESULT_RECORDING_SCHEMA"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_STRATEGY_DRAFT_CLEARANCE_REVIEW_RECEIPT_SCHEMA"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_STRATEGY_DRAFT_CLEARANCE_REVIEW_DECISION_SUMMARY_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "TRADING_LAB_STRATEGY_DRAFT_CLEARANCE_REVIEW_BLOCKER_SUMMARY_MODEL"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "validateTradingLabStrategyDraftClearanceReviewResult"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabStrategyDraftClearanceReviewBlockerSummary"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabStrategyDraftClearanceReviewDecisionSummary"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabStrategyDraftClearanceReviewResult"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabStrategyDraftClearanceReviewReceipt"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildTradingLabStrategyDraftClearanceReviewResultRecordingGate"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "buildAdminTradingLabStrategyDraftClearanceReviewResultStatus"],
  ["server/src/services/tradingAdminLabDashboardShell.js", "admin_only_strategy_draft_clearance_review_result_recording_gate_fail_closed"],
  ...
  ["server/src/routes/adminTradingReadinessRoutes.js", "buildAdminTradingLabStrategyDraftClearanceReviewResultStatus"],
  ["server/src/routes/adminTradingReadinessRoutes.js", 'router.get("/trading-lab-strategy-draft-clearance-review-result"'],
  ["server/src/routes/adminTradingReadinessRoutes.js", "requireAdminAccess"],
  ["server/src/routes/adminTradingReadinessRoutes.test.js", "trading-lab-strategy-draft-clearance-review-result"],
  ["src/components/portfolio/services/serverPortfolioService.js", "fetchAdminTradingLabStrategyDraftClearanceReviewResultStatus"],
  ["src/components/portfolio/services/serverPortfolioService.js", "\"/admin/trading-readiness/trading-lab-strategy-draft-clearance-review-result\""],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabStrategyDraftClearanceReviewResult"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabClearanceReviewCards"],
  ["src/components/TradingReadinessPanel.jsx", "tradingLabClearanceReviewList"],
  ["src/components/TradingReadinessPanel.jsx", "no_order_draft"],
  ["src/App.css", ".tradingLabClearanceReviewCards"],
  ["src/App.css", ".tradingLabClearanceReviewList"],
  ["package.json", "check:trading-step138-admin-trading-lab-strategy-draft-clearance-review-result-recording-gate"],
];
...
function main() {
  ...
  if (!/router\.get\("\/trading-lab-strategy-draft-clearance-review-result"[\s\S]*requireAdminAccess[\s\S]*buildAdminTradingLabStrategyDraftClearanceReviewResultStatus/.test(routeText)) {
    fail("strategy draft clearance review result endpoint must stay admin-only and read-only");
  }
  ...
  const publicExposureTerms = [
    ...
    "trading-lab-strategy-draft-clearance-review-result",
  ...
  if (accountExposure.length > 0) fail(`strategy draft clearance review result must not be exposed on /mypage: ${accountExposure.join(", ")}`);
  const appExposure = publicExposureTerms.filter((term) => appText.includes(term));
  if (appExposure.length > 0) fail(`strategy draft clearance review result must not be exposed on homepage or public router: ${appExposure.join(", ")}`);
  ...
  const touchedScenarioFiles = scenarioFiles.filter((filePath) => fs.existsSync(filePath) && readText(filePath).includes("Step 138"));
  if (touchedScenarioFiles.length > 0) fail(`scenario runtime files must remain untouched: ${touchedScenarioFiles.join(", ")}`);

  console.log("[check-trading-step138-admin-trading-lab-strategy-draft-clearance-review-result-recording-gate] ok");
}
