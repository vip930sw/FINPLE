...
function validateReviewRunbookBoundary(errors, runbook) {
  ...
  if (runbook.futureValidationResultReceiptPath !== FUTURE_VALIDATION_RESULT_RECEIPT_PATH) {
    errors.push(
      makeError(
        ...
        "futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.futureValidationResultReceiptPath",
  ...
  if (runbook.validatorCommandTemplate !== REQUIRED_VALIDATOR_COMMAND) {
    errors.push(
      makeError(
        ...
        "futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.validatorCommandTemplate",
        "validator command must require an explicit owner-supplied redacted response receipt path",
      ),
    );
  }
  if (runbook.reviewPreflightValidatorCommandTemplate !== REQUIRED_REVIEW_PREFLIGHT_COMMAND) {
    errors.push(
      makeError(
        ...
        "futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.reviewPreflightValidatorCommandTemplate",
    ...
    if (runbook[flag] !== false) {
      errors.push(
        makeError(
          ...
          `futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.${flag}`,
          `${flag} must be false`,
        ),
      );
    }
  ...
  pushMissingArrayErrors(
    ...
    "futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.requiredReviewAssertions",
  ...
  pushMissingArrayErrors(
    ...
    "futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.redactedReviewOutputFields",
  ...
  pushMissingArrayErrors(
    ...
    "futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.forbiddenReviewOutputContent",
    "missing_forbidden_review_output_content",
  );
}

function validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewRunbookContract(contract) {
  const errors = [];
  ...
  validateReviewRunbookBoundary(errors, contract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook);
  ...
  for (const { path: valuePath, value } of collectStringValues(contract)) {
    if (
      ...
      valuePath === "$.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.futureValidationResultReceiptPath"
    ) {
      continue;
    }
...
function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/validate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-contract.cjs --contract <path>",
    );
    return;
  }
  if (args.unknown) {
  ...
  const result = validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewRunbookContract(contract);
