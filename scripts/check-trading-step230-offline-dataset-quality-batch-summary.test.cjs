...
test("Step230 checker passes and leaves the working tree unchanged", () => {
  const before = execFileSync("git", ["status", "--short"], { encoding: "utf8" });
  const output = execFileSync(process.execPath, ["scripts/check-trading-step230-offline-dataset-quality-batch-summary.cjs"], {
    encoding: "utf8",
  });
  const after = execFileSync("git", ["status", "--short"], { encoding: "utf8" });

  assert.equal(after, before);
  assert.match(output, /\[check-trading-step230-offline-dataset-quality-batch-summary\] ok/);
  assert.match(output, /"summaryMode": "offline_fixture_batch"/);
  assert.match(output, /"sourceProfileSchemaVersion": "1.0.0"/);
  assert.match(output, /"overallStatus": "blocked"/);
  assert.match(output, new RegExp(`"step228BaselineCommit": "${BASELINE_COMMIT}"`));
});

test("Step230 package script runs through the dedicated checker and tests only", () => {
  const packageJson = fs.readFileSync("package.json", "utf8");

  assert.match(packageJson, /check:trading-step230-offline-dataset-quality-batch-summary/);
  assert.match(packageJson, /scripts\/check-trading-step230-offline-dataset-quality-batch-summary\.cjs/);
  assert.match(packageJson, /server\/src\/services\/tradingAiMlDatasetQualityBatchSummary\.test\.js/);
  assert.match(packageJson, /scripts\/check-trading-step230-offline-dataset-quality-batch-summary\.test\.cjs/);
  assert.match(packageJson, /check:trading-step229-offline-dataset-quality-profile/);
});

test("Step230 keeps the Step228 snapshot counts and baseline unchanged", () => {
...
test("Step230 dirty file boundary excludes Step229, registries, Step192 runtime, UI, DB, and scenario assets", () => {
