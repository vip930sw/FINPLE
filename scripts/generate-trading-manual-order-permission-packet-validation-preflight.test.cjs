...
const SCRIPT_PATH = path.resolve("scripts", "generate-trading-manual-order-permission-packet-validation-preflight.cjs");
const CONTRACT = "trading_lab_step116_manual_order_permission_packet_validation_preflight.json";
const INPUTS = [
  CONTRACT,
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-packet-validation-preflight-"));
...
test("passes with current manual order permission packet validation preflight", () => {
  const workspace = makeWorkspace();
  const result = runPreflight(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /trading_lab_step116_manual_order_permission_packet_validation_preflight\.json/);
});

test("records owner-assisted validation readiness while keeping packet reads, imports, provider calls, routes, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runPreflight(workspace, []);
  ...
  assert.equal(report.readiness.readyForOwnerAssistedManualOrderPermissionPacketValidation, true);
...
test("rejects stale preflight if validation, import, provider, or order flags are manually enabled", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  ...
  assert.match(result.stderr, /trading_lab_step116_manual_order_permission_packet_validation_preflight\.json is out of date/);
});

test("blocks if upstream packet validator fixtures or import preflight open too early", () => {
  const workspace = makeWorkspace();
  const fixtures = readJson(workspace, "trading_lab_step116_manual_order_permission_packet_validator_fixtures.json");
  fixtures.readiness.permissionPacketImportedNow = true;
  writeJson(workspace, "trading_lab_step116_manual_order_permission_packet_validator_fixtures.json", fixtures);
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.checks.packetValidatorFixturesReady, false);
  assert.equal(report.checks.importImplementationPreflightStillBlocked, false);
  assert.match(report.readiness.blockers.join("|"), /manual_order_permission_packet_validator_fixtures_not_ready/);
