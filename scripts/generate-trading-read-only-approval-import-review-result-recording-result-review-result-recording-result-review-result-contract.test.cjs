...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-import-review-result-recording-result-review-result-recording-result-review-result-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-import-review-result-contract-"));
...
test("passes with current read-only approval import review-result contract", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /recording_result_review_result_contract\.json/);
});

test("records only the review-result contract while import, provider, route, UI, DB, and order work stay blocked", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(
    report.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingResultReviewResult,
    true,
  );
...
test("blocks if supply gate reads or records the review result too early", () => {
  const workspace = makeWorkspace();
  const supplyGate = readJson(
    workspace,
    "trading_lab_step116_read_only_approval_import_review_result_recording_result_review_result_recording_result_review_result_supply_gate_contract.json",
  );
  supplyGate.readiness.ownerRedactedReviewResultReadNow = true;
  supplyGate.readiness.ownerRedactedReviewResultRecordedNow = true;
  ...
  assert.match(report.readiness.blockers.join("|"), /review_result_supply_gate_not_ready/);
});
