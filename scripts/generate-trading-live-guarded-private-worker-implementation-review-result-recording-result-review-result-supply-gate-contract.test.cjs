...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-review-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
test("passes with current private worker recording-result review result supply gate", () => {
  ...
  assert.match(result.stdout, /private_worker_implementation_review_result_recording_result_review_result_supply_gate/);
});
...
test("opens review result supply gate without reading, recording, implementing, calls, or orders", () => {
  ...
  assert.equal(
    report.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGate,
    true,
  );
