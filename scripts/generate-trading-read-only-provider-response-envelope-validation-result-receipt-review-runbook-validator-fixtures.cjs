...
const {
  validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewRunbookContract,
} = require("./validate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-contract.cjs");
...
const VALIDATOR_PATH = path.join(
  "scripts",
  "validate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-contract.cjs",
);
...
const CONTRACT_VERSION =
  "trading-lab-step116-read-only-provider-response-envelope-validation-result-receipt-review-runbook-validator-fixtures-v0.1";
...
const FORBIDDEN_RUNTIME_ARTIFACTS = [
  path.join("data", "private", "trading", "read_only_provider_response_envelope_validation_result_receipt.redacted.json"),
...
function invalidSyntheticFixtures(baseContract) {
  return [
    ...
    },
    {
      ...
      contract: {
        ...baseContract,
        futureOwnerAssistedResponseValidationResultReceiptReviewRunbook: {
          ...baseContract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook,
      ...
      contract: {
        ...
        futureOwnerAssistedResponseValidationResultReceiptReviewRunbook: {
          ...
          requiredReviewAssertions:
            baseContract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.requiredReviewAssertions.filter(
      ...
      contract: {
        ...
        futureOwnerAssistedResponseValidationResultReceiptReviewRunbook: {
          ...
          redactedReviewOutputFields:
            baseContract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.redactedReviewOutputFields.filter(
      ...
      contract: {
        ...
        futureOwnerAssistedResponseValidationResultReceiptReviewRunbook: {
          ...
          forbiddenReviewOutputContent:
            baseContract.futureOwnerAssistedResponseValidationResultReceiptReviewRunbook.forbiddenReviewOutputContent.filter(
      ...
      contract: {
        ...
        futureOwnerAssistedResponseValidationResultReceiptReviewRunbook: {
          ...
          validatorCommandTemplate:
            "node scripts/validate-trading-read-only-provider-response-envelope-validation-result-receipt.cjs",
        },
      },
    },
    {
      ...
      contract: {
        ...
        futureOwnerAssistedResponseValidationResultReceiptReviewRunbook: {
          ...
          reviewPreflightValidatorCommandTemplate:
            "node scripts/validate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight.cjs",
        },
      },
    },
    {
...
function fixtureValidationEvidence(validContract, invalidFixtures) {
  const validResult = validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewRunbookContract(validContract);
  const invalidResults = invalidFixtures.map((fixture) => {
    const result = validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewRunbookContract(fixture.contract);
...
function buildContract() {
  ...
  const checks = {
    ...
    runbookContractReady:
      runbookContract.readiness?.readyForOwnerAssistedResponseValidationResultReceiptReviewRunbookReview === true &&
      ...
      runbookContract.readiness?.rawResponseRecorded === false &&
      runbookContract.readiness?.providerPayloadRecorded === false &&
    ...
    validatorExportsLocalValidation:
      validatorSource.includes("validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewRunbookContract") &&
      validatorSource.includes("contract_path_required") &&
      validatorSource.includes("--contract"),
    architectureDocMentionsValidatorFixtures:
      architectureDoc.includes("Trading Read-Only Provider Response Envelope Validation Result Receipt Review Runbook Validator Fixtures") &&
      architectureDoc.includes("read_only_provider_response_envelope_validation_result_receipt_review_runbook_validator_fixtures"),
  ...
  const readyForResponseReviewRunbookValidatorFixtureRegression =
  ...
  return stableJson({
    ...
    step: "Step 116-3L-I",
    scope: "read_only_provider_response_envelope_validation_result_receipt_review_runbook_validator_fixtures",
    sourceFiles: {
    ...
    validation: {
      redactionBoundary:
        "synthetic response review-runbook validator fixtures only; no real validation receipt, response envelope path, account, credential, provider payload, response payload, order payload, token, app key, app secret, execution, fill, or order confirmation content",
    ...
    readiness: {
      status: readyForResponseReviewRunbookValidatorFixtureRegression
        ? "fixtures_ready_for_read_only_provider_response_validation_result_receipt_review_runbook_validator_regression"
        : "blocked_before_read_only_provider_response_validation_result_receipt_review_runbook_validator_fixture_regression",
      readyForResponseReviewRunbookValidatorFixtureRegression,
      fixturesOnly: true,
      currentStepRunsValidator: false,
      currentStepReadsReceipt: false,
      validationReceiptRecordedNow: false,
      ...
      blockers: [
        ...(checks.runbookContractReady
          ? []
          : ["read_only_provider_response_validation_result_receipt_review_runbook_contract_not_ready"]),
        ...(checks.validatorExportsLocalValidation
          ? []
          : ["read_only_provider_response_validation_result_receipt_review_runbook_validator_not_ready"]),
        ...(checks.architectureDocMentionsValidatorFixtures
          ? []
          : ["architecture_doc_missing_read_only_provider_response_validation_result_receipt_review_runbook_validator_fixtures"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-validator-fixtures.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-validator-fixtures.cjs`,
      );
    }
    console.log("[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-validator-fixtures] ok");
    console.log(
      `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-validator-fixtures] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-validator-fixtures] wrote contract");
  console.log(
    `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-runbook-validator-fixtures] readyForResponseReviewRunbookValidatorFixtureRegression=${parsed.readiness.readyForResponseReviewRunbookValidatorFixtureRegression}`,
  );
}
