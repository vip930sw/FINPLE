...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-validation-result-receipt-review-result-validator-fixtures.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-receipt-review-result-fixtures-"));
...
test("passes with current manual order permission validation result receipt review result validator fixtures", () => {
...
test("records synthetic validator fixtures while keeping receipt reads, review-result writes, imports, routes, UI, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForReviewResultValidatorFixtureRegression, true);
...
test("rejects stale fixtures if receipt read or trading flags are manually enabled", () => {
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_manual_order_permission_validation_result_receipt_review_result_validator_fixtures\.json is out of date/,
  );
});
