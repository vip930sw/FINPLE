...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-review-recording-result-supply-"));
...
test("passes with current private worker implementation review result recording result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /private_worker_implementation_review_result_recording_result_supply_gate_contract\.json/);
});
...
test("opens recording result supply gate without reading, recording, implementing, calls, or orders", () => {
  ...
  assert.equal(
    report.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultSupplyGate,
    true,
  );
...
test("blocks if worker review result recording preflight starts recording result", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_review_result_recording_preflight_contract.json",
  );
  preflight.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingPreflight = false;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultSupplyGate, false);
  assert.match(
...
test("blocks if private worker review starts opening implementation", () => {
  const workspace = makeWorkspace();
