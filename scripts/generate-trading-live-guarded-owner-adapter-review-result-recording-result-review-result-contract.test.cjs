...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-owner-adapter-review-result-recording-result-review-result-contract.cjs",
);
const CONTRACT = "trading_lab_step116_live_guarded_owner_adapter_review_result_recording_result_review_result_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-owner-adapter-recording-result-review-result-"));
...
test("passes with current owner adapter review result recording result review result contract", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /live_guarded_owner_adapter_review_result_recording_result_review_result_contract\.json/);
});
...
test("opens review result contract without reading, recording, implementing, calls, or orders", () => {
  ...
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewResultContract, true);
  ...
  assert.equal(report.currentState.ownerAdapterReviewResultRecordingResultReviewResultSuppliedNow, false);
  assert.equal(report.currentState.ownerAdapterReviewResultRecordingResultReviewResultReadNow, false);
...
test("blocks if review result supply gate is no longer ready", () => {
  ...
  supply.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewResultSupplyGate = false;
  supply.readiness.ownerAdapterReviewResultRecordingResultReviewResultReadNow = true;
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedOwnerAdapterReviewResultRecordingResultReviewResultContract, false);
  assert.match(
...
test("blocks if review preflight starts accepting or recording review result", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_live_guarded_owner_adapter_review_result_recording_result_review_preflight_contract.json",
  );
  preflight.readiness.ownerAdapterReviewResultRecordingResultReviewAcceptedNow = true;
  preflight.readiness.ownerAdapterReviewResultReviewRecordedNow = true;
  writeJson(
    workspace,
    "trading_lab_step116_live_guarded_owner_adapter_review_result_recording_result_review_preflight_contract.json",
    preflight,
...
test("blocks if private review result, worker, adapter, route, UI, DB, or scenario artifact appears", () => {
  const workspace = makeWorkspace();
  const files = [
    path.join(workspace, "data", "private", "trading", "live_guarded_adapter_review_result_recording_result_review.redacted.json"),
