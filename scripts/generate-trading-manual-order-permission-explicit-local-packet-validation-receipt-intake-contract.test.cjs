...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-explicit-local-packet-validation-receipt-intake-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-explicit-path-intake-"));
...
test("passes with current explicit local packet validation receipt intake", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /manual_order_permission_explicit_local_packet_validation_receipt_intake_contract\.json/);
});

test("opens owner-supplied explicit local path intake without reading packet, recording path, or creating receipt", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForOwnerSuppliedExplicitLocalPacketValidation, true);
  assert.equal(report.readiness.readyForManualOrderPermissionValidationReceiptWithExplicitLocalPath, true);
...
test("blocks if owner-local packet preparation assertion is no longer ready", () => {
  const workspace = makeWorkspace();
  const assertion = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_owner_local_packet_preparation_assertion_contract.json",
  );
  assertion.readiness.readyForExplicitLocalPathValidationReceipt = false;
  writeJson(
    workspace,
    "trading_lab_step116_manual_order_permission_owner_local_packet_preparation_assertion_contract.json",
    assertion,
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionValidationReceiptWithExplicitLocalPath, false);
  assert.match(report.readiness.blockers.join("|"), /owner_local_packet_preparation_assertion_not_ready/);
  assert.equal(report.readiness.orderSubmissionAllowed, false);
});

test("blocks if validation receipt contract records a receipt or packet path too early", () => {
  const workspace = makeWorkspace();
  ...
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionValidationReceiptWithExplicitLocalPath, false);
  assert.match(report.readiness.blockers.join("|"), /validation_result_receipt_contract_not_ready/);
});

test("blocks if a private packet, receipt, route, or scenario artifact appears", () => {
  const workspace = makeWorkspace();
  ...
  assert.equal(report.evidence.forbiddenRuntimeArtifacts.length, 3);
});
