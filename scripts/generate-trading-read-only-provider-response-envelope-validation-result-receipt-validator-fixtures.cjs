...
} = require("./validate-trading-read-only-provider-response-envelope-validation-result-receipt.cjs");
...
const CONTRACT_VERSION =
  "trading-lab-step116-read-only-provider-response-envelope-validation-result-receipt-validator-fixtures-v0.1";
...
const REQUIRED_INVALID_FIXTURE_IDS = [
  ...
  "envelope_path_recorded_flag_enabled",
  "raw_response_recorded_flag_enabled",
  "provider_payload_recorded_flag_enabled",
...
function validSyntheticReceipt() {
  return {
    responseValidationReceiptId: "response_validation_receipt_fixture_001",
    validationStatus: "valid",
    validatedAt: "2026-06-29T00:00:00.000Z",
    validatorVersionHash: "sha256:fixture_validator_version_hash_123456",
    responseEnvelopeShapeHash: "hmac-sha256:fixture_response_envelope_shape_hash_123456",
    errorCodeHashes: [],
    redactionVersion: "v1",
  ...
  return [
    {
      ...
      receipt: omitField(baseReceipt, "responseEnvelopeShapeHash"),
    },
    { id: "unknown_field", expectedErrorCodes: ["unknown_field"], receipt: { ...baseReceipt, unexpected: "redacted_fixture_value" } },
    {
      id: "invalid_response_validation_receipt_id",
      expectedErrorCodes: ["invalid_response_validation_receipt_id"],
      receipt: { ...baseReceipt, responseValidationReceiptId: "receipt_fixture" },
    },
    ...
    { id: "malformed_timestamp", expectedErrorCodes: ["malformed_timestamp"], receipt: { ...baseReceipt, validatedAt: "2026-06-29" } },
    {
      ...
      receipt: { ...baseReceipt, responseEnvelopeShapeHash: "not-a-labelled-hash" },
    },
    {
    ...
    {
      id: "envelope_path_recorded_flag_enabled",
      expectedErrorCodes: ["forbidden_flag_enabled"],
      receipt: { ...baseReceipt, envelopePathRecorded: true },
    },
    {
      id: "raw_response_recorded_flag_enabled",
      expectedErrorCodes: ["forbidden_flag_enabled"],
      receipt: { ...baseReceipt, rawResponseRecorded: true },
    },
    {
      id: "provider_payload_recorded_flag_enabled",
      expectedErrorCodes: ["forbidden_flag_enabled"],
      receipt: { ...baseReceipt, providerPayloadRecorded: true },
    },
...
function fixtureValidationEvidence(validReceipt, invalidFixtures) {
  const validResult = validateReadOnlyProviderResponseEnvelopeValidationResultReceipt(validReceipt);
  const invalidResults = invalidFixtures.map((fixture) => {
    const result = validateReadOnlyProviderResponseEnvelopeValidationResultReceipt(fixture.receipt);
...
function buildContract() {
  ...
  const checks = {
    ...
    validatorExportsLocalValidation:
      validatorSource.includes("validateReadOnlyProviderResponseEnvelopeValidationResultReceipt") &&
    ...
    architectureDocMentionsReceiptValidatorFixtures:
      architectureDoc.includes("Trading Read-Only Provider Response Envelope Validation Result Receipt Validator Fixtures") &&
      architectureDoc.includes("read_only_provider_response_envelope_validation_result_receipt_validator_fixtures"),
  ...
  const readyForResponseEnvelopeValidationResultReceiptValidatorRegression =
  ...
  return stableJson({
    ...
    step: "Step 116-3L-C",
    scope: "read_only_provider_response_envelope_validation_result_receipt_validator_fixtures",
    sourceFiles: {
    ...
    validation: {
      redactionBoundary:
        "synthetic response-validation receipt fixtures only; no real provider response, private receipt path, raw account, credential, provider payload, order payload, token, app key, app secret, execution, fill, or order confirmation content",
    ...
    readiness: {
      status: readyForResponseEnvelopeValidationResultReceiptValidatorRegression
        ? "fixtures_ready_for_read_only_provider_response_envelope_validation_result_receipt_validator_regression"
        : "blocked_before_read_only_provider_response_envelope_validation_result_receipt_validator_fixture_regression",
      readyForResponseEnvelopeValidationResultReceiptValidatorRegression,
      fixturesOnly: true,
      validationReceiptRecordedNow: false,
      responseEnvelopePathRecorded: false,
      rawResponseRecorded: false,
      ...
      blockers: [
        ...
        ...(checks.architectureDocMentionsReceiptValidatorFixtures
          ? []
          : ["architecture_doc_missing_read_only_provider_response_validation_result_receipt_validator_fixtures"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-validator-fixtures.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-validator-fixtures.cjs`,
      );
    }
    console.log("[generate-trading-read-only-provider-response-envelope-validation-result-receipt-validator-fixtures] ok");
    console.log(
      `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-validator-fixtures] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-read-only-provider-response-envelope-validation-result-receipt-validator-fixtures] wrote contract");
  console.log(
    `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-validator-fixtures] readyForResponseEnvelopeValidationResultReceiptValidatorRegression=${parsed.readiness.readyForResponseEnvelopeValidationResultReceiptValidatorRegression}`,
  );
}
