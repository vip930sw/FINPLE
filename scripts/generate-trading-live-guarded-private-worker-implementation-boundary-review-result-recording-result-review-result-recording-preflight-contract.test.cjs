...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-boundary-review-result-recording-result-review-result-recording-preflight-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-boundary-review-result-recording-preflight-"));
...
test("passes with current boundary review-result recording preflight gate", () => {
  ...
  assert.match(
    result.stdout,
    /private_worker_implementation_boundary_review_result_recording_result_review_result_recording_preflight/,
  );
});

test("opens recording preflight without reading, recording, implementing, calls, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(
    report.currentState.ownerPrivateWorkerImplementationBoundaryReviewResultRecordingResultReviewResultAcceptedNow,
    false,
  );
  assert.equal(
...
test("blocks if architecture doc does not mention recording preflight", () => {
  const workspace = makeWorkspace();
  const docTarget = path.join(workspace, DOC_PATH);
