...
const CONTRACT_VERSION =
  "trading-lab-step116-manual-order-permission-import-implementation-review-result-supply-gate-v0.1";
...
const REQUIRED_REVIEW_RESULT_SUPPLY_GATES = [
  ...
  "owner_validation_receipt_review_result_required_later",
...
function buildContract() {
  ...
  const checks = {
    contractOnly: true,
    resultSupplyGateOnly: true,
    recordingPreflightReady:
      recordingPreflight.readiness?.readyForManualOrderPermissionImportImplementationReviewResultRecordingPreflight ===
        true &&
      recordingPreflight.readiness?.readyForPermissionImportAfterReviewResult === false &&
      recordingPreflight.readiness?.ownerReviewResultSuppliedNow === false &&
      recordingPreflight.readiness?.currentStepReadsReviewResult === false &&
      recordingPreflight.readiness?.currentStepReadsPrivatePacket === false &&
      recordingPreflight.readiness?.currentStepRecordsReviewResult === false &&
      recordingPreflight.readiness?.permissionPacketImportedNow === false &&
    ...
    architectureDocMentionsImportImplementationReviewResultSupplyGate:
      architectureDoc.includes("Trading Manual Order Permission Import Implementation Review Result Supply Gate") &&
      architectureDoc.includes("manual_order_permission_import_implementation_review_result_supply_gate"),
  ...
  const readyForManualOrderPermissionImportImplementationReviewResultSupply =
    ...
    checks.architectureDocMentionsImportImplementationReviewResultSupplyGate &&
  ...
  return stableJson({
    ...
    step: "Step 116-6O",
    scope: "manual_order_permission_import_implementation_review_result_supply_gate",
    sourceFiles: {
      manualOrderPermissionImportImplementationReviewResultRecordingPreflight: RECORDING_PREFLIGHT_PATH,
    ...
    manualOrderPermissionImportImplementationReviewResultSupplyGate: {
      ...
      ownerValidationReceiptReviewResultRequiredLater: true,
      ...
      nextAllowedAction:
        "after the owner supplies a redacted import implementation review result outside repo commits, record a separate permission import result preflight without reading private packet paths or importing permission evidence",
      reviewResultSupplyGates,
      forbiddenReviewResultSupplyContent,
    },
    checks,
    ...
    readiness: {
      status: readyForManualOrderPermissionImportImplementationReviewResultSupply
        ? "import_implementation_review_result_supply_gate_ready_pending_owner_review_result"
        : "blocked_before_import_implementation_review_result_supply_gate",
      readyForManualOrderPermissionImportImplementationReviewResultSupply,
      ...
      blockers: [
        ...
        ...(checks.architectureDocMentionsImportImplementationReviewResultSupplyGate
          ? []
          : ["architecture_doc_missing_import_implementation_review_result_supply_gate"]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log("[generate-trading-manual-order-permission-import-implementation-review-result-supply-gate-contract] ok");
    console.log(
      `[generate-trading-manual-order-permission-import-implementation-review-result-supply-gate-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-manual-order-permission-import-implementation-review-result-supply-gate-contract] wrote contract");
  console.log(
    `[generate-trading-manual-order-permission-import-implementation-review-result-supply-gate-contract] readyForManualOrderPermissionImportImplementationReviewResultSupply=${parsed.readiness.readyForManualOrderPermissionImportImplementationReviewResultSupply}`,
  );
}
