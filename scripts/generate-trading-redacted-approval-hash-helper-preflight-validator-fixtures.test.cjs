...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-redacted-approval-hash-helper-preflight-validator-fixtures.cjs",
);
const CONTRACT = "trading_lab_step116_redacted_approval_hash_helper_preflight_validator_fixtures.json";
const HASH_HELPER_PREFLIGHT = "trading_lab_step116_redacted_approval_hash_helper_preflight.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-redacted-hash-helper-preflight-fixtures-"));
  const processedDir = path.join(workspace, "data", "processed");
  fs.mkdirSync(processedDir, { recursive: true });
  for (const fileName of [CONTRACT, HASH_HELPER_PREFLIGHT]) {
...
test("passes with current redacted approval hash helper preflight validator fixtures", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /trading_lab_step116_redacted_approval_hash_helper_preflight_validator_fixtures\.json/);
});

test("records synthetic hash helper preflight validator fixtures while keeping hash preparation deferred", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForRedactedApprovalHashHelperPreflightValidatorRegression, true);
  assert.equal(report.readiness.ownerHashPreparationDeferred, true);
...
test("keeps fixture catalog redacted and validates expected local failures", () => {
  ...
  assert.match(invalidIds.join("|"), /hash_helper_implementation_enabled/);
  assert.match(invalidIds.join("|"), /hash_generation_enabled/);
  assert.match(invalidIds.join("|"), /approval_packet_creation_enabled/);
...
test("rejects stale fixtures if hash preparation, provider, or order flags are manually enabled", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  report.currentState.hashHelperImplementationAllowed = true;
  report.currentState.ownerHashPreparationDeferred = false;
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_redacted_approval_hash_helper_preflight_validator_fixtures\.json is out of date/,
  );
});

test("blocks if upstream redacted approval hash helper preflight opens too early", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(workspace, HASH_HELPER_PREFLIGHT);
  ...
  writeJson(workspace, HASH_HELPER_PREFLIGHT, preflight);
...
test("blocks if hash helper, private packet, provider, route, UI, DB, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
