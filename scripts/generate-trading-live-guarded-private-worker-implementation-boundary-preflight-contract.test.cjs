...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-boundary-preflight-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-boundary-preflight-"));
...
test("passes with current private worker implementation boundary preflight gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /private_worker_implementation_boundary_preflight/);
});

test("opens implementation boundary preflight without implementing, calls, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(
    report.readiness.readyForLiveGuardedPrivateWorkerImplementationBoundaryPreflight,
    true,
  );
  ...
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryPreflightSuppliedNow, false);
  assert.equal(report.currentState.ownerPrivateWorkerImplementationBoundaryPreflightReadNow, false);
  assert.equal(report.currentState.privateWorkerImplementationReviewResultReviewRecordedNow, false);
...
test("blocks if prior review result contract no longer passes", () => {
  const workspace = makeWorkspace();
  const priorContract = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_review_result_recording_result_review_result_contract.json",
  );
  priorContract.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultReviewResult = false;
  priorContract.readiness.currentStepOpensPrivateWorkerImplementation = true;
...
test("blocks if private worker implementation preflight opens implementation", () => {
  const workspace = makeWorkspace();
  ...
  assert.match(report.readiness.blockers.join("|"), /live_guarded_private_worker_implementation_preflight_not_fail_closed/);
});

test("blocks if review preflight no longer passes", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_review_result_recording_result_review_preflight_contract.json",
  );
  preflight.readiness.readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultReviewPreflight = false;
  preflight.readiness.ownerPrivateWorkerImplementationReviewResultRecordingResultReviewAcceptedNow = true;
  preflight.readiness.privateWorkerImplementationReviewResultReviewRecordedNow = true;
  writeJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_review_result_recording_result_review_preflight_contract.json",
    preflight,
...
test("blocks if recording result contract starts reading or recording result", () => {
  const workspace = makeWorkspace();
  const contract = readJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_review_result_recording_result_contract.json",
  );
  contract.readiness.ownerPrivateWorkerImplementationReviewResultRecordingResultReadNow = true;
  contract.readiness.privateWorkerImplementationReviewResultRecordedNow = true;
...
test("blocks if recording result supply gate starts supplying or recording result", () => {
  const workspace = makeWorkspace();
  ...
  supplyGate.readiness.privateWorkerImplementationReviewResultRecordedNow = true;
  writeJson(
    workspace,
    "trading_lab_step116_live_guarded_private_worker_implementation_review_result_recording_result_supply_gate_contract.json",
    supplyGate,
