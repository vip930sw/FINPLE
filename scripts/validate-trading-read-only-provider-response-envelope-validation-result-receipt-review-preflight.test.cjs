...
function makeContractFile(contract) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "finple-provider-response-receipt-review-preflight-validator-"));
  const filePath = path.join(dir, "provider-response-receipt-review-preflight.json");
...
test("validates the current read-only provider response validation result receipt review preflight", () => {
...
test("rejects trading allow flags and raw secret-shaped values without echoing the raw value", () => {
  ...
  contract.currentState.providerPayloadRecorded = true;
