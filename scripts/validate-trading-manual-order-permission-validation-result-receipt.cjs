...
const FORBIDDEN_STRING_PATTERNS = [
  ...
  /manual[_-]?order[_-]?permission\.redacted\.json/i,
];
...
function validateTradingManualOrderPermissionValidationResultReceipt(receipt) {
  const errors = [];
  ...
  if (Object.hasOwn(receipt, "validationReceiptId")) {
    if (
      typeof receipt.validationReceiptId !== "string" ||
      !/^validation_receipt_[A-Za-z0-9_-]{8,}$/.test(receipt.validationReceiptId)
    ) {
      errors.push(makeError("invalid_validation_receipt_id", "validationReceiptId", "validationReceiptId must be opaque"));
    }
  }
  ...
  for (const field of ["validatorVersionHash", "packetShapeHash"]) {
    ...
    if (!Array.isArray(receipt.errorCodeHashes)) {
      ...
      receipt.errorCodeHashes.forEach((value, index) => {
        if (!isLabelledHash(value)) {
          errors.push(makeError("malformed_error_code_hash", `errorCodeHashes[${index}]`, "error code entries must be labelled hashes"));
        }
      });
    }
  }
...
function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/validate-trading-manual-order-permission-validation-result-receipt.cjs --receipt <path>",
    );
    return;
  }
  if (args.unknown) {
  ...
  if (!args.receiptPath) {
    console.error(JSON.stringify({ valid: false, errors: [makeError("receipt_path_required", "$", "--receipt is required")] }));
    process.exitCode = 2;
    return;
  }
  let receipt;
  try {
    ...
    console.error(JSON.stringify({ valid: false, errors: [makeError("receipt_read_failed", "$", "receipt could not be read")] }));
    process.exitCode = 2;
    return;
  }
  const result = validateTradingManualOrderPermissionValidationResultReceipt(receipt);
