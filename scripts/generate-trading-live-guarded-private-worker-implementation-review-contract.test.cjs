...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-private-worker-implementation-review-contract.cjs",
);
const CONTRACT = "trading_lab_step116_live_guarded_private_worker_implementation_review_contract.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-private-worker-implementation-review-"));
...
test("passes with current private worker implementation review contract", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /live_guarded_private_worker_implementation_review_contract\.json/);
});

test("opens private worker review without implementing worker, calls, routes, UI, DB, or orders", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationReview, true);
  assert.equal(report.readiness.readyForPrivateWorkerImplementationAfterReview, false);
  assert.equal(report.currentState.kisPersonalPermissionExternalBlocker, false);
  assert.equal(report.currentState.ownerPrivateWorkerImplementationReviewSuppliedNow, false);
  assert.equal(report.currentState.ownerPrivateWorkerImplementationReviewReadNow, false);
  assert.equal(report.currentState.privateWorkerImplementationReviewRecordedNow, false);
...
test("blocks if private worker preflight starts implementation", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(workspace, "trading_lab_step116_live_guarded_private_worker_implementation_preflight_contract.json");
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForLiveGuardedPrivateWorkerImplementationReview, false);
  assert.match(report.readiness.blockers.join("|"), /live_guarded_private_worker_implementation_preflight_not_ready/);
});
...
test("blocks if private worker, adapter, route, UI, DB, or scenario artifact appears", () => {
  const workspace = makeWorkspace();
  const files = [
    path.join(workspace, "data", "private", "trading", "live_guarded_private_worker_review.redacted.json"),
