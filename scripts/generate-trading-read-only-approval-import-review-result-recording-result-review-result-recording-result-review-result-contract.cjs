...
const CONTRACT_VERSION =
  "trading-lab-step116-read-only-approval-import-review-result-recording-result-review-result-recording-result-review-result-v0.1";
const AUDITED_AT = "2026-07-03T00:00:00Z";
const REQUIRED_GATES = [
  "read_only_approval_import_review_result_recording_result_review_result_recording_result_review_result_supply_gate_ready",
...
function buildContract() {
  ...
  const checks = {
    ...
    supplyGateReady:
      supplyGate.readiness
        ?.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingResultReviewResultSupplyGate ===
        true &&
      supplyGate.readiness?.readyForFutureReadOnlyApprovalImportImplementationReview === false &&
      supplyGate.readiness?.ownerRedactedReviewResultSuppliedNow === false &&
      supplyGate.readiness?.ownerRedactedReviewResultReadNow === false &&
      supplyGate.readiness?.ownerRedactedReviewResultRecordedNow === false &&
    ...
    architectureDocMentionsReviewResultContract:
      architectureDoc.includes(
        "Trading Read-Only Approval Import Review Result Recording Result Review Result Recording Result Review Result Contract",
      ) &&
      architectureDoc.includes(
        "read_only_approval_import_review_result_recording_result_review_result_recording_result_review_result",
      ),
  ...
  return stableJson({
    ...
    step: "Step 116-5Q-X",
    scope: "read_only_approval_import_review_result_recording_result_review_result_recording_result_review_result",
    sourceFiles: {
    ...
    reviewResultContract: {
      ...
      nextAllowedAction:
        "after an owner redacted review result is supplied outside repo commits, continue only with a separate hash-only recording preflight before approval import implementation review",
      gates,
      forbiddenContent,
    },
    checks,
    ...
    readiness: {
      status: ready
        ? "read_only_approval_import_review_result_recording_result_review_result_recording_result_review_result_ready_import_still_blocked_pending_owner_review_result"
        : "blocked_before_read_only_approval_import_review_result_recording_result_review_result_recording_result_review_result",
      readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingResultReviewResult: ready,
      ...
      blockers: [
        ...(checks.supplyGateReady
          ? []
          : [
              "read_only_approval_import_review_result_recording_result_review_result_recording_result_review_result_supply_gate_not_ready",
            ]),
        ...(checks.reviewPreflightReady
          ? []
          : [
        ...
        ...missingGates.map((gate) => `missing_review_result_gate_${gate}`),
        ...missingForbiddenContent.map((content) => `missing_forbidden_review_result_content_${content}`),
        ...(checks.architectureDocMentionsReviewResultContract
          ? []
          : [
              "architecture_doc_missing_read_only_approval_import_review_result_recording_result_review_result_recording_result_review_result",
            ]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log(
      "[generate-trading-read-only-approval-import-review-result-recording-result-review-result-recording-result-review-result-contract] ok",
    );
    console.log(
      `[generate-trading-read-only-approval-import-review-result-recording-result-review-result-recording-result-review-result-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    "[generate-trading-read-only-approval-import-review-result-recording-result-review-result-recording-result-review-result-contract] wrote contract",
  );
  console.log(
    `[generate-trading-read-only-approval-import-review-result-recording-result-review-result-recording-result-review-result-contract] readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingResultReviewResult=${parsed.readiness.readyForReadOnlyApprovalImportReviewResultRecordingResultReviewResultRecordingResultReviewResult}`,
  );
}
