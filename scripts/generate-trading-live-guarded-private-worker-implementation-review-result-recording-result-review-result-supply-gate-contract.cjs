...
const CONTRACT_VERSION =
  "trading-lab-step116-live-guarded-private-worker-implementation-review-result-recording-result-review-result-supply-gate-v0.1";
const AUDITED_AT = "2026-07-02T00:00:00Z";
const REQUIRED_WORKER_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_GATES = [
...
const FORBIDDEN_WORKER_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_CONTENT = [
...
function buildContract() {
  ...
  const workerReviewResultRecordingResultReviewResultSupplyGates = [
    ...REQUIRED_WORKER_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_GATES,
  ];
  const forbiddenWorkerReviewResultRecordingResultReviewResultSupplyContent = [
    ...FORBIDDEN_WORKER_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_CONTENT,
  ];
  const missingWorkerReviewResultRecordingResultReviewResultSupplyGates = missingValues(
    workerReviewResultRecordingResultReviewResultSupplyGates,
    REQUIRED_WORKER_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_GATES,
  );
  const missingForbiddenWorkerReviewResultRecordingResultReviewResultSupplyContent = missingValues(
    forbiddenWorkerReviewResultRecordingResultReviewResultSupplyContent,
    FORBIDDEN_WORKER_REVIEW_RESULT_RECORDING_RESULT_REVIEW_RESULT_SUPPLY_CONTENT,
  );
  ...
  const checks = {
    contractOnly: true,
    privateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGateOnly: true,
    ...
    workerReviewResultRecordingResultReviewResultSupplyGatesReady:
      missingWorkerReviewResultRecordingResultReviewResultSupplyGates.length === 0,
    forbiddenWorkerReviewResultRecordingResultReviewResultSupplyContentReady:
      missingForbiddenWorkerReviewResultRecordingResultReviewResultSupplyContent.length === 0,
    architectureDocMentionsPrivateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGate:
      architectureDoc.includes(
        "Trading Live-Guarded Private Worker Implementation Review Result Recording Result Review Result Supply Gate",
      ) &&
      architectureDoc.includes(
        "live_guarded_private_worker_implementation_review_result_recording_result_review_result_supply_gate",
      ),
  ...
  const readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGate =
    ...
    checks.workerReviewResultRecordingResultReviewResultSupplyGatesReady &&
    checks.forbiddenWorkerReviewResultRecordingResultReviewResultSupplyContentReady &&
    checks.architectureDocMentionsPrivateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGate &&
  ...
  return stableJson({
    ...
    step: "Step 116-7Q",
    scope: "live_guarded_private_worker_implementation_review_result_recording_result_review_result_supply_gate",
    sourceFiles: {
    ...
    privateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGate: {
      ...
      nextAllowedAction:
        "after the owner supplies a redacted worker review recording-result review result outside repo commits, add a separate hash-only review result contract boundary",
      workerReviewResultRecordingResultReviewResultSupplyGates,
      forbiddenWorkerReviewResultRecordingResultReviewResultSupplyContent,
    ...
    checks,
    evidence: {
      missingWorkerReviewResultRecordingResultReviewResultSupplyGates,
      missingForbiddenWorkerReviewResultRecordingResultReviewResultSupplyContent,
    ...
    readiness: {
      status: readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGate
        ? "live_guarded_private_worker_implementation_review_result_recording_result_review_result_supply_gate_ready_pending_owner_review_result"
        : "blocked_before_live_guarded_private_worker_implementation_review_result_recording_result_review_result_supply_gate",
      readyForLiveGuardedPrivateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGate,
      ...
      blockers: [
        ...
        ...missingWorkerReviewResultRecordingResultReviewResultSupplyGates.map(
          (gate) =>
            `missing_private_worker_implementation_review_result_recording_result_review_result_supply_gate_${gate}`,
        ),
        ...missingForbiddenWorkerReviewResultRecordingResultReviewResultSupplyContent.map(
          (content) =>
            `missing_forbidden_private_worker_implementation_review_result_recording_result_review_result_supply_content_${content}`,
        ),
        ...(checks.architectureDocMentionsPrivateWorkerImplementationReviewResultRecordingResultReviewResultSupplyGate
          ? []
          : [
              "architecture_doc_missing_private_worker_implementation_review_result_recording_result_review_result_supply_gate",
            ]),
...
function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    console.log(
      "[generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-review-result-supply-gate-contract] ok",
    );
    console.log(
      `[generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-review-result-supply-gate-contract] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log(
    `[generate-trading-live-guarded-private-worker-implementation-review-result-recording-result-review-result-supply-gate-contract] wrote ${CONTRACT_PATH}`,
  );
}
