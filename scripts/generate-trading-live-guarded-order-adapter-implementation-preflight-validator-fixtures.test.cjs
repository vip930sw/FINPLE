...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-live-guarded-order-adapter-implementation-preflight-validator-fixtures.cjs",
);
const CONTRACT = "trading_lab_step116_live_guarded_order_adapter_implementation_preflight_validator_fixtures.json";
const LIVE_GUARDED_PREFLIGHT = "trading_lab_step116_live_guarded_order_adapter_implementation_preflight.json";
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-live-guarded-adapter-preflight-fixtures-"));
  const processedDir = path.join(workspace, "data", "processed");
  fs.mkdirSync(processedDir, { recursive: true });
  for (const fileName of [CONTRACT, LIVE_GUARDED_PREFLIGHT]) {
...
test("passes with current live-guarded order adapter implementation preflight validator fixtures", () => {
...
test("records synthetic live-guarded preflight fixtures while keeping provider, orders, routes, UI, and DB blocked", () => {
  const workspace = makeWorkspace();
  const result = runFixtures(workspace, []);
  ...
  assert.equal(report.readiness.readyForLiveGuardedPreflightValidatorRegression, true);
...
test("keeps fixture catalog redacted and validates expected local failures", () => {
  ...
  assert.match(invalidIds.join("|"), /manual_permission_import_enabled/);
  ...
  assert.match(invalidIds.join("|"), /public_ui_creation_enabled/);
  assert.match(invalidIds.join("|"), /changed_future_order_adapter_path/);
...
test("rejects stale fixtures if provider, order, route, or DB flags are manually enabled", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_live_guarded_order_adapter_implementation_preflight_validator_fixtures\.json is out of date/,
  );
});

test("blocks if upstream live-guarded preflight opens too early", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(workspace, LIVE_GUARDED_PREFLIGHT);
  preflight.readiness.orderAdapterImplementationAllowedNow = true;
  preflight.readiness.orderSubmissionAllowed = true;
  writeJson(workspace, LIVE_GUARDED_PREFLIGHT, preflight);

  const result = runFixtures(workspace, []);

  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.checks.liveGuardedPreflightReady, false);
  assert.match(report.readiness.blockers.join("|"), /live_guarded_order_adapter_implementation_preflight_not_ready/);
});

test("blocks if order adapter, route, UI, migration, private packet, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
