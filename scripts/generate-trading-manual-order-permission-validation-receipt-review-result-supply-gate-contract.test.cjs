...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-validation-receipt-review-result-supply-gate-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-receipt-review-result-supply-"));
...
test("passes with current validation receipt review result supply gate", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /manual_order_permission_validation_receipt_review_result_supply_gate_contract\.json/);
});

test("opens review result supply gate without recording review result or importing permission", () => {
  const workspace = makeWorkspace();
  const result = runContract(workspace, []);
  ...
  assert.equal(report.readiness.readyForManualOrderPermissionValidationReceiptReviewResultSupply, true);
  assert.equal(report.readiness.readyForPermissionImportReviewAfterReviewResult, false);
...
test("blocks if review result recording preflight is no longer ready", () => {
  const workspace = makeWorkspace();
  const preflight = readJson(
    workspace,
    "trading_lab_step116_manual_order_permission_validation_receipt_review_result_recording_preflight_contract.json",
  );
  preflight.readiness.readyForManualOrderPermissionValidationReceiptReviewResultRecordingPreflight = false;
  writeJson(
    workspace,
    "trading_lab_step116_manual_order_permission_validation_receipt_review_result_recording_preflight_contract.json",
    preflight,
  ...
  assert.equal(result.status, 0, result.stderr);
  const report = readJson(workspace);
  assert.equal(report.readiness.readyForManualOrderPermissionValidationReceiptReviewResultSupply, false);
  assert.match(report.readiness.blockers.join("|"), /validation_receipt_review_result_recording_preflight_not_ready/);
});
