...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-manual-order-permission-validation-result-receipt-review-runbook-contract.cjs",
);
const CONTRACT = "trading_lab_step116_manual_order_permission_validation_result_receipt_review_runbook_contract.json";
const INPUTS = [
  CONTRACT,
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-manual-order-validation-receipt-review-runbook-"));
...
test("passes with current manual order permission validation result receipt review runbook contract", () => {
...
test("records owner-assisted receipt review runbook while keeping validation execution, receipt reads, imports, provider calls, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runRunbook(workspace, []);
  ...
  assert.equal(report.readiness.readyForOwnerAssistedValidationResultReceiptReviewRunbookReview, true);
...
test("keeps runbook command explicit and redacted", () => {
  ...
  assert.match(
    report.futureOwnerAssistedValidationResultReceiptReviewRunbook.validatorCommandTemplate,
    /--receipt <owner-supplied-redacted-validation-result-receipt-path>/,
  );
  assert.match(
    report.futureOwnerAssistedValidationResultReceiptReviewRunbook.reviewPreflightValidatorCommandTemplate,
    /--contract data\/processed\/trading_lab_step116_manual_order_permission_validation_result_receipt_review_preflight\.json/,
  );
  assert.match(
    report.futureOwnerAssistedValidationResultReceiptReviewRunbook.requiredReviewAssertions.join("|"),
...
test("rejects stale runbook if validation, receipt read, import, provider, or order flags are manually enabled", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_manual_order_permission_validation_result_receipt_review_runbook_contract\.json is out of date/,
  );
});
...
test("blocks if private receipt, packet, importer, adapter, route, UI, or scenario artifacts appear", () => {
  const workspace = makeWorkspace();
  const files = [
