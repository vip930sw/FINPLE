...
const SCRIPT_PATH = path.resolve(
  "scripts",
  "generate-trading-read-only-approval-packet-validation-result-receipt-review-result-contract.cjs",
);
const CONTRACT =
...
function makeWorkspace() {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "finple-approval-receipt-review-result-"));
...
test("passes with current read-only approval validation receipt review result contract", () => {
...
test("records redacted approval review result boundary while keeping receipt reads, provider calls, UI, DB, and orders blocked", () => {
  const workspace = makeWorkspace();
  const result = runReviewResult(workspace, []);
  ...
  assert.equal(report.readiness.readyForFutureReadOnlyApprovalValidationResultReceiptReviewResult, true);
...
test("keeps review result schema redacted and requires separate provider authorization review", () => {
  ...
  assert.match(
    report.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.requiredReviewResultAssertions.join("|"),
  ...
  assert.match(
    report.futureReadOnlyApprovalValidationResultReceiptReviewResultBoundary.forbiddenReviewResultContent.join("|"),
...
test("rejects stale review result if receipt, approval payload, provider, or order flags are manually enabled", () => {
  const workspace = makeWorkspace();
  const report = readJson(workspace);
  report.currentState.validationReceiptReviewRecordedNow = true;
  report.currentState.receiptPathRecorded = true;
  report.readiness.approvalPayloadRecorded = true;
  ...
  assert.match(
    result.stderr,
    /trading_lab_step116_read_only_approval_packet_validation_result_receipt_review_result_contract\.json is out of date/,
  );
});
