...
} = require("./validate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight.cjs");
...
const CONTRACT_VERSION =
  "trading-lab-step116-read-only-provider-response-envelope-validation-result-receipt-review-preflight-validator-fixtures-v0.1";
...
function invalidSyntheticFixtures(basePreflight) {
  ...
  const invalidFuturePath = clone(basePreflight);
  invalidFuturePath.futureValidationResultReceiptReviewBoundary.futureValidationResultReceiptPath =
  ...
  allowFlagEnabled.currentState.rawResponseRecorded = true;
  allowFlagEnabled.currentState.providerPayloadRecorded = true;
...
function fixtureValidationEvidence(validPreflight, invalidFixtures) {
  const validResult = validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewPreflight(validPreflight);
  const invalidResults = invalidFixtures.map((fixture) => {
    const result = validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewPreflight(fixture.contract);
...
function buildContract() {
  ...
  const checks = {
    ...
    reviewPreflightReady:
      preflight.readiness?.readyForFutureResponseEnvelopeValidationResultReceiptReview === true &&
      preflight.readiness?.validationReceiptRecordedNow === false &&
      preflight.readiness?.validationReceiptReadAllowedNow === false &&
      preflight.readiness?.responseEnvelopePathRecorded === false &&
      preflight.readiness?.rawResponseRecorded === false &&
      preflight.readiness?.providerPayloadRecorded === false &&
    ...
    validatorExportsLocalValidation:
      validatorSource.includes("validateReadOnlyProviderResponseEnvelopeValidationResultReceiptReviewPreflight") &&
    ...
    architectureDocMentionsReviewPreflightValidatorFixtures:
      architectureDoc.includes("Trading Read-Only Provider Response Envelope Validation Result Receipt Review Preflight Validator Fixtures") &&
      architectureDoc.includes("read_only_provider_response_envelope_validation_result_receipt_review_preflight_validator_fixtures"),
  ...
  const readyForResponseEnvelopeValidationResultReceiptReviewPreflightValidatorRegression =
  ...
  return stableJson({
    ...
    step: "Step 116-3L-F",
    scope: "read_only_provider_response_envelope_validation_result_receipt_review_preflight_validator_fixtures",
    sourceFiles: {
    ...
    validation: {
      redactionBoundary:
        "synthetic response review-preflight validator fixtures only; no real validation receipt, response envelope path, raw account, credential, provider payload, response payload, order payload, token, app key, app secret, execution, fill, or order confirmation content",
    ...
    readiness: {
      status: readyForResponseEnvelopeValidationResultReceiptReviewPreflightValidatorRegression
        ? "fixtures_ready_for_read_only_provider_response_envelope_validation_result_receipt_review_preflight_validator_regression"
        : "blocked_before_read_only_provider_response_envelope_validation_result_receipt_review_preflight_validator_fixture_regression",
      readyForResponseEnvelopeValidationResultReceiptReviewPreflightValidatorRegression,
      fixturesOnly: true,
      validationReceiptRecordedNow: false,
      validationReceiptReadAllowedNow: false,
      rawResponseRecorded: false,
      ...
      blockers: [
        ...
        ...(checks.architectureDocMentionsReviewPreflightValidatorFixtures
          ? []
          : ["architecture_doc_missing_read_only_provider_response_envelope_validation_result_receipt_review_preflight_validator_fixtures"]),
...
function main() {
  ...
  if (checkOnly) {
    if (!fs.existsSync(CONTRACT_PATH)) {
      fail(
        `${CONTRACT_PATH} not found; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight-validator-fixtures.cjs`,
      );
    }
    const current = fs.readFileSync(CONTRACT_PATH, "utf8");
    if (current !== contract) {
      fail(
        `${CONTRACT_PATH} is out of date; run node scripts/generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight-validator-fixtures.cjs`,
      );
    }
    console.log("[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight-validator-fixtures] ok");
    console.log(
      `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight-validator-fixtures] contract=${CONTRACT_PATH}`,
    );
    return;
  }
  ...
  console.log("[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight-validator-fixtures] wrote contract");
  console.log(
    `[generate-trading-read-only-provider-response-envelope-validation-result-receipt-review-preflight-validator-fixtures] readyForResponseEnvelopeValidationResultReceiptReviewPreflightValidatorRegression=${parsed.readiness.readyForResponseEnvelopeValidationResultReceiptReviewPreflightValidatorRegression}`,
  );
}
