...
test("mypage account, billing, and panel UX restore follow-up behavior", async () => {
  ...
  assert.match(cssSource, /--my-page-followup-row-gap: 12px/);
  ...
  assert.match(cssSource, /\.accountPage \.accountPanelStack \.myPageReactPanel \.myPageReactSingleGrid > div[\s\S]*grid-column: 1 \/ -1/);
  assert.match(cssSource, /\.accountPage \.accountPanelStack \.myPageReactPanel \.myPageSummaryGrid strong,[\s\S]*font-size: 17px/);
