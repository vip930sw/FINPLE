...
function filterCandidates({ query, goal, riskLevel, type, beginnerOnly }) {
  ...
  return SCREENER_CANDIDATES.filter((item) => {
    const matchesQuery = !normalizedQuery || [item.ticker, item.koreanName, item.type, item.strategy, ...(item.tags || [])].join(" ").toLowerCase().includes(normalizedQuery);
...
function ScreenerCandidateCard({ item, isAdded, onAdd }) {
  return (
    <article className={isAdded ? "tickerResultCard added" : "tickerResultCard"}>
      ...
      <p className="tickerResultDescription">{item.koreanName} 후보 자산입니다. 시뮬레이터에서 비중을 조정하세요.</p>
      <div className="tickerResultMetaGrid compact">
        <span>유형 {item.type}</span><span>전략 {item.strategy}</span><span>위험 {getRiskLabel(item.riskLevel)}</span><span>CAGR {formatPercentValue(item.expectedCagr)}</span><span>배당 {formatPercentValue(item.dividendYield)}</span><span>MDD {formatPercentValue(item.mdd)}</span>
      </div>
  ...
  return (
    <section className="assetFinderPanel">
      ...
      <div className="assetFinderPresetBar" aria-label="추천 프리셋">
        {PRESET_OPTIONS.map((preset) => <button key={preset.key} type="button" className={activePresetKey === preset.key ? "assetFinderPresetButton active" : "assetFinderPresetButton"} onClick={() => applyPreset(preset)}><strong>{preset.label}</strong><span>{preset.description}</span></button>)}
      </div>

      <form className="tickerSearchForm" onSubmit={(event) => { event.preventDefault(); showResultCount(); }}>
      ...
      <div className="screenerControls">
        ...
        <label className="beginnerOnlyToggle"><input type="checkbox" checked={beginnerOnly} onChange={(event) => { setBeginnerOnly(event.target.checked); setActivePresetKey(null); }} />초보자 적합 우선</label>
        <button type="button" className="secondaryFinderButton" onClick={showResultCount}>조건 적용</button>
        <button type="button" className="primaryFinderButton" onClick={showResultCount}>검색</button>
      </div>

      <div className="assetFinderResultToolbar"><span>{results.length > 0 ? `${results.length}개 후보 표시` : "후보 자산 없음"}</span><small>이미 담긴 티커는 <b>추가됨</b>으로 표시되어 중복 추가되지 않습니다.</small></div>
