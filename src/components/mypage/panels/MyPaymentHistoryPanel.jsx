...
export default function MyPaymentHistoryPanel({ history }) {
  ...
  return (
    <PanelShell
      ...
      actions={<button type="button" className="primaryButton" onClick={history.refresh} disabled={history.loading || history.refreshing}>결제내역 새로고침</button>}
    >
      ...
      <div className="paymentHistoryList">
        {history.payments.length ? history.payments.slice(0, 10).map((payment) => (
