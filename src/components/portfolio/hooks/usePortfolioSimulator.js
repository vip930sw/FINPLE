...
export default function usePortfolioSimulator() {
  ...
  function updateAsset(index, field, value) {
    const nextAssets = [...assets]; const currentAsset = nextAssets[index]; if (!currentAsset) return;
    const currentPlan = getCurrentPlanConfig(); const assetLimit = currentPlan.limits.assetsPerPortfolio;
  ...
  function createPortfolioFromTemplate(templateKey = "default") {
    ...
    const nextAssets = templateMap[templateKey] || DEFAULT_ASSETS;
    const nextPortfolio = createPortfolio({ name: nameMap[templateKey] || "새 포트폴리오", assets: nextAssets, settings });
    pendingTemplateAutoLookupRef.current = templateKey !== "empty";
    setPortfolioList([nextPortfolio, ...portfolioList]); setActivePortfolioId(nextPortfolio.id); setAssets(cloneAssets(nextPortfolio.assets)); setTargetWeightDrafts({}); setIsNewPortfolioMenuOpen(false);
    setAssetLookupSummary(templateKey === "empty" ? "빈 포트폴리오를 생성했습니다." : "프리셋 포트폴리오를 생성했습니다. 자산 데이터를 자동 조회합니다.");
  }
