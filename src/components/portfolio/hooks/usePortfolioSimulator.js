...
export default function usePortfolioSimulator() {
  ...
  useEffect(() => {
    ...
    function applySnapshot(snapshot) {
      ...
      setPortfolioList((previousList) => previousList.map((portfolio) => ({
        ...portfolio,
        assets: portfolio.assets.map((asset, index) =>
          normalizeAsset(hydrateAssetFromScreenerCandidate(asset), index)
        ),
      })));
      setAssets((previousAssets) => previousAssets.map((asset, index) =>
        normalizeAsset(hydrateAssetFromScreenerCandidate(asset), index)
  ...
  function updateAsset(index, field, value) { const nextAssets = [...assets]; const currentAsset = nextAssets[index]; if (!currentAsset) return; const currentPlan = getCurrentPlanConfig(); const assetLimit = currentPlan.limits.assetsPerPortfolio; if (assetLimit && assetLimit !== Infinity && isActivatingEmptyAsset(currentAsset, field, value) && countRealAssets(nextAssets) >= assetLimit) { showPlanLimitNotice("asset"); return; } if (field === "targetWeight") { updateTargetWeightDraft(index, value); return; } if (field === "ticker") { const currentKey = getAssetDraftKey(currentAsset, index); setTargetWeightDrafts((previousDrafts) => { const nextDrafts = { ...previousDrafts }; delete nextDrafts[currentKey]; return nextDrafts; }); const nextTicker = normalizeTicker(value); const previousTicker = normalizeTicker(currentAsset.ticker); const tickerChanged = nextTicker !== previousTicker; nextAssets[index] = normalizeAsset({ ...currentAsset, ticker: nextTicker, name: tickerChanged ? "" : currentAsset.name, price: tickerChanged ? 0 : currentAsset.price, targetEvaluationAmount: tickerChanged ? null : currentAsset.targetEvaluationAmount, cagr: tickerChanged ? 0 : currentAsset.cagr, beta: tickerChanged ? 0 : currentAsset.beta, mdd: tickerChanged ? 0 : currentAsset.mdd, dividendYield: tickerChanged ? null : currentAsset.dividendYield, displayDividendYield: tickerChanged ? "" : currentAsset.displayDividendYield, exposureType: tickerChanged ? "" : currentAsset.exposureType, distributionType: tickerChanged ? "unknown" : currentAsset.distributionType, distributionFrequency: tickerChanged ? "unknown" : currentAsset.distributionFrequency, trailingDistributionYield: tickerChanged ? null : currentAsset.trailingDistributionYield, cashDistributionYieldTtm: tickerChanged ? null : currentAsset.cashDistributionYieldTtm, distributionYieldPolicy: tickerChanged ? "" : currentAsset.distributionYieldPolicy, distributionCalculationStatus: tickerChanged ? "" : currentAsset.distributionCalculationStatus, priceMode: tickerChanged ? "manual" : currentAsset.priceMode, metricMode: tickerChanged ? "manual" : currentAsset.metricMode, dataSource: tickerChanged ? "manual" : currentAsset.dataSource, cacheMode: tickerChanged ? null : currentAsset.cacheMode, rawPrice: tickerChanged ? null : currentAsset.rawPrice, rawCurrency: tickerChanged ? null : currentAsset.rawCurrency, exchangeRate: tickerChanged ? null : currentAsset.exchangeRate, lastUpdatedAt: tickerChanged ? null : currentAsset.lastUpdatedAt }, index); } else { nextAssets[index] = { ...currentAsset, [field]: value }; } setAssets(nextAssets); }
  ...
  function applyTickerCandidateToAsset(currentAsset, candidate = {}, index = assets.length) { const ticker = normalizeTicker(candidate.ticker || currentAsset.ticker); const currentPrice = Number(currentAsset.price || 0); const currentQuantity = Number(currentAsset.quantity || 0); const previewPatch = createAssetPatchFromScreenerCandidate(candidate); return normalizeAsset({ ...currentAsset, ...previewPatch, ticker, name: candidate.koreanName || candidate.name || currentAsset.name || ticker, market: candidate.market || currentAsset.market || "US", currency: currentAsset.currency || "KRW", quantity: currentQuantity, price: currentPrice, targetEvaluationAmount: currentAsset.targetEvaluationAmount ?? null, cagr: candidate.expectedCagr ?? candidate.cagr ?? currentAsset.cagr ?? (candidate.internalPreviewReviewOnly ? null : 0), beta: candidate.beta ?? currentAsset.beta ?? (candidate.internalPreviewReviewOnly ? null : 0), mdd: candidate.mdd ?? currentAsset.mdd ?? (candidate.internalPreviewReviewOnly ? null : 0), dividendYield: isNonOrdinaryDistribution(previewPatch) ? null : previewPatch.dividendYield ?? currentAsset.dividendYield ?? null, priceMode: currentPrice > 0 ? currentAsset.priceMode : "lookup-required", metricMode: candidate.metricMode || "manual", dataSource: candidate.dataSource || "ticker-master", cacheMode: null, rawPrice: currentAsset.rawPrice || null, rawCurrency: candidate.currency || currentAsset.rawCurrency || null, exchangeRate: currentAsset.exchangeRate || null, lastUpdatedAt: currentAsset.lastUpdatedAt || null }, index); }
  async function resolveTickerCandidate(index, options = {}) { const targetAsset = assets[index]; const ticker = normalizeTicker(options.ticker || targetAsset?.ticker); if (!ticker) return null; try { const candidate = await fetchTickerCandidateByTicker(ticker); setAssets((previousAssets) => { const nextAssets = [...previousAssets]; const currentAsset = nextAssets[index]; if (!currentAsset || normalizeTicker(currentAsset.ticker) !== ticker) return previousAssets; nextAssets[index] = applyTickerCandidateToAsset(currentAsset, candidate, index); return nextAssets; }); if (!options.silent) setAssetLookupSummary(`${ticker} 티커 마스터 정보 적용. 비중을 입력하고 계산 버튼을 누르면 평가금액이 반영됩니다.`); return candidate; } catch (error) { if (!options.silent) setAssetLookupSummary(`${ticker}는 티커 마스터에서 찾지 못했습니다. 직접 입력값으로 유지합니다.`); return null; } }
  function applyFetchedAssetData(currentAsset, assetData, index) { const nextTicker = assetData.ticker || currentAsset.ticker; const normalizedTicker = normalizeTicker(nextTicker); const fetchedName = assetData.name || ""; const currentName = currentAsset.name || ""; const fetchedNameIsTickerOnly = normalizeTicker(fetchedName) === normalizedTicker; const currentNameIsTickerOnly = normalizeTicker(currentName) === normalizeTicker(currentAsset.ticker); const nextName = fetchedName && (!fetchedNameIsTickerOnly || !currentName || currentNameIsTickerOnly) ? fetchedName : currentName; const nextPrice = assetData.price !== null && assetData.price !== undefined ? Number(assetData.price) : Number(currentAsset.price || 0); const plannedValue = getAssetPlannedValue(currentAsset); const nextQuantity = plannedValue > 0 && nextPrice > 0 ? Number((plannedValue / nextPrice).toFixed(6)) : currentAsset.quantity; const preservePreviewNulls = assetData.internalPreviewReviewOnly === true; const nonOrdinaryDistribution = isNonOrdinaryDistribution(currentAsset) || isNonOrdinaryDistribution(assetData); return normalizeAsset({ ...currentAsset, ticker: nextTicker, name: nextName, market: assetData.market || currentAsset.market, currency: assetData.currency || currentAsset.currency, quantity: nextQuantity, price: assetData.price !== null && assetData.price !== undefined ? assetData.price : currentAsset.price, targetEvaluationAmount: plannedValue > 0 ? plannedValue : currentAsset.targetEvaluationAmount, cagr: preservePreviewNulls ? assetData.cagr : assetData.cagr ?? currentAsset.cagr, beta: preservePreviewNulls ? assetData.beta : assetData.beta ?? currentAsset.beta, mdd: preservePreviewNulls ? assetData.mdd : assetData.mdd ?? currentAsset.mdd, dividendYield: nonOrdinaryDistribution ? null : preservePreviewNulls ? assetData.dividendYield : preserveNullableNumber(assetData.dividendYield, currentAsset.dividendYield), priceMode: assetData.priceMode || currentAsset.priceMode, metricMode: assetData.metricMode || currentAsset.metricMode, dataSource: assetData.dataSource || currentAsset.dataSource, cacheMode: assetData.cacheMode || currentAsset.cacheMode || null, rawPrice: assetData.rawPrice !== null && assetData.rawPrice !== undefined ? assetData.rawPrice : currentAsset.rawPrice, rawCurrency: assetData.rawCurrency || currentAsset.rawCurrency || null, exchangeRate: assetData.exchangeRate !== null && assetData.exchangeRate !== undefined ? assetData.exchangeRate : currentAsset.exchangeRate, lastUpdatedAt: assetData.fetchedAt || currentAsset.lastUpdatedAt }, index); }
  ...
  function createAssetFromTickerCandidate(candidate = {}, index = assets.length) { const market = candidate.market || "US"; const previewPatch = createAssetPatchFromScreenerCandidate(candidate); return normalizeAsset({ ...previewPatch, ticker: candidate.ticker || "", name: candidate.koreanName || candidate.name || candidate.ticker || "", market, exchange: candidate.exchange, currency: candidate.currency || "KRW", quoteCurrency: candidate.quoteCurrency || (market === "KR" ? "KRW" : "USD"), assetType: candidate.assetType || candidate.type || "ETF", quantity: 0, price: 0, targetEvaluationAmount: null, cagr: candidate.expectedCagr ?? candidate.cagr ?? (candidate.internalPreviewReviewOnly ? null : 0), beta: candidate.beta ?? (candidate.internalPreviewReviewOnly ? null : 0), mdd: candidate.mdd ?? (candidate.internalPreviewReviewOnly ? null : 0), dividendYield: previewPatch.dividendYield, priceMode: market === "KR" ? "manual" : "lookup-required", metricMode: candidate.metricMode || "manual", dataSource: candidate.dataSource || "ticker-master", cacheMode: null, rawPrice: null, rawCurrency: candidate.quoteCurrency || candidate.currency || (market === "KR" ? "KRW" : "USD"), exchangeRate: null, lastUpdatedAt: null }, index); }
  function addAssetFromTickerCandidate(candidate) { const ticker = normalizeTicker(candidate?.ticker); if (!ticker) { setAssetLookupSummary("추가할 티커 정보가 없습니다."); return { status: "error", message: "추가할 티커 정보가 없습니다." }; } const alreadyExists = assets.some((asset) => { const tickerValue = normalizeTicker(asset?.ticker); return tickerValue === ticker && !isEmptyAssetRow(asset); }); if (alreadyExists) { const message = `${ticker}는 이미 현재 포트폴리오에 추가되어 있습니다.`; setAssetLookupSummary(message); return { status: "duplicate", ticker, message }; } const currentPlan = getCurrentPlanConfig(); const assetLimit = currentPlan.limits.assetsPerPortfolio; const activeAssetCount = countRealAssets(assets); if (assetLimit && assetLimit !== Infinity && activeAssetCount >= assetLimit) { const message = showPlanLimitNotice("asset"); return { status: "limit", ticker, message }; } const nextAsset = createAssetFromTickerCandidate(candidate, assets.length); setAssets((previousAssets) => { const emptyIndex = previousAssets.findIndex((asset) => { const tickerValue = normalizeTicker(asset?.ticker); return !tickerValue || isEmptyAssetRow(asset); }); if (emptyIndex >= 0) return previousAssets.map((asset, index) => index === emptyIndex ? normalizeAsset(nextAsset, index) : asset); return [...previousAssets, normalizeAsset(nextAsset, previousAssets.length)]; }); setRecentlyAddedAssetId(nextAsset.id); window.setTimeout(() => setRecentlyAddedAssetId(null), 4200); const marketLabel = nextAsset.market === "KR" ? "한국" : "미국"; const message = `${ticker} ${marketLabel} 후보 자산을 현재 포트폴리오에 추가했습니다. 비중을 입력하고 계산 버튼을 누르세요.`; setAssetLookupSummary(message); return { status: "success", ticker, asset: nextAsset, message }; }
