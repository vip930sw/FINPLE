...
function buildInputBase({ activePortfolio, assets, settings, monthlyReturns }) {
  ...
  const selectedMonths = Number.isInteger(investmentMonths) && investmentMonths > 0
    ? availableMonths.slice(-investmentMonths)
    : availableMonths;
  const monthlyReturnMatrix = selectedMonths.flatMap((month) =>
    scenarioAssets.map((asset) => {
      const manualCash = isManualCashAsset(asset);
      const row = manualCash ? null : seriesMaps[identities.indexOf(identityForAsset(asset))]?.get(month);
      return {
        month,
...
export function buildStep5ProductionScenarioState({
  ...
  try {
    ...
    return {
      ...
      error: error?.message || "step5_input_adapter_error",
    };
  }
