...
- 화면은 2열 카드 그리드를 사용하며, 현재 3개 응답에는 UI 전용 `해석 범위` 보조 카드를 붙여 `2 x 2`로 표시한다. 이 보조 카드는 API output contract에 포함되지 않으며, 향후 contract가 4개로 확장되면 별도 보조 카드 없이 응답 카드만 `2 x 2`로 표시할 수 있다.
