...
6. 저장소 git status 불변
7. 외부 임시 디렉터리에서 build 완료 후 staging 원자적 교체
...
npx.cmd --yes vercel@56.4.1 link --yes --project finple --cwd $Stage
```
...
```powershell
npx.cmd --yes vercel@56.4.1 deploy --prebuilt --archive=tgz --yes --cwd $Stage
```
...
응답 본문, cookie, 인증 header를 PR에 붙이지 않는다. QA에는 HTTP 상태, 크기, SHA-256 일치 여부만 기록한다.
