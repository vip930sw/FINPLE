...
- AI 품질 평가셋을 v3로 확정했습니다. fixture별 `evaluationFocus`와 전역 `AI_ANALYSIS_EVALUATION_CRITERIA`를 두어 시장(US/KR), 데이터 상태, 위험 초점, output guard, live sample alignment 커버리지를 테스트에서 강제합니다.
...
다음 우선순위:
1. 최신 main과 운영 배포 commit 확인 - 완료
2. status API에서 usage/access가 정상 노출되는지 확인 - 완료
3. Personal 전용 모드 전환 QA 및 프론트 안내 UX 보강 - 완료
4. 관리자 AI 사용량 요약 UI 연결 - 완료
5. 포트폴리오 AI 분석 품질 평가셋 및 regression fixture 설계 - v3 완료
6. 비용/사용량 운영 정책 확정 - 완료

마무리 상태:
- `npm.cmd run check:ai-production`로 Render/Vercel/status smoke 확인
- 운영 계정으로 free/personal/education 화면 UX 최종 육안 확인 완료
- 관리자 토큰으로 `/admin/ai-usage` 실데이터 확인 완료
- 실제 live OpenAI 응답 샘플을 반영해 fixture v3 확정
