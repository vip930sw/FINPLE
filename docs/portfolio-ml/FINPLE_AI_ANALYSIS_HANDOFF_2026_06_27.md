...
5. 포트폴리오 AI 분석 품질 평가셋 및 regression fixture 설계 - v2 완료
6. 비용/사용량 운영 정책 확정 - 완료

마무리 단계:
- `npm.cmd run check:ai-production -- --commit=<최신 main short sha>`로 Render/Vercel/status smoke 확인
- 운영 계정으로 free/personal/education 화면 UX 최종 육안 확인
- 관리자 토큰으로 `/admin/ai-usage` 실데이터 주기 확인
- 실제 live OpenAI 응답 샘플을 축적해 fixture v3 후보 선별
