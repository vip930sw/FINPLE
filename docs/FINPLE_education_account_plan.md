...
- 관리자 콘솔에서 일괄 생성, 조회, CSV 출력, 비활성화, 만료일 변경, 상태 변경을 할 수 있습니다.
...
- `server/src/db/educationAccountRepository.js`
  - 일괄 교육계정 생성, 자동 비밀번호 생성, CSV 생성, 상태/만료일 변경, 교육 권한 적용/회수를 담당합니다.
...
- `server/src/routes/adminRoutes.js`
  - 교육계정 목록, CSV, 일괄 생성, 상태 변경 API를 제공합니다.
...
4. `일괄 생성` 실행
5. 화면의 전달용 패널에서 교육용 ID와 초기 비밀번호 확인
6. 생성된 CSV를 인쇄용 명단 또는 수강생 전달 자료로 사용
...
- 비밀번호는 해시 저장됩니다.
- 초기 비밀번호는 일괄 생성 직후 전달용 패널과 CSV로만 제한적으로 노출합니다.
